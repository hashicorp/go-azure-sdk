...
type SqlScriptRenameSqlScriptOperationResponse struct {
...
func (c SqlScriptsClient) SqlScriptRenameSqlScript(ctx context.Context, id SqlScriptId, input ArtifactRenameRequest) (result SqlScriptRenameSqlScriptOperationResponse, err error) {
...
func (c SqlScriptsClient) SqlScriptRenameSqlScriptThenPoll(ctx context.Context, id SqlScriptId, input ArtifactRenameRequest) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after SqlScriptRenameSqlScript: %+v", err)
	}
