...
func TestPollerProvisioningState_VCRErrorHandling(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			resourceManagerClient.SetTransport(errRoundTripper{errMsg: testCase.errMsg}, client.TransportModeVCRReplay)
			...
			if testCase.expectError {
				...
				if testCase.expectReplayMiss != client.IsVCRReplayMissErrorDeprecated(err) {
					t.Fatalf("expected IsVCRReplayMissError=%t, got %t (err=%v)", testCase.expectReplayMiss, client.IsVCRReplayMissErrorDeprecated(err), err)
				}
