...
func PossibleValuesForIngressTargetPortHTTPScheme() []string {
	return []string{
		string(IngressTargetPortHTTPSchemeHTTP),
		string(IngressTargetPortHTTPSchemeHTTPS),
	}
}
...
func (s *IngressTargetPortHTTPScheme) UnmarshalJSON(bytes []byte) error {
	...
	out, err := parseIngressTargetPortHTTPScheme(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseIngressTargetPortHTTPScheme(input string) (*IngressTargetPortHTTPScheme, error) {
	vals := map[string]IngressTargetPortHTTPScheme{
		"http":  IngressTargetPortHTTPSchemeHTTP,
		"https": IngressTargetPortHTTPSchemeHTTPS,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := IngressTargetPortHTTPScheme(input)
	return &out, nil
}
...
func PossibleValuesForPatchingMode() []string {
	return []string{
		string(PatchingModeAutomatic),
		string(PatchingModeDisabled),
		string(PatchingModeManual),
	}
}
...
func parsePatchingMode(input string) (*PatchingMode, error) {
	vals := map[string]PatchingMode{
		"automatic": PatchingModeAutomatic,
		"disabled":  PatchingModeDisabled,
		"manual":    PatchingModeManual,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
