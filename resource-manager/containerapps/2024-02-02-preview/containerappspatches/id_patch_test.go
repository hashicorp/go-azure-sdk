...
func TestParsePatchID(t *testing.T) {
	...
	for _, v := range testData {
		...
		actual, err := ParsePatchID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.PatchName != v.Expected.PatchName {
			t.Fatalf("Expected %q but got %q for PatchName", v.Expected.PatchName, actual.PatchName)
		}

	}
}

func TestParsePatchIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *PatchId
	...
	for _, v := range testData {
		...
		actual, err := ParsePatchIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForPatchId(t *testing.T) {
	segments := PatchId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("PatchId has no segments")
	}
