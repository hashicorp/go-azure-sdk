...
func TestParseSessionPoolID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.App/sessionPools",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseSessionPoolID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.SessionPoolName != v.Expected.SessionPoolName {
			t.Fatalf("Expected %q but got %q for SessionPoolName", v.Expected.SessionPoolName, actual.SessionPoolName)
		}

	}
}

func TestParseSessionPoolIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SessionPoolId
	...
	for _, v := range testData {
		...
		actual, err := ParseSessionPoolIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSessionPoolId(t *testing.T) {
	segments := SessionPoolId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SessionPoolId has no segments")
	}
