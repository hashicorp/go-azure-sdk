...
func TestParseConnectedEnvironmentStorageID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseConnectedEnvironmentStorageID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseConnectedEnvironmentStorageIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ConnectedEnvironmentStorageId
	...
	for _, v := range testData {
		...
		actual, err := ParseConnectedEnvironmentStorageIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForConnectedEnvironmentStorageId(t *testing.T) {
	segments := ConnectedEnvironmentStorageId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ConnectedEnvironmentStorageId has no segments")
	}
