...
func TestParseRevisionsApiRevisionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseRevisionsApiRevisionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseRevisionsApiRevisionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *RevisionsApiRevisionId
	...
	for _, v := range testData {
		...
		actual, err := ParseRevisionsApiRevisionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForRevisionsApiRevisionId(t *testing.T) {
	segments := RevisionsApiRevisionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("RevisionsApiRevisionId has no segments")
	}
