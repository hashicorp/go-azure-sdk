...
func PossibleValuesForLinuxLicenseType() []string {
	return []string{
		string(LinuxLicenseTypeLinuxServer),
		string(LinuxLicenseTypeNoLicenseType),
		string(LinuxLicenseTypeNotSpecified),
	}
}

func (s *LinuxLicenseType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseLinuxLicenseType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseLinuxLicenseType(input string) (*LinuxLicenseType, error) {
	vals := map[string]LinuxLicenseType{
		"linuxserver":   LinuxLicenseTypeLinuxServer,
		"nolicensetype": LinuxLicenseTypeNoLicenseType,
		"notspecified":  LinuxLicenseTypeNotSpecified,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
