...
func TestNewReplicationProtectionClusterID(t *testing.T) {
	id := NewReplicationProtectionClusterID("12345678-1234-9876-4563-123456789012", "example-resource-group", "vaultName", "replicationFabricName", "replicationProtectionContainerName", "replicationProtectionClusterName")
...
func TestFormatReplicationProtectionClusterID(t *testing.T) {
	actual := NewReplicationProtectionClusterID("12345678-1234-9876-4563-123456789012", "example-resource-group", "vaultName", "replicationFabricName", "replicationProtectionContainerName", "replicationProtectionClusterName").ID()
...
func TestParseReplicationProtectionClusterID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ReplicationProtectionClusterId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.RecoveryServices/vaults/vaultName/replicationFabrics/replicationFabricName/replicationProtectionContainers/replicationProtectionContainerName/replicationProtectionClusters",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseReplicationProtectionClusterID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseReplicationProtectionClusterIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ReplicationProtectionClusterId
	...
	for _, v := range testData {
		...
		actual, err := ParseReplicationProtectionClusterIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForReplicationProtectionClusterId(t *testing.T) {
	segments := ReplicationProtectionClusterId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ReplicationProtectionClusterId has no segments")
	}
