...
// RawJobDetailsImpl is returned when the Discriminated Value
...
func unmarshalJobDetailsImplementation(input []byte) (JobDetails, error) {
	...
	if strings.EqualFold(value, "ClusterFailoverJobDetails") {
		var out ClusterFailoverJobDetails
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into ClusterFailoverJobDetails: %+v", err)
		}
		return out, nil
	}

	if strings.EqualFold(value, "ClusterSwitchProtectionJobDetails") {
		var out ClusterSwitchProtectionJobDetails
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into ClusterSwitchProtectionJobDetails: %+v", err)
		}
		return out, nil
	}

	if strings.EqualFold(value, "ClusterTestFailoverJobDetails") {
		var out ClusterTestFailoverJobDetails
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into ClusterTestFailoverJobDetails: %+v", err)
		}
		return out, nil
	}
