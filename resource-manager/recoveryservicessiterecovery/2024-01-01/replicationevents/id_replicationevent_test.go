...
func TestParseReplicationEventID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseReplicationEventID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ReplicationEventName != v.Expected.ReplicationEventName {
			t.Fatalf("Expected %q but got %q for ReplicationEventName", v.Expected.ReplicationEventName, actual.ReplicationEventName)
		}

	}
}

func TestParseReplicationEventIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ReplicationEventId
	...
	for _, v := range testData {
		...
		actual, err := ParseReplicationEventIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForReplicationEventId(t *testing.T) {
	segments := ReplicationEventId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ReplicationEventId has no segments")
	}
