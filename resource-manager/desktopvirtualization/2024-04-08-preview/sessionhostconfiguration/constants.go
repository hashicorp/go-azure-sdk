...
func PossibleValuesForProvisioningStateSHC() []string {
	return []string{
		string(ProvisioningStateSHCCanceled),
		string(ProvisioningStateSHCFailed),
		string(ProvisioningStateSHCProvisioning),
		string(ProvisioningStateSHCSucceeded),
	}
}
...
func (s *ProvisioningStateSHC) UnmarshalJSON(bytes []byte) error {
	...
	out, err := parseProvisioningStateSHC(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseProvisioningStateSHC(input string) (*ProvisioningStateSHC, error) {
	vals := map[string]ProvisioningStateSHC{
		"canceled":     ProvisioningStateSHCCanceled,
		"failed":       ProvisioningStateSHCFailed,
		"provisioning": ProvisioningStateSHCProvisioning,
		"succeeded":    ProvisioningStateSHCSucceeded,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := ProvisioningStateSHC(input)
	return &out, nil
}
