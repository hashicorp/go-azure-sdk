...
func TestParseSolutionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SolutionId
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.OperationsManagement/solutions",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseSolutionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseSolutionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SolutionId
	...
	for _, v := range testData {
		...
		actual, err := ParseSolutionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSolutionId(t *testing.T) {
	segments := SolutionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SolutionId has no segments")
	}
