...
func TestParseManagementConfigurationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.OperationsManagement",
			Error: true,
		},
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.OperationsManagement/managementConfigurations",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseManagementConfigurationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ManagementConfigurationName != v.Expected.ManagementConfigurationName {
			t.Fatalf("Expected %q but got %q for ManagementConfigurationName", v.Expected.ManagementConfigurationName, actual.ManagementConfigurationName)
		}

	}
}

func TestParseManagementConfigurationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ManagementConfigurationId
	...
	for _, v := range testData {
		...
		actual, err := ParseManagementConfigurationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForManagementConfigurationId(t *testing.T) {
	segments := ManagementConfigurationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ManagementConfigurationId has no segments")
	}
