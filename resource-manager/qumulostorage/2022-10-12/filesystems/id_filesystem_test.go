...
func TestParseFileSystemID(t *testing.T) {
	testData := []struct {
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Qumulo.Storage",
			Error: true,
		},
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Qumulo.Storage/fileSystems",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseFileSystemID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.FileSystemName != v.Expected.FileSystemName {
			t.Fatalf("Expected %q but got %q for FileSystemName", v.Expected.FileSystemName, actual.FileSystemName)
		}

	}
}

func TestParseFileSystemIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *FileSystemId
	...
	for _, v := range testData {
		...
		actual, err := ParseFileSystemIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForFileSystemId(t *testing.T) {
	segments := FileSystemId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("FileSystemId has no segments")
	}
