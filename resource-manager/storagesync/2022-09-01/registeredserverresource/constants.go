...
func PossibleValuesForServerAuthType() []string {
	return []string{
		string(ServerAuthTypeCertificate),
		string(ServerAuthTypeManagedIdentity),
	}
}
...
func (s *ServerAuthType) UnmarshalJSON(bytes []byte) error {
	...
	out, err := parseServerAuthType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseServerAuthType(input string) (*ServerAuthType, error) {
	vals := map[string]ServerAuthType{
		"certificate":     ServerAuthTypeCertificate,
		"managedidentity": ServerAuthTypeManagedIdentity,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
