...
func TestParseBareMetalMachineKeySetID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseBareMetalMachineKeySetID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.BareMetalMachineKeySetName != v.Expected.BareMetalMachineKeySetName {
			t.Fatalf("Expected %q but got %q for BareMetalMachineKeySetName", v.Expected.BareMetalMachineKeySetName, actual.BareMetalMachineKeySetName)
		}

	}
}

func TestParseBareMetalMachineKeySetIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *BareMetalMachineKeySetId
	...
	for _, v := range testData {
		...
		actual, err := ParseBareMetalMachineKeySetIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForBareMetalMachineKeySetId(t *testing.T) {
	segments := BareMetalMachineKeySetId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("BareMetalMachineKeySetId has no segments")
	}
