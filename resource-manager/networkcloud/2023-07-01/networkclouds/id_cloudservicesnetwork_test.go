...
func TestParseCloudServicesNetworkID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.NetworkCloud/cloudServicesNetworks",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseCloudServicesNetworkID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.CloudServicesNetworkName != v.Expected.CloudServicesNetworkName {
			t.Fatalf("Expected %q but got %q for CloudServicesNetworkName", v.Expected.CloudServicesNetworkName, actual.CloudServicesNetworkName)
		}

	}
}

func TestParseCloudServicesNetworkIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CloudServicesNetworkId
	...
	for _, v := range testData {
		...
		actual, err := ParseCloudServicesNetworkIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForCloudServicesNetworkId(t *testing.T) {
	segments := CloudServicesNetworkId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("CloudServicesNetworkId has no segments")
	}
