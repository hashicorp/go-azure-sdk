...
func TestParseL3NetworkID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.NetworkCloud/l3Networks",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseL3NetworkID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.L3NetworkName != v.Expected.L3NetworkName {
			t.Fatalf("Expected %q but got %q for L3NetworkName", v.Expected.L3NetworkName, actual.L3NetworkName)
		}

	}
}

func TestParseL3NetworkIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *L3NetworkId
	...
	for _, v := range testData {
		...
		actual, err := ParseL3NetworkIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForL3NetworkId(t *testing.T) {
	segments := L3NetworkId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("L3NetworkId has no segments")
	}
