...
func TestParseClusterManagerID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.NetworkCloud/clusterManagers",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseClusterManagerID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ClusterManagerName != v.Expected.ClusterManagerName {
			t.Fatalf("Expected %q but got %q for ClusterManagerName", v.Expected.ClusterManagerName, actual.ClusterManagerName)
		}

	}
}

func TestParseClusterManagerIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ClusterManagerId
	...
	for _, v := range testData {
		...
		actual, err := ParseClusterManagerIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForClusterManagerId(t *testing.T) {
	segments := ClusterManagerId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ClusterManagerId has no segments")
	}
