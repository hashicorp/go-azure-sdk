...
func TestParseTrunkedNetworkID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.NetworkCloud/trunkedNetworks",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseTrunkedNetworkID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.TrunkedNetworkName != v.Expected.TrunkedNetworkName {
			t.Fatalf("Expected %q but got %q for TrunkedNetworkName", v.Expected.TrunkedNetworkName, actual.TrunkedNetworkName)
		}

	}
}

func TestParseTrunkedNetworkIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *TrunkedNetworkId
	...
	for _, v := range testData {
		...
		actual, err := ParseTrunkedNetworkIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForTrunkedNetworkId(t *testing.T) {
	segments := TrunkedNetworkId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("TrunkedNetworkId has no segments")
	}
