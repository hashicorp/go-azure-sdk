...
func TestParseL2NetworkID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseL2NetworkID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.L2NetworkName != v.Expected.L2NetworkName {
			t.Fatalf("Expected %q but got %q for L2NetworkName", v.Expected.L2NetworkName, actual.L2NetworkName)
		}

	}
}

func TestParseL2NetworkIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *L2NetworkId
	...
	for _, v := range testData {
		...
		actual, err := ParseL2NetworkIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForL2NetworkId(t *testing.T) {
	segments := L2NetworkId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("L2NetworkId has no segments")
	}
