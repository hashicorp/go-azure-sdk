...
func TestParseStorageApplianceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.NetworkCloud/storageAppliances",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseStorageApplianceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.StorageApplianceName != v.Expected.StorageApplianceName {
			t.Fatalf("Expected %q but got %q for StorageApplianceName", v.Expected.StorageApplianceName, actual.StorageApplianceName)
		}

	}
}

func TestParseStorageApplianceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *StorageApplianceId
	...
	for _, v := range testData {
		...
		actual, err := ParseStorageApplianceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForStorageApplianceId(t *testing.T) {
	segments := StorageApplianceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("StorageApplianceId has no segments")
	}
