...
func TestParseServiceFabricID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseServiceFabricID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseServiceFabricIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ServiceFabricId
	...
	for _, v := range testData {
		...
		actual, err := ParseServiceFabricIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForServiceFabricId(t *testing.T) {
	segments := ServiceFabricId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ServiceFabricId has no segments")
	}
