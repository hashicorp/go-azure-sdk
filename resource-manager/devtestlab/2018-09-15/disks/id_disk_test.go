...
func TestParseDiskID(t *testing.T) {
	testData := []struct {
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.DevTestLab/labs/labValue/users/userValue/disks",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseDiskID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseDiskIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DiskId
	...
	for _, v := range testData {
		...
		actual, err := ParseDiskIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDiskId(t *testing.T) {
	segments := DiskId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DiskId has no segments")
	}
