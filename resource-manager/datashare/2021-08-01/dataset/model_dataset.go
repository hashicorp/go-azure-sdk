...
func UnmarshalDataSetImplementation(input []byte) (DataSet, error) {
	...
	if strings.EqualFold(value, "KustoTable") {
		var out KustoTableDataSet
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into KustoTableDataSet: %+v", err)
		}
		return out, nil
	}
	...
	if strings.EqualFold(value, "SynapseWorkspaceSqlPoolTable") {
		var out SynapseWorkspaceSqlPoolTableDataSet
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into SynapseWorkspaceSqlPoolTableDataSet: %+v", err)
		}
		return out, nil
	}
