...
func TestParseSynchronizationSettingID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseSynchronizationSettingID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.SynchronizationSettingName != v.Expected.SynchronizationSettingName {
			t.Fatalf("Expected %q but got %q for SynchronizationSettingName", v.Expected.SynchronizationSettingName, actual.SynchronizationSettingName)
		}

	}
}

func TestParseSynchronizationSettingIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SynchronizationSettingId
	...
	for _, v := range testData {
		...
		actual, err := ParseSynchronizationSettingIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSynchronizationSettingId(t *testing.T) {
	segments := SynchronizationSettingId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SynchronizationSettingId has no segments")
	}
