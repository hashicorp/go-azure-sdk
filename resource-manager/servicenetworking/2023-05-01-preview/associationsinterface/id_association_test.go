...
func TestParseAssociationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseAssociationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseAssociationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AssociationId
	...
	for _, v := range testData {
		...
		actual, err := ParseAssociationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForAssociationId(t *testing.T) {
	segments := AssociationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("AssociationId has no segments")
	}
