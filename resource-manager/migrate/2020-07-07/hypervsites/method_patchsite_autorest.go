...
func (c HyperVSitesClient) PatchSite(ctx context.Context, id HyperVSiteId, input HyperVSite) (result PatchSiteOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "hypervsites.HyperVSitesClient", "PatchSite", nil, "Failure preparing request")
		return
	}

	result.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "hypervsites.HyperVSitesClient", "PatchSite", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForPatchSite(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "hypervsites.HyperVSitesClient", "PatchSite", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c HyperVSitesClient) preparerForPatchSite(ctx context.Context, id HyperVSiteId, input HyperVSite) (*http.Request, error) {
