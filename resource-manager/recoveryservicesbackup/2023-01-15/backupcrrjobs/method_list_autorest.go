...
// List ...
func (c BackupCrrJobsClient) List(ctx context.Context, id LocationId, input CrrJobRequest, options ListOperationOptions) (resp ListOperationResponse, err error) {
	req, err := c.preparerForList(ctx, id, input, options)
	if err != nil {
		err = autorest.NewErrorWithError(err, "backupcrrjobs.BackupCrrJobsClient", "List", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "backupcrrjobs.BackupCrrJobsClient", "List", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "backupcrrjobs.BackupCrrJobsClient", "List", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c BackupCrrJobsClient) preparerForList(ctx context.Context, id LocationId, input CrrJobRequest, options ListOperationOptions) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/backupCrrJobs", id.ID())),
...
func (c BackupCrrJobsClient) preparerForListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c BackupCrrJobsClient) responderForList(resp *http.Response) (result ListOperationResponse, err error) {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "backupcrrjobs.BackupCrrJobsClient", "List", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "backupcrrjobs.BackupCrrJobsClient", "List", result.HttpResponse, "Failure responding to request")
				return
			}
