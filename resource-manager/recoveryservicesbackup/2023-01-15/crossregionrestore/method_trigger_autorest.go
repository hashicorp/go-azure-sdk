...
func (c CrossRegionRestoreClient) Trigger(ctx context.Context, id LocationId, input CrossRegionRestoreRequest) (result TriggerOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "crossregionrestore.CrossRegionRestoreClient", "Trigger", nil, "Failure preparing request")
		return
	}

	result, err = c.senderForTrigger(ctx, req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "crossregionrestore.CrossRegionRestoreClient", "Trigger", result.HttpResponse, "Failure sending request")
		return
	}
...
func (c CrossRegionRestoreClient) preparerForTrigger(ctx context.Context, id LocationId, input CrossRegionRestoreRequest) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/backupCrossRegionRestore", id.ID())),
...
func (c CrossRegionRestoreClient) senderForTrigger(ctx context.Context, req *http.Request) (future TriggerOperationResponse, err error) {
