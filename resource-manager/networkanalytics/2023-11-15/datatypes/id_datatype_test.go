...
func TestParseDataTypeID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DataTypeId
	...
	for _, v := range testData {
		...
		actual, err := ParseDataTypeID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.DataTypeName != v.Expected.DataTypeName {
			t.Fatalf("Expected %q but got %q for DataTypeName", v.Expected.DataTypeName, actual.DataTypeName)
		}

	}
}

func TestParseDataTypeIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DataTypeId
	...
	for _, v := range testData {
		...
		actual, err := ParseDataTypeIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDataTypeId(t *testing.T) {
	segments := DataTypeId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DataTypeId has no segments")
	}
