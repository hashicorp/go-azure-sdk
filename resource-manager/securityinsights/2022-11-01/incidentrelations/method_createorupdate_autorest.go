...
func (c IncidentRelationsClient) CreateOrUpdate(ctx context.Context, id RelationId, input Relation) (result CreateOrUpdateOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "incidentrelations.IncidentRelationsClient", "CreateOrUpdate", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCreateOrUpdate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "incidentrelations.IncidentRelationsClient", "CreateOrUpdate", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c IncidentRelationsClient) responderForCreateOrUpdate(resp *http.Response) (result CreateOrUpdateOperationResponse, err error) {
