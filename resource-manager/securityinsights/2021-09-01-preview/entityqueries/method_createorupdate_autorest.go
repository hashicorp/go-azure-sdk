...
func (c EntityQueriesClient) CreateOrUpdate(ctx context.Context, id EntityQueriesId, input CustomEntityQuery) (result CreateOrUpdateOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "entityqueries.EntityQueriesClient", "CreateOrUpdate", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCreateOrUpdate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "entityqueries.EntityQueriesClient", "CreateOrUpdate", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c EntityQueriesClient) responderForCreateOrUpdate(resp *http.Response) (result CreateOrUpdateOperationResponse, err error) {
	...
	if err != nil {
		return result, fmt.Errorf("reading response body for EntityQuery: %+v", err)
	}
	model, err := unmarshalEntityQueryImplementation(b)
	if err != nil {
		return
	}
	result.Model = &model
