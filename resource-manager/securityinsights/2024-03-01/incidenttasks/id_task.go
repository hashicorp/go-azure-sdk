...
func (id *TaskId) FromParseResult(input resourceids.ParseResult) error {
	...
	if id.IncidentTaskId, ok = input.Parsed["incidentTaskId"]; !ok {
		return resourceids.NewSegmentNotSpecifiedError(id, "incidentTaskId", input)
	}
...
func (id TaskId) ID() string {
	fmtString := "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.OperationalInsights/workspaces/%s/providers/Microsoft.SecurityInsights/incidents/%s/tasks/%s"
	return fmt.Sprintf(fmtString, id.SubscriptionId, id.ResourceGroupName, id.WorkspaceName, id.IncidentId, id.IncidentTaskId)
}
...
func (id TaskId) String() string {
	components := []string{
		...
		fmt.Sprintf("Incident Task: %q", id.IncidentTaskId),
	}
