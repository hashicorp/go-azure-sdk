...
func ParseIncidentID(input string) (*IncidentId, error) {
	...
	if id.IncidentIdentifier, ok = parsed.Parsed["incidentIdentifier"]; !ok {
		return nil, fmt.Errorf("the segment 'incidentIdentifier' was not found in the resource id %q", input)
	}
