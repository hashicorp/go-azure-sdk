...
func init() {
	recaser.RegisterResourceId(&EntityQueryTemplateId{})
}
...
func (id *EntityQueryTemplateId) FromParseResult(input resourceids.ParseResult) error {
	...
	if id.EntityQueryTemplateId, ok = input.Parsed["entityQueryTemplateId"]; !ok {
		return resourceids.NewSegmentNotSpecifiedError(id, "entityQueryTemplateId", input)
	}
