...
func (id *CommentId) FromParseResult(input resourceids.ParseResult) error {
	...
	if id.HuntCommentId, ok = input.Parsed["huntCommentId"]; !ok {
		return resourceids.NewSegmentNotSpecifiedError(id, "huntCommentId", input)
	}
...
func (id CommentId) ID() string {
	fmtString := "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.OperationalInsights/workspaces/%s/providers/Microsoft.SecurityInsights/hunts/%s/comments/%s"
	return fmt.Sprintf(fmtString, id.SubscriptionId, id.ResourceGroupName, id.WorkspaceName, id.HuntId, id.HuntCommentId)
}
...
func (id CommentId) Segments() []resourceids.Segment {
	return []resourceids.Segment{
		...
		resourceids.UserSpecifiedSegment("huntCommentId", "huntCommentId"),
	}
}
...
func (id CommentId) String() string {
	components := []string{
		...
		fmt.Sprintf("Hunt Comment: %q", id.HuntCommentId),
	}
