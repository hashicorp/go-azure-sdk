...
var _ json.Unmarshaler = &BookmarkExpandResponseValue{}
...
func (s *BookmarkExpandResponseValue) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling BookmarkExpandResponseValue into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["entities"]; ok {
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'Entities' for 'BookmarkExpandResponseValue': %+v", i, err)
			}
