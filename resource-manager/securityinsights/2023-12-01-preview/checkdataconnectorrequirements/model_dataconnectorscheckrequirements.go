...
func (s RawDataConnectorsCheckRequirementsImpl) MarshalJSON() ([]byte, error) {
...
func UnmarshalDataConnectorsCheckRequirementsImplementation(input []byte) (DataConnectorsCheckRequirements, error) {
	...
	if strings.EqualFold(value, "MicrosoftPurviewInformationProtection") {
		var out MicrosoftPurviewInformationProtectionCheckRequirements
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into MicrosoftPurviewInformationProtectionCheckRequirements: %+v", err)
		}
		return out, nil
	}
