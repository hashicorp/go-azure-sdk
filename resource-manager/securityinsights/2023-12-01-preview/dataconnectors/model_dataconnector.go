...
func UnmarshalDataConnectorImplementation(input []byte) (DataConnector, error) {
	...
	if strings.EqualFold(value, "GCP") {
		var out GCPDataConnector
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into GCPDataConnector: %+v", err)
		}
		return out, nil
	}
	...
	if strings.EqualFold(value, "MicrosoftPurviewInformationProtection") {
		var out MicrosoftPurviewInformationProtectionDataConnector
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into MicrosoftPurviewInformationProtectionDataConnector: %+v", err)
		}
		return out, nil
	}
