...
func (c BillingStatisticsClient) List(ctx context.Context, id WorkspaceId) (result ListOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/providers/Microsoft.SecurityInsights/billingStatistics", id.ID()),
	}
	...
	temp := make([]BillingStatistic, 0)
	if values.Values != nil {
		for i, v := range *values.Values {
			val, err := UnmarshalBillingStatisticImplementation(v)
			if err != nil {
				err = fmt.Errorf("unmarshalling item %d for BillingStatistic (%q): %+v", i, v, err)
				return result, err
			}
...
func (c BillingStatisticsClient) ListComplete(ctx context.Context, id WorkspaceId) (ListCompleteResult, error) {
	return c.ListCompleteMatchingPredicate(ctx, id, BillingStatisticOperationPredicate{})
}
