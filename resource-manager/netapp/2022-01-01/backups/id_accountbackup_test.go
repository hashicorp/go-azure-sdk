...
func TestParseAccountBackupID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseAccountBackupID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseAccountBackupIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AccountBackupId
	...
	for _, v := range testData {
		...
		actual, err := ParseAccountBackupIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForAccountBackupId(t *testing.T) {
	segments := AccountBackupId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("AccountBackupId has no segments")
	}
