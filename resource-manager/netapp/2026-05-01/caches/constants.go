...
func PossibleValuesForCacheFileAccessLogs() []string {
	return []string{
		string(CacheFileAccessLogsDisabled),
		string(CacheFileAccessLogsEnabled),
	}
}

func (s *CacheFileAccessLogs) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseCacheFileAccessLogs(decoded)
	if err != nil {
...
func parseCacheFileAccessLogs(input string) (*CacheFileAccessLogs, error) {
	vals := map[string]CacheFileAccessLogs{
		"disabled": CacheFileAccessLogsDisabled,
		"enabled":  CacheFileAccessLogsEnabled,
	}
