...
// ListByMobileNetwork ...
func (c SlicesClient) ListByMobileNetwork(ctx context.Context, id MobileNetworkId) (resp ListByMobileNetworkOperationResponse, err error) {
	req, err := c.preparerForListByMobileNetwork(ctx, id)
	if err != nil {
		err = autorest.NewErrorWithError(err, "slices.SlicesClient", "ListByMobileNetwork", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "slices.SlicesClient", "ListByMobileNetwork", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByMobileNetwork(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "slices.SlicesClient", "ListByMobileNetwork", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c SlicesClient) preparerForListByMobileNetwork(ctx context.Context, id MobileNetworkId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/slices", id.ID())),
...
func (c SlicesClient) preparerForListByMobileNetworkWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c SlicesClient) responderForListByMobileNetwork(resp *http.Response) (result ListByMobileNetworkOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByMobileNetworkOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "slices.SlicesClient", "ListByMobileNetwork", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByMobileNetwork(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "slices.SlicesClient", "ListByMobileNetwork", result.HttpResponse, "Failure responding to request")
				return
			}
...
func (c SlicesClient) ListByMobileNetworkCompleteMatchingPredicate(ctx context.Context, id MobileNetworkId, predicate SliceOperationPredicate) (resp ListByMobileNetworkCompleteResult, err error) {
	items := make([]Slice, 0)
