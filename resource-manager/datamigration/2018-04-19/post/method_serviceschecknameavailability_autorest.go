...
func (c POSTClient) ServicesCheckNameAvailability(ctx context.Context, id LocationId, input NameAvailabilityRequest) (result ServicesCheckNameAvailabilityOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "post.POSTClient", "ServicesCheckNameAvailability", nil, "Failure preparing request")
		return
	}

	result.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "post.POSTClient", "ServicesCheckNameAvailability", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForServicesCheckNameAvailability(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "post.POSTClient", "ServicesCheckNameAvailability", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c POSTClient) preparerForServicesCheckNameAvailability(ctx context.Context, id LocationId, input NameAvailabilityRequest) (*http.Request, error) {
...
func (c POSTClient) responderForServicesCheckNameAvailability(resp *http.Response) (result ServicesCheckNameAvailabilityOperationResponse, err error) {
