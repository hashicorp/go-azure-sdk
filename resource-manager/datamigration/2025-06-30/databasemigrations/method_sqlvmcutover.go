...
type SqlVMcutoverOperationResponse struct {
...
// SqlVMcutover ...
func (c DatabaseMigrationsClient) SqlVMcutover(ctx context.Context, id SqlVirtualMachineProviders2DatabaseMigrationId, input MigrationOperationInput) (result SqlVMcutoverOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:       fmt.Sprintf("%s/cutover", id.ID()),
	}
...
func (c DatabaseMigrationsClient) SqlVMcutoverThenPoll(ctx context.Context, id SqlVirtualMachineProviders2DatabaseMigrationId, input MigrationOperationInput) error {
	...
	if err := result.Poller.PollUntilDone(ctx); err != nil {
		return fmt.Errorf("polling after SqlVMcutover: %+v", err)
	}
