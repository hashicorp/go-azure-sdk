...
func unmarshalCommandPropertiesImplementation(input []byte) (CommandProperties, error) {
	...
	if strings.EqualFold(value, "cancel") {
		var out MongoDbCancelCommand
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into MongoDbCancelCommand: %+v", err)
		}
		return out, nil
	}

	if strings.EqualFold(value, "finish") {
		var out MongoDbFinishCommand
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into MongoDbFinishCommand: %+v", err)
		}
		return out, nil
	}

	if strings.EqualFold(value, "restart") {
		var out MongoDbRestartCommand
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into MongoDbRestartCommand: %+v", err)
		}
		return out, nil
	}
