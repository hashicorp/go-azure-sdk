...
// UsagesList ...
func (c GETClient) UsagesList(ctx context.Context, id LocationId) (resp UsagesListOperationResponse, err error) {
	req, err := c.preparerForUsagesList(ctx, id)
	if err != nil {
		err = autorest.NewErrorWithError(err, "get.GETClient", "UsagesList", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "get.GETClient", "UsagesList", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForUsagesList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "get.GETClient", "UsagesList", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c GETClient) preparerForUsagesList(ctx context.Context, id LocationId) (*http.Request, error) {
...
func (c GETClient) preparerForUsagesListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c GETClient) responderForUsagesList(resp *http.Response) (result UsagesListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result UsagesListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "get.GETClient", "UsagesList", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForUsagesList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "get.GETClient", "UsagesList", result.HttpResponse, "Failure responding to request")
				return
			}
...
func (c GETClient) UsagesListCompleteMatchingPredicate(ctx context.Context, id LocationId, predicate QuotaOperationPredicate) (resp UsagesListCompleteResult, err error) {
	items := make([]Quota, 0)
