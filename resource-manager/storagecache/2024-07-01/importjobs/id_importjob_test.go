...
func TestParseImportJobID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseImportJobID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ImportJobName != v.Expected.ImportJobName {
			t.Fatalf("Expected %q but got %q for ImportJobName", v.Expected.ImportJobName, actual.ImportJobName)
		}

	}
}

func TestParseImportJobIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ImportJobId
	...
	for _, v := range testData {
		...
		actual, err := ParseImportJobIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForImportJobId(t *testing.T) {
	segments := ImportJobId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ImportJobId has no segments")
	}
