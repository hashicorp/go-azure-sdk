...
func TestParseVirtualHardDiskID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.AzureStackHCI/virtualHardDisks",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseVirtualHardDiskID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.VirtualHardDiskName != v.Expected.VirtualHardDiskName {
			t.Fatalf("Expected %q but got %q for VirtualHardDiskName", v.Expected.VirtualHardDiskName, actual.VirtualHardDiskName)
		}

	}
}

func TestParseVirtualHardDiskIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VirtualHardDiskId
	...
	for _, v := range testData {
		...
		actual, err := ParseVirtualHardDiskIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForVirtualHardDiskId(t *testing.T) {
	segments := VirtualHardDiskId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("VirtualHardDiskId has no segments")
	}
