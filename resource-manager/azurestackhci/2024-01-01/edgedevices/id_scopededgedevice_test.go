...
func TestParseScopedEdgeDeviceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/some-resource-group/providers/Microsoft.AzureStackHCI",
			Error: true,
		},
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/some-resource-group/providers/Microsoft.AzureStackHCI/edgeDevices",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedEdgeDeviceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.EdgeDeviceName != v.Expected.EdgeDeviceName {
			t.Fatalf("Expected %q but got %q for EdgeDeviceName", v.Expected.EdgeDeviceName, actual.EdgeDeviceName)
		}

	}
}

func TestParseScopedEdgeDeviceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ScopedEdgeDeviceId
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedEdgeDeviceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForScopedEdgeDeviceId(t *testing.T) {
	segments := ScopedEdgeDeviceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ScopedEdgeDeviceId has no segments")
	}
