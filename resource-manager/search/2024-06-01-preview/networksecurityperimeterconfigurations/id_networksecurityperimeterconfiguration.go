...
func (id NetworkSecurityPerimeterConfigurationId) ID() string {
	fmtString := "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Search/searchServices/%s/networkSecurityPerimeterConfigurations/%s"
	return fmt.Sprintf(fmtString, id.SubscriptionId, id.ResourceGroupName, id.SearchServiceName, id.NetworkSecurityPerimeterConfigurationName)
}
