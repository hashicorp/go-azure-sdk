...
func TestParseScriptActionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseScriptActionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ScriptActionName != v.Expected.ScriptActionName {
			t.Fatalf("Expected %q but got %q for ScriptActionName", v.Expected.ScriptActionName, actual.ScriptActionName)
		}

	}
}

func TestParseScriptActionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ScriptActionId
	...
	for _, v := range testData {
		...
		actual, err := ParseScriptActionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForScriptActionId(t *testing.T) {
	segments := ScriptActionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ScriptActionId has no segments")
	}
