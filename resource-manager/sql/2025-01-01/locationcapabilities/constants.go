...
func PossibleValuesForCapabilityGroup() []string {
	return []string{
		...
		string(CapabilityGroupSupportedJobAgentVersions),
...
func parseCapabilityGroup(input string) (*CapabilityGroup, error) {
	vals := map[string]CapabilityGroup{
		...
		"supportedjobagentversions":        CapabilityGroupSupportedJobAgentVersions,
...
func PossibleValuesForStorageCapabilityStorageAccountType() []string {
	return []string{
		string(StorageCapabilityStorageAccountTypeGRS),
		string(StorageCapabilityStorageAccountTypeGZRS),
		string(StorageCapabilityStorageAccountTypeLRS),
		string(StorageCapabilityStorageAccountTypeZRS),
	}
}

func (s *StorageCapabilityStorageAccountType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseStorageCapabilityStorageAccountType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseStorageCapabilityStorageAccountType(input string) (*StorageCapabilityStorageAccountType, error) {
	vals := map[string]StorageCapabilityStorageAccountType{
		"grs":  StorageCapabilityStorageAccountTypeGRS,
		"gzrs": StorageCapabilityStorageAccountTypeGZRS,
		"lrs":  StorageCapabilityStorageAccountTypeLRS,
		"zrs":  StorageCapabilityStorageAccountTypeZRS,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := StorageCapabilityStorageAccountType(input)
	return &out, nil
}
