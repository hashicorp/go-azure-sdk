...
func TestParseVirtualClusterID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Sql/virtualClusters",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseVirtualClusterID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.VirtualClusterName != v.Expected.VirtualClusterName {
			t.Fatalf("Expected %q but got %q for VirtualClusterName", v.Expected.VirtualClusterName, actual.VirtualClusterName)
		}

	}
}

func TestParseVirtualClusterIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VirtualClusterId
	...
	for _, v := range testData {
		...
		actual, err := ParseVirtualClusterIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForVirtualClusterId(t *testing.T) {
	segments := VirtualClusterId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("VirtualClusterId has no segments")
	}
