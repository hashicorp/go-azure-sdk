...
func TestParseManagedInstanceRestorableDroppedDatabaseID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseManagedInstanceRestorableDroppedDatabaseID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseManagedInstanceRestorableDroppedDatabaseIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ManagedInstanceRestorableDroppedDatabaseId
	...
	for _, v := range testData {
		...
		actual, err := ParseManagedInstanceRestorableDroppedDatabaseIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForManagedInstanceRestorableDroppedDatabaseId(t *testing.T) {
	segments := ManagedInstanceRestorableDroppedDatabaseId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ManagedInstanceRestorableDroppedDatabaseId has no segments")
	}
