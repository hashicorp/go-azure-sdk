...
func ParseInstanceFailoverGroupID(input string) (*InstanceFailoverGroupId, error) {
	...
	if id.InstanceFailoverGroupName, ok = parsed.Parsed["instanceFailoverGroupName"]; !ok {
		return nil, resourceids.NewSegmentNotSpecifiedError(id, "instanceFailoverGroupName", *parsed)
	}
