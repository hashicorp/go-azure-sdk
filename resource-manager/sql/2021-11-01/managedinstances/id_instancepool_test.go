...
func TestParseInstancePoolID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseInstancePoolID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseInstancePoolIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *InstancePoolId
	...
	for _, v := range testData {
		...
		actual, err := ParseInstancePoolIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForInstancePoolId(t *testing.T) {
	segments := InstancePoolId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("InstancePoolId has no segments")
	}
