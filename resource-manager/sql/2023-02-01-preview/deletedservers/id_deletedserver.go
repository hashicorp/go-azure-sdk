...
func init() {
	recaser.RegisterResourceId(&DeletedServerId{})
}
...
func (id *DeletedServerId) FromParseResult(input resourceids.ParseResult) error {
	...
	if id.DeletedServerName, ok = input.Parsed["deletedServerName"]; !ok {
		return resourceids.NewSegmentNotSpecifiedError(id, "deletedServerName", input)
	}
