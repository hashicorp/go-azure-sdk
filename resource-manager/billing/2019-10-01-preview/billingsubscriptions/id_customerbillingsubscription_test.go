...
func TestParseCustomerBillingSubscriptionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseCustomerBillingSubscriptionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseCustomerBillingSubscriptionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CustomerBillingSubscriptionId
	...
	for _, v := range testData {
		...
		actual, err := ParseCustomerBillingSubscriptionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForCustomerBillingSubscriptionId(t *testing.T) {
	segments := CustomerBillingSubscriptionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("CustomerBillingSubscriptionId has no segments")
	}
