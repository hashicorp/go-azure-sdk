...
func init() {
	recaser.RegisterResourceId(&SavingsPlanId{})
}
...
func (id SavingsPlanId) ID() string {
	fmtString := "/providers/Microsoft.Billing/billingAccounts/%s/savingsPlanOrders/%s/savingsPlans/%s"
	return fmt.Sprintf(fmtString, id.BillingAccountName, id.SavingsPlanOrderId, id.SavingsPlanId)
}
