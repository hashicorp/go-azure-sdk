...
func PossibleValuesForDnsZoneType() []string {
	return []string{
		string(DnsZoneTypePrivate),
		string(DnsZoneTypePublic),
	}
}
...
func (s *DnsZoneType) UnmarshalJSON(bytes []byte) error {
	...
	out, err := parseDnsZoneType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseDnsZoneType(input string) (*DnsZoneType, error) {
	vals := map[string]DnsZoneType{
		"private": DnsZoneTypePrivate,
		"public":  DnsZoneTypePublic,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
