...
func PossibleValuesForResourceSkuResourceType() []string {
	return []string{
		string(ResourceSkuResourceTypePrivateClouds),
		string(ResourceSkuResourceTypePrivateCloudsClusters),
	}
}

func (s *ResourceSkuResourceType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseResourceSkuResourceType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseResourceSkuResourceType(input string) (*ResourceSkuResourceType, error) {
	vals := map[string]ResourceSkuResourceType{
		"privateclouds":          ResourceSkuResourceTypePrivateClouds,
		"privateclouds/clusters": ResourceSkuResourceTypePrivateCloudsClusters,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := ResourceSkuResourceType(input)
	return &out, nil
}
