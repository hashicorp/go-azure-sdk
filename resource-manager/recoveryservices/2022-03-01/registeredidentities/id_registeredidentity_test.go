...
func TestParseRegisteredIdentityID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.RecoveryServices/vaults/vaultValue/registeredIdentities",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseRegisteredIdentityID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseRegisteredIdentityIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *RegisteredIdentityId
	...
	for _, v := range testData {
		...
		actual, err := ParseRegisteredIdentityIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForRegisteredIdentityId(t *testing.T) {
	segments := RegisteredIdentityId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("RegisteredIdentityId has no segments")
	}
