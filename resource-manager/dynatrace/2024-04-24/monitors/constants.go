...
func PossibleValuesForAction() []string {
	return []string{
		string(ActionInstall),
		string(ActionUninstall),
	}
}
...
func PossibleValuesForMarketplaceSaasAutoRenew() []string {
	return []string{
		string(MarketplaceSaasAutoRenewOff),
		string(MarketplaceSaasAutoRenewOn),
	}
}

func (s *MarketplaceSaasAutoRenew) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseMarketplaceSaasAutoRenew(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseMarketplaceSaasAutoRenew(input string) (*MarketplaceSaasAutoRenew, error) {
	vals := map[string]MarketplaceSaasAutoRenew{
		"off": MarketplaceSaasAutoRenewOff,
		"on":  MarketplaceSaasAutoRenewOn,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := MarketplaceSaasAutoRenew(input)
	return &out, nil
}
...
func PossibleValuesForMonitoringType() []string {
	return []string{
		...
		string(MonitoringTypeDISCOVERY),
...
func parseMonitoringType(input string) (*MonitoringType, error) {
	vals := map[string]MonitoringType{
		...
		"discovery":            MonitoringTypeDISCOVERY,
