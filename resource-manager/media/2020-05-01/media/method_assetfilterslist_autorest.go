...
func (c MediaClient) AssetFiltersList(ctx context.Context, id AssetId) (resp AssetFiltersListOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetFiltersList", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetFiltersList", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForAssetFiltersList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetFiltersList", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c MediaClient) preparerForAssetFiltersListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c MediaClient) responderForAssetFiltersList(resp *http.Response) (result AssetFiltersListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result AssetFiltersListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetFiltersList", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForAssetFiltersList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetFiltersList", result.HttpResponse, "Failure responding to request")
				return
			}
