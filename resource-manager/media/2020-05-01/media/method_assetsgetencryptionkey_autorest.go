...
func (c MediaClient) AssetsGetEncryptionKey(ctx context.Context, id AssetId) (result AssetsGetEncryptionKeyOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetsGetEncryptionKey", nil, "Failure preparing request")
		return
	}

	result.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetsGetEncryptionKey", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForAssetsGetEncryptionKey(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetsGetEncryptionKey", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c MediaClient) preparerForAssetsGetEncryptionKey(ctx context.Context, id AssetId) (*http.Request, error) {
...
func (c MediaClient) responderForAssetsGetEncryptionKey(resp *http.Response) (result AssetsGetEncryptionKeyOperationResponse, err error) {
