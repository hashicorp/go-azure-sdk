...
func (c MediaClient) JobsList(ctx context.Context, id TransformId, options JobsListOperationOptions) (resp JobsListOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "JobsList", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "JobsList", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForJobsList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "JobsList", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c MediaClient) preparerForJobsListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c MediaClient) responderForJobsList(resp *http.Response) (result JobsListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result JobsListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "media.MediaClient", "JobsList", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForJobsList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "media.MediaClient", "JobsList", result.HttpResponse, "Failure responding to request")
				return
			}
