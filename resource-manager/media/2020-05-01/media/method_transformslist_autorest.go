...
func (c MediaClient) TransformsList(ctx context.Context, id MediaServiceId, options TransformsListOperationOptions) (resp TransformsListOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "TransformsList", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "TransformsList", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForTransformsList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "TransformsList", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c MediaClient) preparerForTransformsListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c MediaClient) responderForTransformsList(resp *http.Response) (result TransformsListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result TransformsListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "media.MediaClient", "TransformsList", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForTransformsList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "media.MediaClient", "TransformsList", result.HttpResponse, "Failure responding to request")
				return
			}
