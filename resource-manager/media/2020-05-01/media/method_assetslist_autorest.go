...
func (c MediaClient) AssetsList(ctx context.Context, id MediaServiceId, options AssetsListOperationOptions) (resp AssetsListOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetsList", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetsList", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForAssetsList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetsList", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c MediaClient) preparerForAssetsList(ctx context.Context, id MediaServiceId, options AssetsListOperationOptions) (*http.Request, error) {
...
func (c MediaClient) preparerForAssetsListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c MediaClient) responderForAssetsList(resp *http.Response) (result AssetsListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result AssetsListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetsList", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForAssetsList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "media.MediaClient", "AssetsList", result.HttpResponse, "Failure responding to request")
				return
			}
