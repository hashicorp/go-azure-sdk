...
func TestParseAzureTrafficCollectorID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseAzureTrafficCollectorID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseAzureTrafficCollectorIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AzureTrafficCollectorId
	...
	for _, v := range testData {
		...
		actual, err := ParseAzureTrafficCollectorIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForAzureTrafficCollectorId(t *testing.T) {
	segments := AzureTrafficCollectorId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("AzureTrafficCollectorId has no segments")
	}
