...
func NewClientWithBaseURI(api sdkEnv.Api, configureFunc func(c *resourcemanager.Client)) (*Client, error) {
	autoScaleVCoresClient, err := autoscalevcores.NewAutoScaleVCoresClientWithBaseURI(api)
	if err != nil {
		return nil, fmt.Errorf("building AutoScaleVCores client: %+v", err)
	}
	configureFunc(autoScaleVCoresClient.Client)

	capacitiesClient, err := capacities.NewCapacitiesClientWithBaseURI(api)
	if err != nil {
		return nil, fmt.Errorf("building Capacities client: %+v", err)
	}
	configureFunc(capacitiesClient.Client)

	powerBIDedicatedClient, err := powerbidedicated.NewPowerBIDedicatedClientWithBaseURI(api)
	if err != nil {
