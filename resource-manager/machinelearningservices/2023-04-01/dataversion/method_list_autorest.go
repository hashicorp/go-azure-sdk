...
// List ...
func (c DataVersionClient) List(ctx context.Context, id WorkspaceDataId, options ListOperationOptions) (resp ListOperationResponse, err error) {
	req, err := c.preparerForList(ctx, id, options)
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataversion.DataVersionClient", "List", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataversion.DataVersionClient", "List", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForList(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataversion.DataVersionClient", "List", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c DataVersionClient) preparerForList(ctx context.Context, id WorkspaceDataId, options ListOperationOptions) (*http.Request, error) {
...
func (c DataVersionClient) preparerForListWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c DataVersionClient) responderForList(resp *http.Response) (result ListOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "dataversion.DataVersionClient", "List", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForList(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "dataversion.DataVersionClient", "List", result.HttpResponse, "Failure responding to request")
				return
			}
