...
func PossibleValuesForFineTuningTaskType() []string {
	return []string{
		string(FineTuningTaskTypeChatCompletion),
		string(FineTuningTaskTypeImageClassification),
		string(FineTuningTaskTypeImageInstanceSegmentation),
		string(FineTuningTaskTypeImageObjectDetection),
		string(FineTuningTaskTypeQuestionAnswering),
		string(FineTuningTaskTypeTextClassification),
		string(FineTuningTaskTypeTextCompletion),
		string(FineTuningTaskTypeTextSummarization),
		string(FineTuningTaskTypeTextTranslation),
		string(FineTuningTaskTypeTokenClassification),
		string(FineTuningTaskTypeVideoMultiObjectTracking),
	}
}

func (s *FineTuningTaskType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseFineTuningTaskType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseFineTuningTaskType(input string) (*FineTuningTaskType, error) {
	vals := map[string]FineTuningTaskType{
		"chatcompletion":            FineTuningTaskTypeChatCompletion,
		"imageclassification":       FineTuningTaskTypeImageClassification,
		"imageinstancesegmentation": FineTuningTaskTypeImageInstanceSegmentation,
		"imageobjectdetection":      FineTuningTaskTypeImageObjectDetection,
		"questionanswering":         FineTuningTaskTypeQuestionAnswering,
		"textclassification":        FineTuningTaskTypeTextClassification,
		"textcompletion":            FineTuningTaskTypeTextCompletion,
		"textsummarization":         FineTuningTaskTypeTextSummarization,
		"texttranslation":           FineTuningTaskTypeTextTranslation,
		"tokenclassification":       FineTuningTaskTypeTokenClassification,
		"videomultiobjecttracking":  FineTuningTaskTypeVideoMultiObjectTracking,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
...
func PossibleValuesForModelProvider() []string {
	return []string{
...
func (s *ModelProvider) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseModelProvider(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseModelProvider(input string) (*ModelProvider, error) {
	vals := map[string]ModelProvider{
		"azureopenai": ModelProviderAzureOpenAI,
		"custom":      ModelProviderCustom,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
