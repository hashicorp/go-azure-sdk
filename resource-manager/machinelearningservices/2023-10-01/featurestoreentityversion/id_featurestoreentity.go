...
func ParseFeatureStoreEntityID(input string) (*FeatureStoreEntityId, error) {
	...
	if id.FeatureStoreEntityName, ok = parsed.Parsed["featureStoreEntityName"]; !ok {
		return nil, resourceids.NewSegmentNotSpecifiedError(id, "featureStoreEntityName", *parsed)
	}
