...
func TestParseCodeID(t *testing.T) {
	...
	for _, v := range testData {
		...
		actual, err := ParseCodeID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseCodeIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CodeId
	...
	for _, v := range testData {
		...
		actual, err := ParseCodeIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForCodeId(t *testing.T) {
	segments := CodeId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("CodeId has no segments")
	}
