...
type BatchDeployment struct {
	...
	DeploymentConfiguration   BatchDeploymentConfiguration `json:"deploymentConfiguration"`
...
func (s *BatchDeployment) UnmarshalJSON(bytes []byte) error {
	...
	if v, ok := temp["deploymentConfiguration"]; ok {
		impl, err := UnmarshalBatchDeploymentConfigurationImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'DeploymentConfiguration' for 'BatchDeployment': %+v", err)
		}
		s.DeploymentConfiguration = impl
	}
