...
func PossibleValuesForExportFormatType() []string {
	return []string{
		string(ExportFormatTypeCSV),
		string(ExportFormatTypeCoco),
		string(ExportFormatTypeDataset),
	}
}

func (s *ExportFormatType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseExportFormatType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseExportFormatType(input string) (*ExportFormatType, error) {
	vals := map[string]ExportFormatType{
		"csv":     ExportFormatTypeCSV,
		"coco":    ExportFormatTypeCoco,
		"dataset": ExportFormatTypeDataset,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
