...
func unmarshalDatastoreSecretsImplementation(input []byte) (DatastoreSecrets, error) {
	...
	if strings.EqualFold(value, "KerberosKeytab") {
		var out KerberosKeytabSecrets
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into KerberosKeytabSecrets: %+v", err)
		}
		return out, nil
	}

	if strings.EqualFold(value, "KerberosPassword") {
		var out KerberosPasswordSecrets
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into KerberosPasswordSecrets: %+v", err)
		}
		return out, nil
	}
