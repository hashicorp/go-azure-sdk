...
func (c AppPlatformClient) ServicesCheckNameAvailability(ctx context.Context, id LocationId, input NameAvailabilityParameters) (result ServicesCheckNameAvailabilityOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "appplatform.AppPlatformClient", "ServicesCheckNameAvailability", nil, "Failure preparing request")
		return
	}

	result.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "appplatform.AppPlatformClient", "ServicesCheckNameAvailability", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForServicesCheckNameAvailability(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "appplatform.AppPlatformClient", "ServicesCheckNameAvailability", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c AppPlatformClient) preparerForServicesCheckNameAvailability(ctx context.Context, id LocationId, input NameAvailabilityParameters) (*http.Request, error) {
...
func (c AppPlatformClient) responderForServicesCheckNameAvailability(resp *http.Response) (result ServicesCheckNameAvailabilityOperationResponse, err error) {
