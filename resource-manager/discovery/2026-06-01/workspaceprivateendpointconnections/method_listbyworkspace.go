...
type ListByWorkspaceCompleteResult struct {
	...
	Items              []WorkspacePrivateEndpointConnection
}
...
func (c WorkspacePrivateEndpointConnectionsClient) ListByWorkspace(ctx context.Context, id WorkspaceId) (result ListByWorkspaceOperationResponse, err error) {
	...
	var values struct {
		Values *[]WorkspacePrivateEndpointConnection `json:"value"`
	}
...
func (c WorkspacePrivateEndpointConnectionsClient) ListByWorkspaceComplete(ctx context.Context, id WorkspaceId) (ListByWorkspaceCompleteResult, error) {
	return c.ListByWorkspaceCompleteMatchingPredicate(ctx, id, WorkspacePrivateEndpointConnectionOperationPredicate{})
}

// ListByWorkspaceCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c WorkspacePrivateEndpointConnectionsClient) ListByWorkspaceCompleteMatchingPredicate(ctx context.Context, id WorkspaceId, predicate WorkspacePrivateEndpointConnectionOperationPredicate) (result ListByWorkspaceCompleteResult, err error) {
	items := make([]WorkspacePrivateEndpointConnection, 0)
