...
func TestNewCodeSigningAccountID(t *testing.T) {
	id := NewCodeSigningAccountID("12345678-1234-9876-4563-123456789012", "example-resource-group", "codeSigningAccountName")
...
func TestFormatCodeSigningAccountID(t *testing.T) {
	actual := NewCodeSigningAccountID("12345678-1234-9876-4563-123456789012", "example-resource-group", "codeSigningAccountName").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.CodeSigning/codeSigningAccounts/codeSigningAccountName"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseCodeSigningAccountID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CodeSigningAccountId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseCodeSigningAccountID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseCodeSigningAccountIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CodeSigningAccountId
	...
	for _, v := range testData {
		...
		actual, err := ParseCodeSigningAccountIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForCodeSigningAccountId(t *testing.T) {
	segments := CodeSigningAccountId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("CodeSigningAccountId has no segments")
	}
