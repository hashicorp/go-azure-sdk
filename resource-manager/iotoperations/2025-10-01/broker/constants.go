...
func PossibleValuesForBrokerPersistencePolicyMode() []string {
	return []string{
		string(BrokerPersistencePolicyModeAll),
		string(BrokerPersistencePolicyModeCustom),
		string(BrokerPersistencePolicyModeNone),
	}
}

func (s *BrokerPersistencePolicyMode) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseBrokerPersistencePolicyMode(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseBrokerPersistencePolicyMode(input string) (*BrokerPersistencePolicyMode, error) {
	vals := map[string]BrokerPersistencePolicyMode{
		"all":    BrokerPersistencePolicyModeAll,
		"custom": BrokerPersistencePolicyModeCustom,
		"none":   BrokerPersistencePolicyModeNone,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := BrokerPersistencePolicyMode(input)
	return &out, nil
}
...
func PossibleValuesForBrokerStateStoreKeyType() []string {
	return []string{
		string(BrokerStateStoreKeyTypeBinary),
		string(BrokerStateStoreKeyTypePattern),
		string(BrokerStateStoreKeyTypeString),
	}
}

func (s *BrokerStateStoreKeyType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseBrokerStateStoreKeyType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseBrokerStateStoreKeyType(input string) (*BrokerStateStoreKeyType, error) {
	vals := map[string]BrokerStateStoreKeyType{
		"binary":  BrokerStateStoreKeyTypeBinary,
		"pattern": BrokerStateStoreKeyTypePattern,
		"string":  BrokerStateStoreKeyTypeString,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	...
	// otherwise presume it's an undefined value and best-effort it
	out := BrokerStateStoreKeyType(input)
	return &out, nil
}
