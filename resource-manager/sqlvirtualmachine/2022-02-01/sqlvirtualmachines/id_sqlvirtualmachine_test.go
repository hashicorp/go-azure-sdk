...
func TestParseSqlVirtualMachineID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseSqlVirtualMachineID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseSqlVirtualMachineIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SqlVirtualMachineId
	...
	for _, v := range testData {
		...
		actual, err := ParseSqlVirtualMachineIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSqlVirtualMachineId(t *testing.T) {
	segments := SqlVirtualMachineId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SqlVirtualMachineId has no segments")
	}
