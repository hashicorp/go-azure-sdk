...
func TestParseGitLabGroupID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseGitLabGroupID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGitLabGroupIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GitLabGroupId
	...
	for _, v := range testData {
		...
		actual, err := ParseGitLabGroupIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGitLabGroupId(t *testing.T) {
	segments := GitLabGroupId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GitLabGroupId has no segments")
	}
