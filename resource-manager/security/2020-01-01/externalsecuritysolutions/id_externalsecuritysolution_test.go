...
func TestParseExternalSecuritySolutionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseExternalSecuritySolutionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ExternalSecuritySolutionName != v.Expected.ExternalSecuritySolutionName {
			t.Fatalf("Expected %q but got %q for ExternalSecuritySolutionName", v.Expected.ExternalSecuritySolutionName, actual.ExternalSecuritySolutionName)
		}

	}
}

func TestParseExternalSecuritySolutionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ExternalSecuritySolutionId
	...
	for _, v := range testData {
		...
		actual, err := ParseExternalSecuritySolutionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForExternalSecuritySolutionId(t *testing.T) {
	segments := ExternalSecuritySolutionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ExternalSecuritySolutionId has no segments")
	}
