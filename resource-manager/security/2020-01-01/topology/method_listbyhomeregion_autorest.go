...
func (c TopologyClient) ListByHomeRegion(ctx context.Context, id LocationId) (resp ListByHomeRegionOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "topology.TopologyClient", "ListByHomeRegion", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "topology.TopologyClient", "ListByHomeRegion", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByHomeRegion(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "topology.TopologyClient", "ListByHomeRegion", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c TopologyClient) preparerForListByHomeRegion(ctx context.Context, id LocationId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/topologies", id.ID())),
...
func (c TopologyClient) preparerForListByHomeRegionWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c TopologyClient) responderForListByHomeRegion(resp *http.Response) (result ListByHomeRegionOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByHomeRegionOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "topology.TopologyClient", "ListByHomeRegion", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByHomeRegion(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "topology.TopologyClient", "ListByHomeRegion", result.HttpResponse, "Failure responding to request")
				return
			}
