...
func TestParseMetadataSchemaID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseMetadataSchemaID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.MetadataSchemaName != v.Expected.MetadataSchemaName {
			t.Fatalf("Expected %q but got %q for MetadataSchemaName", v.Expected.MetadataSchemaName, actual.MetadataSchemaName)
		}

	}
}

func TestParseMetadataSchemaIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MetadataSchemaId
	...
	for _, v := range testData {
		...
		actual, err := ParseMetadataSchemaIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMetadataSchemaId(t *testing.T) {
	segments := MetadataSchemaId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MetadataSchemaId has no segments")
	}
