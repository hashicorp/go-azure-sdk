...
func TestParseSoftwareUpdateConfigurationRunID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseSoftwareUpdateConfigurationRunID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.SoftwareUpdateConfigurationRunId != v.Expected.SoftwareUpdateConfigurationRunId {
			t.Fatalf("Expected %q but got %q for SoftwareUpdateConfigurationRunId", v.Expected.SoftwareUpdateConfigurationRunId, actual.SoftwareUpdateConfigurationRunId)
		}

	}
}

func TestParseSoftwareUpdateConfigurationRunIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SoftwareUpdateConfigurationRunId
	...
	for _, v := range testData {
		...
		actual, err := ParseSoftwareUpdateConfigurationRunIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSoftwareUpdateConfigurationRunId(t *testing.T) {
	segments := SoftwareUpdateConfigurationRunId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SoftwareUpdateConfigurationRunId has no segments")
	}
