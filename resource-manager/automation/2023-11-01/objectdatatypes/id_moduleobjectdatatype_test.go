...
func TestParseModuleObjectDataTypeID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Automation/automationAccounts/automationAccountValue/modules/moduleValue/objectDataTypes",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseModuleObjectDataTypeID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseModuleObjectDataTypeIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ModuleObjectDataTypeId
	...
	for _, v := range testData {
		...
		actual, err := ParseModuleObjectDataTypeIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForModuleObjectDataTypeId(t *testing.T) {
	segments := ModuleObjectDataTypeId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ModuleObjectDataTypeId has no segments")
	}
