...
func TestParseApplicationDefinitionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Solutions/applicationDefinitions",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseApplicationDefinitionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ApplicationDefinitionName != v.Expected.ApplicationDefinitionName {
			t.Fatalf("Expected %q but got %q for ApplicationDefinitionName", v.Expected.ApplicationDefinitionName, actual.ApplicationDefinitionName)
		}

	}
}

func TestParseApplicationDefinitionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ApplicationDefinitionId
	...
	for _, v := range testData {
		...
		actual, err := ParseApplicationDefinitionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForApplicationDefinitionId(t *testing.T) {
	segments := ApplicationDefinitionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ApplicationDefinitionId has no segments")
	}
