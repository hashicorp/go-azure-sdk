...
func TestParseJitRequestID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Solutions/jitRequests",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseJitRequestID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.JitRequestName != v.Expected.JitRequestName {
			t.Fatalf("Expected %q but got %q for JitRequestName", v.Expected.JitRequestName, actual.JitRequestName)
		}

	}
}

func TestParseJitRequestIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *JitRequestId
	...
	for _, v := range testData {
		...
		actual, err := ParseJitRequestIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForJitRequestId(t *testing.T) {
	segments := JitRequestId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("JitRequestId has no segments")
	}
