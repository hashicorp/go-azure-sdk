...
func TestParseScopedQuotaID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/some-resource-group/providers/Microsoft.Quota/quotas",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedQuotaID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseScopedQuotaIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ScopedQuotaId
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedQuotaIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForScopedQuotaId(t *testing.T) {
	segments := ScopedQuotaId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ScopedQuotaId has no segments")
	}
