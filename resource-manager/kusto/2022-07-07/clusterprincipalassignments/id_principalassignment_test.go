...
func TestParsePrincipalAssignmentID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParsePrincipalAssignmentID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParsePrincipalAssignmentIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *PrincipalAssignmentId
	...
	for _, v := range testData {
		...
		actual, err := ParsePrincipalAssignmentIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForPrincipalAssignmentId(t *testing.T) {
	segments := PrincipalAssignmentId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("PrincipalAssignmentId has no segments")
	}
