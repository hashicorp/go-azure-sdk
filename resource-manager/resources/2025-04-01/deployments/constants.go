...
func PossibleValuesForValidationLevel() []string {
	return []string{
		string(ValidationLevelProvider),
		string(ValidationLevelProviderNoRbac),
		string(ValidationLevelTemplate),
	}
}

func (s *ValidationLevel) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseValidationLevel(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseValidationLevel(input string) (*ValidationLevel, error) {
	vals := map[string]ValidationLevel{
		"provider":       ValidationLevelProvider,
		"providernorbac": ValidationLevelProviderNoRbac,
