...
func TestParseTemplateSpecID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseTemplateSpecID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseTemplateSpecIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *TemplateSpecId
	...
	for _, v := range testData {
		...
		actual, err := ParseTemplateSpecIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForTemplateSpecId(t *testing.T) {
	segments := TemplateSpecId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("TemplateSpecId has no segments")
	}
