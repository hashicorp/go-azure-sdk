...
func (c ResourcesClient) CheckExistence(ctx context.Context, id ResourceId) (result CheckExistenceOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "resources.ResourcesClient", "CheckExistence", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCheckExistence(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "resources.ResourcesClient", "CheckExistence", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c ResourcesClient) responderForCheckExistence(resp *http.Response) (result CheckExistenceOperationResponse, err error) {
