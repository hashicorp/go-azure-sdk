...
func TestParseDeploymentStackID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/providers/Microsoft.Resources/deploymentStacks",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseDeploymentStackID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseDeploymentStackIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DeploymentStackId
	...
	for _, v := range testData {
		...
		actual, err := ParseDeploymentStackIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDeploymentStackId(t *testing.T) {
	segments := DeploymentStackId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DeploymentStackId has no segments")
	}
