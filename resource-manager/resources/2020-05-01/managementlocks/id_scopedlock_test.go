...
func TestParseScopedLockID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedLockID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseScopedLockIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ScopedLockId
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedLockIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForScopedLockId(t *testing.T) {
	segments := ScopedLockId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ScopedLockId has no segments")
	}
