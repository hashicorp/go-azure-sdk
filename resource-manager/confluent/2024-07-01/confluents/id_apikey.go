...
func NewApiKeyID(subscriptionId string, resourceGroupName string, organizationName string, apiKeyId string) ApiKeyId {
	return ApiKeyId{
...
func (id *ApiKeyId) FromParseResult(input resourceids.ParseResult) error {
	...
	if id.ApiKeyId, ok = input.Parsed["apiKeyId"]; !ok {
		return resourceids.NewSegmentNotSpecifiedError(id, "apiKeyId", input)
	}
...
func (id ApiKeyId) ID() string {
	fmtString := "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Confluent/organizations/%s/apiKeys/%s"
	return fmt.Sprintf(fmtString, id.SubscriptionId, id.ResourceGroupName, id.OrganizationName, id.ApiKeyId)
}
...
func (id ApiKeyId) Segments() []resourceids.Segment {
	return []resourceids.Segment{
		...
		resourceids.UserSpecifiedSegment("apiKeyId", "apiKeyId"),
	}
}
...
func (id ApiKeyId) String() string {
	components := []string{
		...
		fmt.Sprintf("Api Key: %q", id.ApiKeyId),
	}
