...
func TestParseProximityPlacementGroupID(t *testing.T) {
	testData := []struct {
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Compute/proximityPlacementGroups",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseProximityPlacementGroupID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ProximityPlacementGroupName != v.Expected.ProximityPlacementGroupName {
			t.Fatalf("Expected %q but got %q for ProximityPlacementGroupName", v.Expected.ProximityPlacementGroupName, actual.ProximityPlacementGroupName)
		}

	}
}

func TestParseProximityPlacementGroupIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ProximityPlacementGroupId
	...
	for _, v := range testData {
		...
		actual, err := ParseProximityPlacementGroupIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForProximityPlacementGroupId(t *testing.T) {
	segments := ProximityPlacementGroupId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ProximityPlacementGroupId has no segments")
	}
