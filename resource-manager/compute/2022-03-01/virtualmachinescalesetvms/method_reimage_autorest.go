...
func (c VirtualMachineScaleSetVMsClient) Reimage(ctx context.Context, id VirtualMachineId, input VirtualMachineReimageParameters) (result ReimageOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "virtualmachinescalesetvms.VirtualMachineScaleSetVMsClient", "Reimage", result.HttpResponse, "Failure sending request")
		return
	}
...
func (c VirtualMachineScaleSetVMsClient) senderForReimage(ctx context.Context, req *http.Request) (future ReimageOperationResponse, err error) {
