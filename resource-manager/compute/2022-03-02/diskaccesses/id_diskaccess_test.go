...
func TestParseDiskAccessID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseDiskAccessID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseDiskAccessIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DiskAccessId
	...
	for _, v := range testData {
		...
		actual, err := ParseDiskAccessIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDiskAccessId(t *testing.T) {
	segments := DiskAccessId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DiskAccessId has no segments")
	}
