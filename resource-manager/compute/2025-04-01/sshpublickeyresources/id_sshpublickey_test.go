...
func TestParseSshPublicKeyID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Compute/sshPublicKeys",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseSshPublicKeyID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.SshPublicKeyName != v.Expected.SshPublicKeyName {
			t.Fatalf("Expected %q but got %q for SshPublicKeyName", v.Expected.SshPublicKeyName, actual.SshPublicKeyName)
		}

	}
}

func TestParseSshPublicKeyIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SshPublicKeyId
	...
	for _, v := range testData {
		...
		actual, err := ParseSshPublicKeyIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSshPublicKeyId(t *testing.T) {
	segments := SshPublicKeyId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SshPublicKeyId has no segments")
	}
