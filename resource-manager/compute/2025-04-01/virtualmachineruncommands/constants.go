...
const (
	...
	ScriptShellTypesPowershellSeven ScriptShellTypes = "Powershell7"
)

func PossibleValuesForScriptShellTypes() []string {
	return []string{
		string(ScriptShellTypesDefault),
		string(ScriptShellTypesPowershellSeven),
	}
}

func (s *ScriptShellTypes) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseScriptShellTypes(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseScriptShellTypes(input string) (*ScriptShellTypes, error) {
	vals := map[string]ScriptShellTypes{
		"default":     ScriptShellTypesDefault,
		"powershell7": ScriptShellTypesPowershellSeven,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
