...
func (c VirtualMachineScaleSetsClient) PerformMaintenance(ctx context.Context, id VirtualMachineScaleSetId, input VirtualMachineScaleSetVMInstanceIDs) (result PerformMaintenanceOperationResponse, err error) {
	req, err := c.preparerForPerformMaintenance(ctx, id, input)
	if err != nil {
		err = autorest.NewErrorWithError(err, "virtualmachinescalesets.VirtualMachineScaleSetsClient", "PerformMaintenance", nil, "Failure preparing request")
		return
	}

	result, err = c.senderForPerformMaintenance(ctx, req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "virtualmachinescalesets.VirtualMachineScaleSetsClient", "PerformMaintenance", result.HttpResponse, "Failure sending request")
		return
	}
...
func (c VirtualMachineScaleSetsClient) preparerForPerformMaintenance(ctx context.Context, id VirtualMachineScaleSetId, input VirtualMachineScaleSetVMInstanceIDs) (*http.Request, error) {
...
func (c VirtualMachineScaleSetsClient) senderForPerformMaintenance(ctx context.Context, req *http.Request) (future PerformMaintenanceOperationResponse, err error) {
