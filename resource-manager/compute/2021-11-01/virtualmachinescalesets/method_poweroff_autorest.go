...
func (c VirtualMachineScaleSetsClient) PowerOff(ctx context.Context, id VirtualMachineScaleSetId, input VirtualMachineScaleSetVMInstanceIDs, options PowerOffOperationOptions) (result PowerOffOperationResponse, err error) {
	req, err := c.preparerForPowerOff(ctx, id, input, options)
	if err != nil {
		err = autorest.NewErrorWithError(err, "virtualmachinescalesets.VirtualMachineScaleSetsClient", "PowerOff", nil, "Failure preparing request")
		return
	}

	result, err = c.senderForPowerOff(ctx, req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "virtualmachinescalesets.VirtualMachineScaleSetsClient", "PowerOff", result.HttpResponse, "Failure sending request")
		return
	}
...
func (c VirtualMachineScaleSetsClient) preparerForPowerOff(ctx context.Context, id VirtualMachineScaleSetId, input VirtualMachineScaleSetVMInstanceIDs, options PowerOffOperationOptions) (*http.Request, error) {
...
func (c VirtualMachineScaleSetsClient) senderForPowerOff(ctx context.Context, req *http.Request) (future PowerOffOperationResponse, err error) {
