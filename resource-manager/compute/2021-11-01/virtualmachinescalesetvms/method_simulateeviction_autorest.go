...
func (c VirtualMachineScaleSetVMsClient) SimulateEviction(ctx context.Context, id VirtualMachineScaleSetVirtualMachineId) (result SimulateEvictionOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "virtualmachinescalesetvms.VirtualMachineScaleSetVMsClient", "SimulateEviction", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForSimulateEviction(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "virtualmachinescalesetvms.VirtualMachineScaleSetVMsClient", "SimulateEviction", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c VirtualMachineScaleSetVMsClient) responderForSimulateEviction(resp *http.Response) (result SimulateEvictionOperationResponse, err error) {
