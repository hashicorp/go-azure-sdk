...
func TestParseGuardrailsVersionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseGuardrailsVersionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.GuardrailsVersionName != v.Expected.GuardrailsVersionName {
			t.Fatalf("Expected %q but got %q for GuardrailsVersionName", v.Expected.GuardrailsVersionName, actual.GuardrailsVersionName)
		}

	}
}

func TestParseGuardrailsVersionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GuardrailsVersionId
	...
	for _, v := range testData {
		...
		actual, err := ParseGuardrailsVersionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGuardrailsVersionId(t *testing.T) {
	segments := GuardrailsVersionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GuardrailsVersionId has no segments")
	}
