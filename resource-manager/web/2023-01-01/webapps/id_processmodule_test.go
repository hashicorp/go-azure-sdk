...
func TestParseProcessModuleID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseProcessModuleID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseProcessModuleIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ProcessModuleId
	...
	for _, v := range testData {
		...
		actual, err := ParseProcessModuleIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForProcessModuleId(t *testing.T) {
	segments := ProcessModuleId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ProcessModuleId has no segments")
	}
