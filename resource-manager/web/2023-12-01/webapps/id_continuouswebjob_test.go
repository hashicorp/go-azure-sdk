...
func TestParseContinuousWebJobID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseContinuousWebJobID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseContinuousWebJobIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ContinuousWebJobId
	...
	for _, v := range testData {
		...
		actual, err := ParseContinuousWebJobIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForContinuousWebJobId(t *testing.T) {
	segments := ContinuousWebJobId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ContinuousWebJobId has no segments")
	}
