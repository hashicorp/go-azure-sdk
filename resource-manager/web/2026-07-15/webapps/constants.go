...
const (
	PlatformReleaseChannelExtended PlatformReleaseChannel = "Extended"
	PlatformReleaseChannelLatest   PlatformReleaseChannel = "Latest"
	PlatformReleaseChannelStandard PlatformReleaseChannel = "Standard"
)

func PossibleValuesForPlatformReleaseChannel() []string {
	return []string{
		string(PlatformReleaseChannelExtended),
		string(PlatformReleaseChannelLatest),
		string(PlatformReleaseChannelStandard),
	}
}

func (s *PlatformReleaseChannel) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parsePlatformReleaseChannel(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parsePlatformReleaseChannel(input string) (*PlatformReleaseChannel, error) {
	vals := map[string]PlatformReleaseChannel{
		"extended": PlatformReleaseChannelExtended,
		"latest":   PlatformReleaseChannelLatest,
		"standard": PlatformReleaseChannelStandard,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := PlatformReleaseChannel(input)
	return &out, nil
}
