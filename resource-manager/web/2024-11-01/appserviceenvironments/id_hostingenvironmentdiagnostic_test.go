...
func TestParseHostingEnvironmentDiagnosticID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseHostingEnvironmentDiagnosticID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseHostingEnvironmentDiagnosticIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *HostingEnvironmentDiagnosticId
	...
	for _, v := range testData {
		...
		actual, err := ParseHostingEnvironmentDiagnosticIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForHostingEnvironmentDiagnosticId(t *testing.T) {
	segments := HostingEnvironmentDiagnosticId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("HostingEnvironmentDiagnosticId has no segments")
	}
