...
func TestParseNetworkFeatureID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseNetworkFeatureID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseNetworkFeatureIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *NetworkFeatureId
	...
	for _, v := range testData {
		...
		actual, err := ParseNetworkFeatureIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForNetworkFeatureId(t *testing.T) {
	segments := NetworkFeatureId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("NetworkFeatureId has no segments")
	}
