...
func NewClientWithBaseURI(api environments.Api, configureFunc func(c *resourcemanager.Client)) (*Client, error) {
	accessPoliciesClient, err := accesspolicies.NewAccessPoliciesClientWithBaseURI(api)
	if err != nil {
	...
	eventSourcesClient, err := eventsources.NewEventSourcesClientWithBaseURI(api)
	if err != nil {
		return nil, fmt.Errorf("building EventSources client: %+v", err)
	}
	configureFunc(eventSourcesClient.Client)

	referenceDataSetsClient, err := referencedatasets.NewReferenceDataSetsClientWithBaseURI(api)
	if err != nil {
