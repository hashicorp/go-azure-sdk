...
func TestParseEventSourceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseEventSourceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.EventSourceName != v.Expected.EventSourceName {
			t.Fatalf("Expected %q but got %q for EventSourceName", v.Expected.EventSourceName, actual.EventSourceName)
		}

	}
}

func TestParseEventSourceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *EventSourceId
	...
	for _, v := range testData {
		...
		actual, err := ParseEventSourceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForEventSourceId(t *testing.T) {
	segments := EventSourceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("EventSourceId has no segments")
	}
