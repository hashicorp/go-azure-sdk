...
func TestParseReferenceDataSetID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.TimeSeriesInsights",
			Error: true,
		},
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.TimeSeriesInsights/environments",
			Error: true,
		},
		{
			// Incomplete URI
	...
	for _, v := range testData {
		...
		actual, err := ParseReferenceDataSetID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ReferenceDataSetName != v.Expected.ReferenceDataSetName {
			t.Fatalf("Expected %q but got %q for ReferenceDataSetName", v.Expected.ReferenceDataSetName, actual.ReferenceDataSetName)
		}

	}
}

func TestParseReferenceDataSetIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ReferenceDataSetId
	...
	for _, v := range testData {
		...
		actual, err := ParseReferenceDataSetIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForReferenceDataSetId(t *testing.T) {
	segments := ReferenceDataSetId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ReferenceDataSetId has no segments")
	}
