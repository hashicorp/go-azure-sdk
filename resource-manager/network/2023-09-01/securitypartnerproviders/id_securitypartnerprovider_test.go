...
func TestParseSecurityPartnerProviderID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/securityPartnerProviders",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseSecurityPartnerProviderID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.SecurityPartnerProviderName != v.Expected.SecurityPartnerProviderName {
			t.Fatalf("Expected %q but got %q for SecurityPartnerProviderName", v.Expected.SecurityPartnerProviderName, actual.SecurityPartnerProviderName)
		}

	}
}

func TestParseSecurityPartnerProviderIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SecurityPartnerProviderId
	...
	for _, v := range testData {
		...
		actual, err := ParseSecurityPartnerProviderIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSecurityPartnerProviderId(t *testing.T) {
	segments := SecurityPartnerProviderId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SecurityPartnerProviderId has no segments")
	}
