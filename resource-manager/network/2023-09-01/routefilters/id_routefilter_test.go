...
func TestParseRouteFilterID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
...
func TestParseRouteFilterIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *RouteFilterId
	...
	for _, v := range testData {
		...
		actual, err := ParseRouteFilterIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForRouteFilterId(t *testing.T) {
	segments := RouteFilterId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("RouteFilterId has no segments")
	}
