...
func TestParseDdosProtectionPlanID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/ddosProtectionPlans",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseDdosProtectionPlanID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.DdosProtectionPlanName != v.Expected.DdosProtectionPlanName {
			t.Fatalf("Expected %q but got %q for DdosProtectionPlanName", v.Expected.DdosProtectionPlanName, actual.DdosProtectionPlanName)
		}

	}
}

func TestParseDdosProtectionPlanIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DdosProtectionPlanId
	...
	for _, v := range testData {
		...
		actual, err := ParseDdosProtectionPlanIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDdosProtectionPlanId(t *testing.T) {
	segments := DdosProtectionPlanId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DdosProtectionPlanId has no segments")
	}
