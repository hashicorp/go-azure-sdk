...
func TestParseConnectionMonitorID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseConnectionMonitorID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ConnectionMonitorName != v.Expected.ConnectionMonitorName {
			t.Fatalf("Expected %q but got %q for ConnectionMonitorName", v.Expected.ConnectionMonitorName, actual.ConnectionMonitorName)
		}

	}
}

func TestParseConnectionMonitorIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ConnectionMonitorId
	...
	for _, v := range testData {
		...
		actual, err := ParseConnectionMonitorIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForConnectionMonitorId(t *testing.T) {
	segments := ConnectionMonitorId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ConnectionMonitorId has no segments")
	}
