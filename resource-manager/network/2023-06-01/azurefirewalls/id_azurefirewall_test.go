...
func TestParseAzureFirewallID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/azureFirewalls",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseAzureFirewallID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseAzureFirewallIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *AzureFirewallId
	...
	for _, v := range testData {
		...
		actual, err := ParseAzureFirewallIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForAzureFirewallId(t *testing.T) {
	segments := AzureFirewallId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("AzureFirewallId has no segments")
	}
