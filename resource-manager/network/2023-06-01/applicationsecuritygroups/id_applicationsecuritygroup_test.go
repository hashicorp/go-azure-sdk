...
func TestParseApplicationSecurityGroupID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/applicationSecurityGroups",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseApplicationSecurityGroupID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.ApplicationSecurityGroupName != v.Expected.ApplicationSecurityGroupName {
			t.Fatalf("Expected %q but got %q for ApplicationSecurityGroupName", v.Expected.ApplicationSecurityGroupName, actual.ApplicationSecurityGroupName)
		}

	}
}

func TestParseApplicationSecurityGroupIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ApplicationSecurityGroupId
	...
	for _, v := range testData {
		...
		actual, err := ParseApplicationSecurityGroupIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForApplicationSecurityGroupId(t *testing.T) {
	segments := ApplicationSecurityGroupId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ApplicationSecurityGroupId has no segments")
	}
