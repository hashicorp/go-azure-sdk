...
func TestParseDscpConfigurationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/dscpConfigurations",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseDscpConfigurationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.DscpConfigurationName != v.Expected.DscpConfigurationName {
			t.Fatalf("Expected %q but got %q for DscpConfigurationName", v.Expected.DscpConfigurationName, actual.DscpConfigurationName)
		}

	}
}

func TestParseDscpConfigurationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DscpConfigurationId
	...
	for _, v := range testData {
		...
		actual, err := ParseDscpConfigurationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDscpConfigurationId(t *testing.T) {
	segments := DscpConfigurationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DscpConfigurationId has no segments")
	}
