...
func ParseVpnSiteLinkID(input string) (*VpnSiteLinkId, error) {
	...
	if id.VpnSiteName, ok = parsed.Parsed["vpnSiteName"]; !ok {
		return nil, fmt.Errorf("the segment 'vpnSiteName' was not found in the resource id %q", input)
	}

	if id.VpnSiteLinkName, ok = parsed.Parsed["vpnSiteLinkName"]; !ok {
		return nil, fmt.Errorf("the segment 'vpnSiteLinkName' was not found in the resource id %q", input)
	}
