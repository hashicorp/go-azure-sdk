...
func TestParseSecurityUserConfigurationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseSecurityUserConfigurationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseSecurityUserConfigurationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SecurityUserConfigurationId
	...
	for _, v := range testData {
		...
		actual, err := ParseSecurityUserConfigurationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSecurityUserConfigurationId(t *testing.T) {
	segments := SecurityUserConfigurationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SecurityUserConfigurationId has no segments")
	}
