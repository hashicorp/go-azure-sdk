...
func TestParseDdosCustomPolicyID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/ddosCustomPolicies",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseDdosCustomPolicyID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.DdosCustomPolicyName != v.Expected.DdosCustomPolicyName {
			t.Fatalf("Expected %q but got %q for DdosCustomPolicyName", v.Expected.DdosCustomPolicyName, actual.DdosCustomPolicyName)
		}

	}
}

func TestParseDdosCustomPolicyIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DdosCustomPolicyId
	...
	for _, v := range testData {
		...
		actual, err := ParseDdosCustomPolicyIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDdosCustomPolicyId(t *testing.T) {
	segments := DdosCustomPolicyId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DdosCustomPolicyId has no segments")
	}
