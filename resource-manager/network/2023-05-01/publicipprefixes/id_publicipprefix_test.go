...
func TestParsePublicIPPrefixID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/publicIPPrefixes",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParsePublicIPPrefixID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.PublicIPPrefixName != v.Expected.PublicIPPrefixName {
			t.Fatalf("Expected %q but got %q for PublicIPPrefixName", v.Expected.PublicIPPrefixName, actual.PublicIPPrefixName)
		}

	}
}

func TestParsePublicIPPrefixIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *PublicIPPrefixId
	...
	for _, v := range testData {
		...
		actual, err := ParsePublicIPPrefixIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForPublicIPPrefixId(t *testing.T) {
	segments := PublicIPPrefixId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("PublicIPPrefixId has no segments")
	}
