...
func TestParseRuleCollectionGroupID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseRuleCollectionGroupID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseRuleCollectionGroupIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *RuleCollectionGroupId
	...
	for _, v := range testData {
		...
		actual, err := ParseRuleCollectionGroupIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForRuleCollectionGroupId(t *testing.T) {
	segments := RuleCollectionGroupId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("RuleCollectionGroupId has no segments")
	}
