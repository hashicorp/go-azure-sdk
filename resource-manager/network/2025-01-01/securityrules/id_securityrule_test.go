...
func TestParseSecurityRuleID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseSecurityRuleID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.SecurityRuleName != v.Expected.SecurityRuleName {
			t.Fatalf("Expected %q but got %q for SecurityRuleName", v.Expected.SecurityRuleName, actual.SecurityRuleName)
		}

	}
}

func TestParseSecurityRuleIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SecurityRuleId
	...
	for _, v := range testData {
		...
		actual, err := ParseSecurityRuleIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSecurityRuleId(t *testing.T) {
	segments := SecurityRuleId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SecurityRuleId has no segments")
	}
