...
func TestParseLocalNetworkGatewayID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/localNetworkGateways",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseLocalNetworkGatewayID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.LocalNetworkGatewayName != v.Expected.LocalNetworkGatewayName {
			t.Fatalf("Expected %q but got %q for LocalNetworkGatewayName", v.Expected.LocalNetworkGatewayName, actual.LocalNetworkGatewayName)
		}

	}
}

func TestParseLocalNetworkGatewayIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *LocalNetworkGatewayId
	...
	for _, v := range testData {
		...
		actual, err := ParseLocalNetworkGatewayIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForLocalNetworkGatewayId(t *testing.T) {
	segments := LocalNetworkGatewayId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("LocalNetworkGatewayId has no segments")
	}
