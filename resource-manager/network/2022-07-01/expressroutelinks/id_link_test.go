...
func TestParseLinkID(t *testing.T) {
	...
	for _, v := range testData {
		...
		actual, err := ParseLinkID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseLinkIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *LinkId
	...
	for _, v := range testData {
		...
		actual, err := ParseLinkIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForLinkId(t *testing.T) {
	segments := LinkId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("LinkId has no segments")
	}
