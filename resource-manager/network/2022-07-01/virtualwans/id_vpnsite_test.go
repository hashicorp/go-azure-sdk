...
func TestParseVpnSiteID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VpnSiteId
	...
	for _, v := range testData {
		...
		actual, err := ParseVpnSiteID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseVpnSiteIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VpnSiteId
	...
	for _, v := range testData {
		...
		actual, err := ParseVpnSiteIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForVpnSiteId(t *testing.T) {
	segments := VpnSiteId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("VpnSiteId has no segments")
	}
