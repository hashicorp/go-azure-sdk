...
func TestParseNetworkProfileID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/networkProfiles",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseNetworkProfileID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.NetworkProfileName != v.Expected.NetworkProfileName {
			t.Fatalf("Expected %q but got %q for NetworkProfileName", v.Expected.NetworkProfileName, actual.NetworkProfileName)
		}

	}
}

func TestParseNetworkProfileIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *NetworkProfileId
	...
	for _, v := range testData {
		...
		actual, err := ParseNetworkProfileIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForNetworkProfileId(t *testing.T) {
	segments := NetworkProfileId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("NetworkProfileId has no segments")
	}
