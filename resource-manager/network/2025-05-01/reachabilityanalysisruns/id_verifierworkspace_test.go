...
func TestNewVerifierWorkspaceID(t *testing.T) {
	id := NewVerifierWorkspaceID("12345678-1234-9876-4563-123456789012", "example-resource-group", "networkManagerName", "verifierWorkspaceName")
...
func TestFormatVerifierWorkspaceID(t *testing.T) {
	actual := NewVerifierWorkspaceID("12345678-1234-9876-4563-123456789012", "example-resource-group", "networkManagerName", "verifierWorkspaceName").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Network/networkManagers/networkManagerName/verifierWorkspaces/verifierWorkspaceName"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseVerifierWorkspaceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VerifierWorkspaceId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseVerifierWorkspaceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseVerifierWorkspaceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VerifierWorkspaceId
	...
	for _, v := range testData {
		...
		actual, err := ParseVerifierWorkspaceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForVerifierWorkspaceId(t *testing.T) {
	segments := VerifierWorkspaceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("VerifierWorkspaceId has no segments")
	}
