...
func TestParseVirtualRouterID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseVirtualRouterID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseVirtualRouterIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VirtualRouterId
	...
	for _, v := range testData {
		...
		actual, err := ParseVirtualRouterIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForVirtualRouterId(t *testing.T) {
	segments := VirtualRouterId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("VirtualRouterId has no segments")
	}
