...
var _ json.Unmarshaler = &ManagedNetworkSettingsEx{}

func (s *ManagedNetworkSettingsEx) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		ChangeableIsolationModes *[]IsolationMode                 `json:"changeableIsolationModes,omitempty"`
		FirewallPublicIPAddress  *string                          `json:"firewallPublicIpAddress,omitempty"`
		FirewallSku              *FirewallSku                     `json:"firewallSku,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling ManagedNetworkSettingsEx into map[string]json.RawMessage: %+v", err)
	}
	...
	if v, ok := temp["outboundRules"]; ok {
		...
		for key, val := range dictionaryTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling key %q field 'OutboundRules' for 'ManagedNetworkSettingsEx': %+v", key, err)
			}
			output[key] = impl
		}
