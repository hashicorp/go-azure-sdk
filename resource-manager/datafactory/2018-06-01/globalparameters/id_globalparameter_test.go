...
func TestParseGlobalParameterID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseGlobalParameterID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.GlobalParameterName != v.Expected.GlobalParameterName {
			t.Fatalf("Expected %q but got %q for GlobalParameterName", v.Expected.GlobalParameterName, actual.GlobalParameterName)
		}

	}
}

func TestParseGlobalParameterIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GlobalParameterId
	...
	for _, v := range testData {
		...
		actual, err := ParseGlobalParameterIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGlobalParameterId(t *testing.T) {
	segments := GlobalParameterId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GlobalParameterId has no segments")
	}
