...
// ListByFactory ...
func (c DataFlowsClient) ListByFactory(ctx context.Context, id FactoryId) (resp ListByFactoryOperationResponse, err error) {
	req, err := c.preparerForListByFactory(ctx, id)
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataflows.DataFlowsClient", "ListByFactory", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataflows.DataFlowsClient", "ListByFactory", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByFactory(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "dataflows.DataFlowsClient", "ListByFactory", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c DataFlowsClient) preparerForListByFactory(ctx context.Context, id FactoryId) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/dataflows", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByFactoryWithNextLink prepares the ListByFactory request with the given nextLink token.
func (c DataFlowsClient) preparerForListByFactoryWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c DataFlowsClient) responderForListByFactory(resp *http.Response) (result ListByFactoryOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByFactoryOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "dataflows.DataFlowsClient", "ListByFactory", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByFactory(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "dataflows.DataFlowsClient", "ListByFactory", result.HttpResponse, "Failure responding to request")
				return
			}
...
func (c DataFlowsClient) ListByFactoryCompleteMatchingPredicate(ctx context.Context, id FactoryId, predicate DataFlowResourceOperationPredicate) (resp ListByFactoryCompleteResult, err error) {
