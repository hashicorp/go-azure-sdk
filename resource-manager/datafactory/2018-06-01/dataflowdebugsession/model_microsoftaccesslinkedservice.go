...
var _ json.Unmarshaler = &MicrosoftAccessLinkedService{}
...
func (s *MicrosoftAccessLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MicrosoftAccessLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'MicrosoftAccessLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
