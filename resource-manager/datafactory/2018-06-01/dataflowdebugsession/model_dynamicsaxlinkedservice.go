...
var _ json.Unmarshaler = &DynamicsAXLinkedService{}
...
func (s *DynamicsAXLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling DynamicsAXLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'DynamicsAXLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
