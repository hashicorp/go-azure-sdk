...
var _ json.Unmarshaler = &AzureDatabricksLinkedService{}
...
func (s *AzureDatabricksLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling AzureDatabricksLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'AzureDatabricksLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
