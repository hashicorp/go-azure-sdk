...
var _ json.Unmarshaler = &GoogleCloudStorageLinkedService{}
...
func (s *GoogleCloudStorageLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling GoogleCloudStorageLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'GoogleCloudStorageLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
