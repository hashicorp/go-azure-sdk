...
var _ json.Unmarshaler = &PostgreSqlV2LinkedService{}
...
func (s *PostgreSqlV2LinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling PostgreSqlV2LinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'PostgreSqlV2LinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
