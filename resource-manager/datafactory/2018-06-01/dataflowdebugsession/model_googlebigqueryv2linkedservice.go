...
var _ json.Unmarshaler = &GoogleBigQueryV2LinkedService{}
...
func (s *GoogleBigQueryV2LinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling GoogleBigQueryV2LinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'GoogleBigQueryV2LinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
