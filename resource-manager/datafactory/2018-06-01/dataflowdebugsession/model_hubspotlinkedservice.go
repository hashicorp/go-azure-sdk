...
var _ json.Unmarshaler = &HubspotLinkedService{}
...
func (s *HubspotLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling HubspotLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'HubspotLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
