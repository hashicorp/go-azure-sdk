...
var _ json.Unmarshaler = &AmazonRedshiftLinkedService{}
...
func (s *AmazonRedshiftLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling AmazonRedshiftLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'AmazonRedshiftLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
