...
var _ json.Unmarshaler = &SalesforceV2LinkedService{}
...
func (s *SalesforceV2LinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling SalesforceV2LinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'SalesforceV2LinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
