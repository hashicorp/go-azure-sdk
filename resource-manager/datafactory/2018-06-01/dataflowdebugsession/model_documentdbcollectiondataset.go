...
var _ json.Unmarshaler = &DocumentDbCollectionDataset{}
...
func (s *DocumentDbCollectionDataset) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling DocumentDbCollectionDataset into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["linkedServiceName"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'LinkedServiceName' for 'DocumentDbCollectionDataset': %+v", err)
		}
