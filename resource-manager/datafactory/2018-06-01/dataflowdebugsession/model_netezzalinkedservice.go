...
var _ json.Unmarshaler = &NetezzaLinkedService{}
...
func (s *NetezzaLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling NetezzaLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'NetezzaLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
