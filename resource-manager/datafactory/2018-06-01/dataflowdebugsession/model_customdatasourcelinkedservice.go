...
var _ json.Unmarshaler = &CustomDataSourceLinkedService{}
...
func (s *CustomDataSourceLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling CustomDataSourceLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'CustomDataSourceLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
