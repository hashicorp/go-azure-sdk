...
var _ json.Unmarshaler = &SapCloudForCustomerLinkedService{}
...
func (s *SapCloudForCustomerLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling SapCloudForCustomerLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'SapCloudForCustomerLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
