...
var _ json.Unmarshaler = &TeamDeskLinkedService{}
...
func (s *TeamDeskLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling TeamDeskLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'TeamDeskLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
