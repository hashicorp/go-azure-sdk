...
var _ json.Unmarshaler = &SalesforceServiceCloudLinkedService{}
...
func (s *SalesforceServiceCloudLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling SalesforceServiceCloudLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'SalesforceServiceCloudLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
