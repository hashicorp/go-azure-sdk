...
var _ json.Unmarshaler = &MongoDbV2CollectionDataset{}
...
func (s *MongoDbV2CollectionDataset) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling MongoDbV2CollectionDataset into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["linkedServiceName"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'LinkedServiceName' for 'MongoDbV2CollectionDataset': %+v", err)
		}
