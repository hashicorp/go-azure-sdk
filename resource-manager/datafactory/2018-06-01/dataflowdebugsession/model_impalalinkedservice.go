...
var _ json.Unmarshaler = &ImpalaLinkedService{}
...
func (s *ImpalaLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling ImpalaLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'ImpalaLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
