...
var _ json.Unmarshaler = &AzureSqlDWLinkedService{}
...
func (s *AzureSqlDWLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling AzureSqlDWLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'AzureSqlDWLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
