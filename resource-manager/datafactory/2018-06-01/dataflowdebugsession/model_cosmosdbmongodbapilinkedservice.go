...
var _ json.Unmarshaler = &CosmosDbMongoDbApiLinkedService{}
...
func (s *CosmosDbMongoDbApiLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling CosmosDbMongoDbApiLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'CosmosDbMongoDbApiLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
