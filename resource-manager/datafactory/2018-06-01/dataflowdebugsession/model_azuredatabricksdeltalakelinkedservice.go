...
var _ json.Unmarshaler = &AzureDatabricksDeltaLakeLinkedService{}
...
func (s *AzureDatabricksDeltaLakeLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling AzureDatabricksDeltaLakeLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'AzureDatabricksDeltaLakeLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
