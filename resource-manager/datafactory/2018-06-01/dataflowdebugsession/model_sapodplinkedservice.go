...
var _ json.Unmarshaler = &SapOdpLinkedService{}
...
func (s *SapOdpLinkedService) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling SapOdpLinkedService into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'SapOdpLinkedService': %+v", err)
		}
		s.ConnectVia = impl
	}
