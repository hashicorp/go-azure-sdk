...
var _ json.Unmarshaler = &BaseLinkedServiceImpl{}
...
func (s *BaseLinkedServiceImpl) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling BaseLinkedServiceImpl into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["connectVia"]; ok {
		impl, err := UnmarshalReferenceImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ConnectVia' for 'BaseLinkedServiceImpl': %+v", err)
		}
		s.ConnectVia = impl
	}
