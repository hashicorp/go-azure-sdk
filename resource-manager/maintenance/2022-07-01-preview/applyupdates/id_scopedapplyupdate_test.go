...
func TestParseScopedApplyUpdateID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedApplyUpdateID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseScopedApplyUpdateIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ScopedApplyUpdateId
	...
	for _, v := range testData {
		...
		actual, err := ParseScopedApplyUpdateIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForScopedApplyUpdateId(t *testing.T) {
	segments := ScopedApplyUpdateId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ScopedApplyUpdateId has no segments")
	}
