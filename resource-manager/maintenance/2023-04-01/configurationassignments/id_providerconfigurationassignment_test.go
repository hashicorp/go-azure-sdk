...
func TestParseProviderConfigurationAssignmentID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.Maintenance/configurationAssignments",
			Error: true,
		},
		{
			// Valid URI
	...
	for _, v := range testData {
		...
		actual, err := ParseProviderConfigurationAssignmentID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseProviderConfigurationAssignmentIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ProviderConfigurationAssignmentId
	...
	for _, v := range testData {
		...
		actual, err := ParseProviderConfigurationAssignmentIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForProviderConfigurationAssignmentId(t *testing.T) {
	segments := ProviderConfigurationAssignmentId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ProviderConfigurationAssignmentId has no segments")
	}
