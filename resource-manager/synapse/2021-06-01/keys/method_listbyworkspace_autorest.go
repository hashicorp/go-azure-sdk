...
// ListByWorkspace ...
func (c KeysClient) ListByWorkspace(ctx context.Context, id WorkspaceId) (resp ListByWorkspaceOperationResponse, err error) {
	req, err := c.preparerForListByWorkspace(ctx, id)
	if err != nil {
		err = autorest.NewErrorWithError(err, "keys.KeysClient", "ListByWorkspace", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "keys.KeysClient", "ListByWorkspace", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByWorkspace(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "keys.KeysClient", "ListByWorkspace", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c KeysClient) preparerForListByWorkspace(ctx context.Context, id WorkspaceId) (*http.Request, error) {
...
func (c KeysClient) preparerForListByWorkspaceWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c KeysClient) responderForListByWorkspace(resp *http.Response) (result ListByWorkspaceOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByWorkspaceOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "keys.KeysClient", "ListByWorkspace", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByWorkspace(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "keys.KeysClient", "ListByWorkspace", result.HttpResponse, "Failure responding to request")
				return
			}
...
func (c KeysClient) ListByWorkspaceCompleteMatchingPredicate(ctx context.Context, id WorkspaceId, predicate KeyOperationPredicate) (resp ListByWorkspaceCompleteResult, err error) {
	items := make([]Key, 0)
