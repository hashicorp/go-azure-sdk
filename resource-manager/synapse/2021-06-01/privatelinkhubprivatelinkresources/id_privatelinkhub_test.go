...
func TestParsePrivateLinkHubID(t *testing.T) {
	...
	for _, v := range testData {
		...
		actual, err := ParsePrivateLinkHubID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParsePrivateLinkHubIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *PrivateLinkHubId
	...
	for _, v := range testData {
		...
		actual, err := ParsePrivateLinkHubIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForPrivateLinkHubId(t *testing.T) {
	segments := PrivateLinkHubId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("PrivateLinkHubId has no segments")
	}
