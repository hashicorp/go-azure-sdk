...
// ListByServer ...
func (c QueryTextsClient) ListByServer(ctx context.Context, id ServerId, options ListByServerOperationOptions) (resp ListByServerOperationResponse, err error) {
	req, err := c.preparerForListByServer(ctx, id, options)
	if err != nil {
		err = autorest.NewErrorWithError(err, "querytexts.QueryTextsClient", "ListByServer", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "querytexts.QueryTextsClient", "ListByServer", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByServer(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "querytexts.QueryTextsClient", "ListByServer", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c QueryTextsClient) preparerForListByServer(ctx context.Context, id ServerId, options ListByServerOperationOptions) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/queryTexts", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByServerWithNextLink prepares the ListByServer request with the given nextLink token.
func (c QueryTextsClient) preparerForListByServerWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c QueryTextsClient) responderForListByServer(resp *http.Response) (result ListByServerOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByServerOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "querytexts.QueryTextsClient", "ListByServer", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByServer(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "querytexts.QueryTextsClient", "ListByServer", result.HttpResponse, "Failure responding to request")
				return
			}
