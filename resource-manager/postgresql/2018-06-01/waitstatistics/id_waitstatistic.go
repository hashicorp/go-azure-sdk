...
func (id *WaitStatisticId) FromParseResult(input resourceids.ParseResult) error {
	...
	if id.WaitStatisticsId, ok = input.Parsed["waitStatisticsId"]; !ok {
		return resourceids.NewSegmentNotSpecifiedError(id, "waitStatisticsId", input)
	}
...
func (id WaitStatisticId) ID() string {
	fmtString := "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.DBforPostgreSQL/servers/%s/waitStatistics/%s"
