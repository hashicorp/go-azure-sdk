...
func (c VideosClient) VideosListStreamingToken(ctx context.Context, id VideoId) (result VideosListStreamingTokenOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "videos.VideosClient", "VideosListStreamingToken", nil, "Failure preparing request")
		return
	}

	result.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "videos.VideosClient", "VideosListStreamingToken", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForVideosListStreamingToken(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "videos.VideosClient", "VideosListStreamingToken", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c VideosClient) preparerForVideosListStreamingToken(ctx context.Context, id VideoId) (*http.Request, error) {
...
func (c VideosClient) responderForVideosListStreamingToken(resp *http.Response) (result VideosListStreamingTokenOperationResponse, err error) {
