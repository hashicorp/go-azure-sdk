...
func (c VideosClient) AccessPoliciesUpdate(ctx context.Context, id AccessPolicyId, input AccessPolicyEntity) (result AccessPoliciesUpdateOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "videos.VideosClient", "AccessPoliciesUpdate", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForAccessPoliciesUpdate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "videos.VideosClient", "AccessPoliciesUpdate", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c VideosClient) responderForAccessPoliciesUpdate(resp *http.Response) (result AccessPoliciesUpdateOperationResponse, err error) {
