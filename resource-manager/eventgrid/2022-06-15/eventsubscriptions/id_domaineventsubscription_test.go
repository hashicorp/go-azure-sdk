...
func TestParseDomainEventSubscriptionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseDomainEventSubscriptionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseDomainEventSubscriptionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *DomainEventSubscriptionId
	...
	for _, v := range testData {
		...
		actual, err := ParseDomainEventSubscriptionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForDomainEventSubscriptionId(t *testing.T) {
	segments := DomainEventSubscriptionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("DomainEventSubscriptionId has no segments")
	}
