...
func TestParseCentralInstanceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseCentralInstanceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.CentralInstanceName != v.Expected.CentralInstanceName {
			t.Fatalf("Expected %q but got %q for CentralInstanceName", v.Expected.CentralInstanceName, actual.CentralInstanceName)
		}

	}
}

func TestParseCentralInstanceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *CentralInstanceId
	...
	for _, v := range testData {
		...
		actual, err := ParseCentralInstanceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForCentralInstanceId(t *testing.T) {
	segments := CentralInstanceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("CentralInstanceId has no segments")
	}
