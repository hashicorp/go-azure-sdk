...
func (c ApiManagementServiceClient) Restore(ctx context.Context, id ServiceId, input ApiManagementServiceBackupRestoreParameters) (result RestoreOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "apimanagementservice.ApiManagementServiceClient", "Restore", nil, "Failure preparing request")
		return
	}

	result, err = c.senderForRestore(ctx, req)
	if err != nil {
		err = autorest.NewErrorWithError(err, "apimanagementservice.ApiManagementServiceClient", "Restore", result.HttpResponse, "Failure sending request")
		return
	}
...
func (c ApiManagementServiceClient) preparerForRestore(ctx context.Context, id ServiceId, input ApiManagementServiceBackupRestoreParameters) (*http.Request, error) {
...
// senderForRestore sends the Restore request. The method will close the
// http.Response Body if it receives an error.
func (c ApiManagementServiceClient) senderForRestore(ctx context.Context, req *http.Request) (future RestoreOperationResponse, err error) {
