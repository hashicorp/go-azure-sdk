...
func (c ApiIssueAttachmentClient) ListByService(ctx context.Context, id IssueId, options ListByServiceOperationOptions) (resp ListByServiceOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "apiissueattachment.ApiIssueAttachmentClient", "ListByService", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByService(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "apiissueattachment.ApiIssueAttachmentClient", "ListByService", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c ApiIssueAttachmentClient) preparerForListByService(ctx context.Context, id IssueId, options ListByServiceOperationOptions) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/attachments", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByServiceWithNextLink prepares the ListByService request with the given nextLink token.
func (c ApiIssueAttachmentClient) preparerForListByServiceWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c ApiIssueAttachmentClient) responderForListByService(resp *http.Response) (result ListByServiceOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByServiceOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "apiissueattachment.ApiIssueAttachmentClient", "ListByService", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByService(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "apiissueattachment.ApiIssueAttachmentClient", "ListByService", result.HttpResponse, "Failure responding to request")
				return
			}
