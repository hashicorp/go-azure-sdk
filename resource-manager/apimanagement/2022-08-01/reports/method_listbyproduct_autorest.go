...
func (c ReportsClient) ListByProduct(ctx context.Context, id ServiceId, options ListByProductOperationOptions) (resp ListByProductOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByProduct", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByProduct", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByProduct(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByProduct", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c ReportsClient) preparerForListByProduct(ctx context.Context, id ServiceId, options ListByProductOperationOptions) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/reports/byProduct", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByProductWithNextLink prepares the ListByProduct request with the given nextLink token.
func (c ReportsClient) preparerForListByProductWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c ReportsClient) responderForListByProduct(resp *http.Response) (result ListByProductOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByProductOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByProduct", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByProduct(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByProduct", result.HttpResponse, "Failure responding to request")
				return
			}
