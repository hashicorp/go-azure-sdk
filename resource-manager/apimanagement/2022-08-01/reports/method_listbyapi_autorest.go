...
func (c ReportsClient) ListByApi(ctx context.Context, id ServiceId, options ListByApiOperationOptions) (resp ListByApiOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByApi", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByApi", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByApi(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByApi", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c ReportsClient) preparerForListByApi(ctx context.Context, id ServiceId, options ListByApiOperationOptions) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/reports/byApi", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByApiWithNextLink prepares the ListByApi request with the given nextLink token.
func (c ReportsClient) preparerForListByApiWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c ReportsClient) responderForListByApi(resp *http.Response) (result ListByApiOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByApiOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByApi", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByApi(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListByApi", result.HttpResponse, "Failure responding to request")
				return
			}
