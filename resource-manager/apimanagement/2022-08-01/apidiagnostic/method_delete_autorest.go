...
func (c ApiDiagnosticClient) Delete(ctx context.Context, id DiagnosticId, options DeleteOperationOptions) (result DeleteOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "apidiagnostic.ApiDiagnosticClient", "Delete", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForDelete(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "apidiagnostic.ApiDiagnosticClient", "Delete", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c ApiDiagnosticClient) responderForDelete(resp *http.Response) (result DeleteOperationResponse, err error) {
