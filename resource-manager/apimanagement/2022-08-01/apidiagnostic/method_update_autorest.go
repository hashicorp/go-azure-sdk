...
func (c ApiDiagnosticClient) Update(ctx context.Context, id ApiDiagnosticId, input DiagnosticContract, options UpdateOperationOptions) (result UpdateOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "apidiagnostic.ApiDiagnosticClient", "Update", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForUpdate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "apidiagnostic.ApiDiagnosticClient", "Update", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c ApiDiagnosticClient) responderForUpdate(resp *http.Response) (result UpdateOperationResponse, err error) {
