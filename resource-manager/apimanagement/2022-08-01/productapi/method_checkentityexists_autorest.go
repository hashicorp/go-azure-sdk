...
func (c ProductApiClient) CheckEntityExists(ctx context.Context, id ProductApiId) (result CheckEntityExistsOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "productapi.ProductApiClient", "CheckEntityExists", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForCheckEntityExists(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "productapi.ProductApiClient", "CheckEntityExists", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c ProductApiClient) responderForCheckEntityExists(resp *http.Response) (result CheckEntityExistsOperationResponse, err error) {
