...
// ListByProduct ...
func (c ProductApiClient) ListByProduct(ctx context.Context, id ProductId, options ListByProductOperationOptions) (resp ListByProductOperationResponse, err error) {
	req, err := c.preparerForListByProduct(ctx, id, options)
	if err != nil {
		err = autorest.NewErrorWithError(err, "productapi.ProductApiClient", "ListByProduct", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "productapi.ProductApiClient", "ListByProduct", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListByProduct(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "productapi.ProductApiClient", "ListByProduct", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c ProductApiClient) preparerForListByProduct(ctx context.Context, id ProductId, options ListByProductOperationOptions) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/apis", id.ID())),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// preparerForListByProductWithNextLink prepares the ListByProduct request with the given nextLink token.
func (c ProductApiClient) preparerForListByProductWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c ProductApiClient) responderForListByProduct(resp *http.Response) (result ListByProductOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListByProductOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "productapi.ProductApiClient", "ListByProduct", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListByProduct(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "productapi.ProductApiClient", "ListByProduct", result.HttpResponse, "Failure responding to request")
				return
			}
...
func (c ProductApiClient) ListByProductCompleteMatchingPredicate(ctx context.Context, id ProductId, options ListByProductOperationOptions, predicate ApiContractOperationPredicate) (resp ListByProductCompleteResult, err error) {
