...
func (c GatewayApiClient) GetEntityTag(ctx context.Context, id ApiId) (result GetEntityTagOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "gatewayapi.GatewayApiClient", "GetEntityTag", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForGetEntityTag(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "gatewayapi.GatewayApiClient", "GetEntityTag", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c GatewayApiClient) responderForGetEntityTag(resp *http.Response) (result GetEntityTagOperationResponse, err error) {
