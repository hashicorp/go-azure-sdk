...
func (c ReportsClient) ListBySubscription(ctx context.Context, id ServiceId, options ListBySubscriptionOperationOptions) (resp ListBySubscriptionOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListBySubscription", nil, "Failure preparing request")
		return
	}

	resp.HttpResponse, err = c.Client.Send(req, azure.DoRetryWithRegistration(c.Client))
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListBySubscription", resp.HttpResponse, "Failure sending request")
		return
	}

	resp, err = c.responderForListBySubscription(resp.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListBySubscription", resp.HttpResponse, "Failure responding to request")
		return
	}
	return
}
...
func (c ReportsClient) preparerForListBySubscription(ctx context.Context, id ServiceId, options ListBySubscriptionOperationOptions) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath(fmt.Sprintf("%s/reports/bySubscription", id.ID())),
...
func (c ReportsClient) preparerForListBySubscriptionWithNextLink(ctx context.Context, nextLink string) (*http.Request, error) {
...
func (c ReportsClient) responderForListBySubscription(resp *http.Response) (result ListBySubscriptionOperationResponse, err error) {
	type page struct {
	...
	if respObj.NextLink != nil {
		result.nextPageFunc = func(ctx context.Context, nextLink string) (result ListBySubscriptionOperationResponse, err error) {
			...
			if err != nil {
				err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListBySubscription", result.HttpResponse, "Failure sending request")
				return
			}

			result, err = c.responderForListBySubscription(result.HttpResponse)
			if err != nil {
				err = autorest.NewErrorWithError(err, "reports.ReportsClient", "ListBySubscription", result.HttpResponse, "Failure responding to request")
				return
			}
