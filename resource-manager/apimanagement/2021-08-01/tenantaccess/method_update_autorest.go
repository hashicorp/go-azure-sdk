...
func (c TenantAccessClient) Update(ctx context.Context, id AccessId, input AccessInformationUpdateParameters, options UpdateOperationOptions) (result UpdateOperationResponse, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "tenantaccess.TenantAccessClient", "Update", result.HttpResponse, "Failure sending request")
		return
	}

	result, err = c.responderForUpdate(result.HttpResponse)
	if err != nil {
		err = autorest.NewErrorWithError(err, "tenantaccess.TenantAccessClient", "Update", result.HttpResponse, "Failure responding to request")
		return
	}
...
func (c TenantAccessClient) responderForUpdate(resp *http.Response) (result UpdateOperationResponse, err error) {
