...
func TestParseIntegrationAccountID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseIntegrationAccountID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseIntegrationAccountIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *IntegrationAccountId
	...
	for _, v := range testData {
		...
		actual, err := ParseIntegrationAccountIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForIntegrationAccountId(t *testing.T) {
	segments := IntegrationAccountId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("IntegrationAccountId has no segments")
	}
