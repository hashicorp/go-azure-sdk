...
func TestParseBatchConfigurationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseBatchConfigurationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseBatchConfigurationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *BatchConfigurationId
	...
	for _, v := range testData {
		...
		actual, err := ParseBatchConfigurationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForBatchConfigurationId(t *testing.T) {
	segments := BatchConfigurationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("BatchConfigurationId has no segments")
	}
