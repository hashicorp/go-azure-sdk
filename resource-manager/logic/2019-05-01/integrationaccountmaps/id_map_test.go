...
func TestParseMapIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MapId
	...
	for _, v := range testData {
		...
		actual, err := ParseMapIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMapId(t *testing.T) {
	segments := MapId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MapId has no segments")
	}
