package invitation

import (
	"context"
	"fmt"
	"net/http"

	"github.com/hashicorp/go-azure-sdk/microsoft-graph/common/beta/models"
	"github.com/hashicorp/go-azure-sdk/sdk/client"
	"github.com/hashicorp/go-azure-sdk/sdk/odata"
)

// Copyright (c) HashiCorp Inc. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.

type ListInvitationsOperationResponse struct {
	HttpResponse *http.Response
	OData        *odata.OData
	Model        *[]models.InvitationCollectionResponse
}

type ListInvitationsCompleteResult struct {
	Items []models.InvitationCollectionResponse
}

// ListInvitations ...
func (c InvitationClient) ListInvitations(ctx context.Context) (result ListInvitationsOperationResponse, err error) {
	opts := client.RequestOptions{
		ContentType: "application/json; charset=utf-8",
		ExpectedStatusCodes: []int{
			http.StatusOK,
		},
		HttpMethod: http.MethodGet,
		Path:       "/invitations",
	}

	req, err := c.Client.NewRequest(ctx, opts)
	if err != nil {
		return
	}

	var resp *client.Response
	resp, err = req.ExecutePaged(ctx)
	if resp != nil {
		result.OData = resp.OData
		result.HttpResponse = resp.Response
	}
	if err != nil {
		return
	}

	var values struct {
		Values *[]models.InvitationCollectionResponse `json:"value"`
	}
	if err = resp.Unmarshal(&values); err != nil {
		return
	}

	result.Model = values.Values

	return
}

// ListInvitationsComplete retrieves all the results into a single object
func (c InvitationClient) ListInvitationsComplete(ctx context.Context) (ListInvitationsCompleteResult, error) {
	return c.ListInvitationsCompleteMatchingPredicate(ctx, models.InvitationCollectionResponseOperationPredicate{})
}

// ListInvitationsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c InvitationClient) ListInvitationsCompleteMatchingPredicate(ctx context.Context, predicate models.InvitationCollectionResponseOperationPredicate) (result ListInvitationsCompleteResult, err error) {
	items := make([]models.InvitationCollectionResponse, 0)

	resp, err := c.ListInvitations(ctx)
	if err != nil {
		err = fmt.Errorf("loading results: %+v", err)
		return
	}
	if resp.Model != nil {
		for _, v := range *resp.Model {
			if predicate.Matches(v) {
				items = append(items, v)
			}
		}
	}

	result = ListInvitationsCompleteResult{
		Items: items,
	}
	return
}
