...
func (c ServicePrincipalSignInActivityClient) ListServicePrincipalSignInActivities(ctx context.Context) (result ListServicePrincipalSignInActivitiesOperationResponse, err error) {
...
func (c ServicePrincipalSignInActivityClient) ListServicePrincipalSignInActivitiesComplete(ctx context.Context) (ListServicePrincipalSignInActivitiesCompleteResult, error) {
	return c.ListServicePrincipalSignInActivitiesCompleteMatchingPredicate(ctx, ServicePrincipalSignInActivityOperationPredicate{})
}

// ListServicePrincipalSignInActivitiesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ServicePrincipalSignInActivityClient) ListServicePrincipalSignInActivitiesCompleteMatchingPredicate(ctx context.Context, predicate ServicePrincipalSignInActivityOperationPredicate) (result ListServicePrincipalSignInActivitiesCompleteResult, err error) {
	items := make([]beta.ServicePrincipalSignInActivity, 0)

	resp, err := c.ListServicePrincipalSignInActivities(ctx)
	if err != nil {
