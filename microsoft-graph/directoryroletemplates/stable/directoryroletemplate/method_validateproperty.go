...
type ValidatePropertyOperationResponse struct {
	HttpResponse *http.Response
	OData        *odata.OData
}

// ValidateProperty - Invoke action validateProperties. Validate that a Microsoft 365 group's display name or mail
// nickname complies with naming policies. Clients can use this API to determine whether a display name or mail nickname
// is valid before trying to create a Microsoft 365 group. To validate the properties of an existing group, use the
// group: validateProperties function. The following policy validations are performed for the display name and mail
...
func (c DirectoryRoleTemplateClient) ValidateProperty(ctx context.Context, input ValidatePropertyRequest) (result ValidatePropertyOperationResponse, err error) {
