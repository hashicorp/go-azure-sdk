...
func (s *WindowsInformationProtectionAppCollectionResponse) UnmarshalJSON(bytes []byte) error {
	type alias WindowsInformationProtectionAppCollectionResponse
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into WindowsInformationProtectionAppCollectionResponse: %+v", err)
	}
