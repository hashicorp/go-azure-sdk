...
func (s *BaseDeviceManagementConfigurationSimpleSettingDefinitionImpl) UnmarshalJSON(bytes []byte) error {
	type alias BaseDeviceManagementConfigurationSimpleSettingDefinitionImpl
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into BaseDeviceManagementConfigurationSimpleSettingDefinitionImpl: %+v", err)
	}
