...
func (s *AppleExpeditedCheckinConfigurationBaseCollectionResponse) UnmarshalJSON(bytes []byte) error {
	type alias AppleExpeditedCheckinConfigurationBaseCollectionResponse
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into AppleExpeditedCheckinConfigurationBaseCollectionResponse: %+v", err)
	}
