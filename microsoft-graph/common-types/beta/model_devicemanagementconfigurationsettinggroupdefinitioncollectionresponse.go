...
func (s *DeviceManagementConfigurationSettingGroupDefinitionCollectionResponse) UnmarshalJSON(bytes []byte) error {
	type alias DeviceManagementConfigurationSettingGroupDefinitionCollectionResponse
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into DeviceManagementConfigurationSettingGroupDefinitionCollectionResponse: %+v", err)
	}
