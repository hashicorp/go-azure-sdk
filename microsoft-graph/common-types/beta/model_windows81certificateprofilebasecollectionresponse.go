...
func (s *Windows81CertificateProfileBaseCollectionResponse) UnmarshalJSON(bytes []byte) error {
	type alias Windows81CertificateProfileBaseCollectionResponse
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into Windows81CertificateProfileBaseCollectionResponse: %+v", err)
	}
