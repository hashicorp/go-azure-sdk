...
func (s *DeviceManagementSettingCategoryCollectionResponse) UnmarshalJSON(bytes []byte) error {
	type alias DeviceManagementSettingCategoryCollectionResponse
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into DeviceManagementSettingCategoryCollectionResponse: %+v", err)
	}
