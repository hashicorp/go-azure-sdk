...
func (s *WindowsPhone81CertificateProfileBaseCollectionResponse) UnmarshalJSON(bytes []byte) error {
	type alias WindowsPhone81CertificateProfileBaseCollectionResponse
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into WindowsPhone81CertificateProfileBaseCollectionResponse: %+v", err)
	}
