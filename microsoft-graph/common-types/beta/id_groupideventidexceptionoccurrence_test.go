...
func TestParseGroupIdEventIdExceptionOccurrenceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdEventIdExceptionOccurrenceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupIdEventIdExceptionOccurrenceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupIdEventIdExceptionOccurrenceId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdEventIdExceptionOccurrenceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupIdEventIdExceptionOccurrenceId(t *testing.T) {
	segments := GroupIdEventIdExceptionOccurrenceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupIdEventIdExceptionOccurrenceId has no segments")
	}
