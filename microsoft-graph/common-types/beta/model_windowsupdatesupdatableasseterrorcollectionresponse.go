...
func (s *WindowsUpdatesUpdatableAssetErrorCollectionResponse) UnmarshalJSON(bytes []byte) error {
	type alias WindowsUpdatesUpdatableAssetErrorCollectionResponse
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into WindowsUpdatesUpdatableAssetErrorCollectionResponse: %+v", err)
	}
