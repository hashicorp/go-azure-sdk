...
func (s *AndroidDeviceOwnerWiFiConfigurationCollectionResponse) UnmarshalJSON(bytes []byte) error {
	type alias AndroidDeviceOwnerWiFiConfigurationCollectionResponse
	var decoded alias
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling into AndroidDeviceOwnerWiFiConfigurationCollectionResponse: %+v", err)
	}
