...
func (c OwnedObjectClient) ListOwnedObjectApplications(ctx context.Context, id stable.ServicePrincipalId, options ListOwnedObjectApplicationsOperationOptions) (result ListOwnedObjectApplicationsOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:          fmt.Sprintf("%s/ownedObjects/application", id.ID()),
...
func (c OwnedObjectClient) ListOwnedObjectApplicationsComplete(ctx context.Context, id stable.ServicePrincipalId, options ListOwnedObjectApplicationsOperationOptions) (ListOwnedObjectApplicationsCompleteResult, error) {
	return c.ListOwnedObjectApplicationsCompleteMatchingPredicate(ctx, id, options, ApplicationOperationPredicate{})
}

// ListOwnedObjectApplicationsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OwnedObjectClient) ListOwnedObjectApplicationsCompleteMatchingPredicate(ctx context.Context, id stable.ServicePrincipalId, options ListOwnedObjectApplicationsOperationOptions, predicate ApplicationOperationPredicate) (result ListOwnedObjectApplicationsCompleteResult, err error) {
	items := make([]stable.Application, 0)

	resp, err := c.ListOwnedObjectApplications(ctx, id, options)
	if err != nil {
