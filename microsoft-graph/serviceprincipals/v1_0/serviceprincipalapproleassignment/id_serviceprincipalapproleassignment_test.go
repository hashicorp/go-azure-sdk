...
func TestNewServicePrincipalAppRoleAssignmentID(t *testing.T) {
	id := NewServicePrincipalAppRoleAssignmentID("servicePrincipalIdValue", "appRoleAssignmentIdValue")
...
func TestFormatServicePrincipalAppRoleAssignmentID(t *testing.T) {
	actual := NewServicePrincipalAppRoleAssignmentID("servicePrincipalIdValue", "appRoleAssignmentIdValue").ID()
...
func TestParseServicePrincipalAppRoleAssignmentID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ServicePrincipalAppRoleAssignmentId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseServicePrincipalAppRoleAssignmentID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseServicePrincipalAppRoleAssignmentIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ServicePrincipalAppRoleAssignmentId
	...
	for _, v := range testData {
		...
		actual, err := ParseServicePrincipalAppRoleAssignmentIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForServicePrincipalAppRoleAssignmentId(t *testing.T) {
	segments := ServicePrincipalAppRoleAssignmentId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ServicePrincipalAppRoleAssignmentId has no segments")
	}
