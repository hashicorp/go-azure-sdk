...
func TestNewServicePrincipalAppManagementPolicyID(t *testing.T) {
	id := NewServicePrincipalAppManagementPolicyID("servicePrincipalIdValue", "appManagementPolicyIdValue")
...
func TestFormatServicePrincipalAppManagementPolicyID(t *testing.T) {
	actual := NewServicePrincipalAppManagementPolicyID("servicePrincipalIdValue", "appManagementPolicyIdValue").ID()
...
func TestParseServicePrincipalAppManagementPolicyID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ServicePrincipalAppManagementPolicyId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseServicePrincipalAppManagementPolicyID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseServicePrincipalAppManagementPolicyIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ServicePrincipalAppManagementPolicyId
	...
	for _, v := range testData {
		...
		actual, err := ParseServicePrincipalAppManagementPolicyIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForServicePrincipalAppManagementPolicyId(t *testing.T) {
	segments := ServicePrincipalAppManagementPolicyId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("ServicePrincipalAppManagementPolicyId has no segments")
	}
