package administrativeunitscopedrolemember

import (
	"fmt"

	"github.com/hashicorp/go-azure-sdk/sdk/client/msgraph"
	sdkEnv "github.com/hashicorp/go-azure-sdk/sdk/environments"
)

// Copyright (c) HashiCorp Inc. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.

type AdministrativeUnitScopedRoleMemberClient struct {
	Client *msgraph.Client
}

func NewAdministrativeUnitScopedRoleMemberClientWithBaseURI(api sdkEnv.Api) (*AdministrativeUnitScopedRoleMemberClient, error) {
	client, err := msgraph.NewMsGraphClient(api, "administrativeunitscopedrolemember", defaultApiVersion)
	if err != nil {
		return nil, fmt.Errorf("instantiating AdministrativeUnitScopedRoleMemberClient: %+v", err)
	}

	return &AdministrativeUnitScopedRoleMemberClient{
		Client: client,
	}, nil
}
