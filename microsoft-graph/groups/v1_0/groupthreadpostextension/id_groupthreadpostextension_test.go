...
func TestNewGroupThreadPostExtensionID(t *testing.T) {
	id := NewGroupThreadPostExtensionID("groupIdValue", "conversationThreadIdValue", "postIdValue", "extensionIdValue")
...
func TestFormatGroupThreadPostExtensionID(t *testing.T) {
	actual := NewGroupThreadPostExtensionID("groupIdValue", "conversationThreadIdValue", "postIdValue", "extensionIdValue").ID()
...
func TestParseGroupThreadPostExtensionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupThreadPostExtensionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupThreadPostExtensionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupThreadPostExtensionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupThreadPostExtensionId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupThreadPostExtensionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupThreadPostExtensionId(t *testing.T) {
	segments := GroupThreadPostExtensionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupThreadPostExtensionId has no segments")
	}
