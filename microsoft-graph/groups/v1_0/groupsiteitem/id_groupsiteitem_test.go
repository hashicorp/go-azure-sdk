...
func TestNewGroupSiteItemID(t *testing.T) {
	id := NewGroupSiteItemID("groupIdValue", "siteIdValue", "baseItemIdValue")
...
func TestFormatGroupSiteItemID(t *testing.T) {
	actual := NewGroupSiteItemID("groupIdValue", "siteIdValue", "baseItemIdValue").ID()
...
func TestParseGroupSiteItemID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupSiteItemId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupSiteItemID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupSiteItemIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupSiteItemId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupSiteItemIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupSiteItemId(t *testing.T) {
	segments := GroupSiteItemId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupSiteItemId has no segments")
	}
