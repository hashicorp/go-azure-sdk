...
func TestNewGroupPlannerPlanTaskID(t *testing.T) {
	id := NewGroupPlannerPlanTaskID("groupIdValue", "plannerPlanIdValue", "plannerTaskIdValue")
...
func TestFormatGroupPlannerPlanTaskID(t *testing.T) {
	actual := NewGroupPlannerPlanTaskID("groupIdValue", "plannerPlanIdValue", "plannerTaskIdValue").ID()
...
func TestParseGroupPlannerPlanTaskID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupPlannerPlanTaskId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupPlannerPlanTaskID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupPlannerPlanTaskIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupPlannerPlanTaskId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupPlannerPlanTaskIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupPlannerPlanTaskId(t *testing.T) {
	segments := GroupPlannerPlanTaskId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupPlannerPlanTaskId has no segments")
	}
