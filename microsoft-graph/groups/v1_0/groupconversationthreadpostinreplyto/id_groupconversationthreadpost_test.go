...
func TestNewGroupConversationThreadPostID(t *testing.T) {
	id := NewGroupConversationThreadPostID("groupIdValue", "conversationIdValue", "conversationThreadIdValue", "postIdValue")
...
func TestFormatGroupConversationThreadPostID(t *testing.T) {
	actual := NewGroupConversationThreadPostID("groupIdValue", "conversationIdValue", "conversationThreadIdValue", "postIdValue").ID()
...
func TestParseGroupConversationThreadPostID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupConversationThreadPostId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupConversationThreadPostID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupConversationThreadPostIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupConversationThreadPostId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupConversationThreadPostIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupConversationThreadPostId(t *testing.T) {
	segments := GroupConversationThreadPostId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupConversationThreadPostId has no segments")
	}
