...
func TestNewGroupPlannerPlanBucketTaskID(t *testing.T) {
	id := NewGroupPlannerPlanBucketTaskID("groupIdValue", "plannerPlanIdValue", "plannerBucketIdValue", "plannerTaskIdValue")
...
func TestFormatGroupPlannerPlanBucketTaskID(t *testing.T) {
	actual := NewGroupPlannerPlanBucketTaskID("groupIdValue", "plannerPlanIdValue", "plannerBucketIdValue", "plannerTaskIdValue").ID()
...
func TestParseGroupPlannerPlanBucketTaskID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupPlannerPlanBucketTaskId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupPlannerPlanBucketTaskID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupPlannerPlanBucketTaskIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupPlannerPlanBucketTaskId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupPlannerPlanBucketTaskIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupPlannerPlanBucketTaskId(t *testing.T) {
	segments := GroupPlannerPlanBucketTaskId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupPlannerPlanBucketTaskId has no segments")
	}
