...
func TestNewGroupOnenotePageID(t *testing.T) {
	id := NewGroupOnenotePageID("groupIdValue", "onenotePageIdValue")
...
func TestFormatGroupOnenotePageID(t *testing.T) {
	actual := NewGroupOnenotePageID("groupIdValue", "onenotePageIdValue").ID()
...
func TestParseGroupOnenotePageID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupOnenotePageId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupOnenotePageID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupOnenotePageIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupOnenotePageId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupOnenotePageIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupOnenotePageId(t *testing.T) {
	segments := GroupOnenotePageId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupOnenotePageId has no segments")
	}
