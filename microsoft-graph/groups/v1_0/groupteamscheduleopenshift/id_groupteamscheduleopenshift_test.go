...
func TestNewGroupTeamScheduleOpenShiftID(t *testing.T) {
	id := NewGroupTeamScheduleOpenShiftID("groupIdValue", "openShiftIdValue")
...
func TestFormatGroupTeamScheduleOpenShiftID(t *testing.T) {
	actual := NewGroupTeamScheduleOpenShiftID("groupIdValue", "openShiftIdValue").ID()
...
func TestParseGroupTeamScheduleOpenShiftID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupTeamScheduleOpenShiftId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupTeamScheduleOpenShiftID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupTeamScheduleOpenShiftIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupTeamScheduleOpenShiftId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupTeamScheduleOpenShiftIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupTeamScheduleOpenShiftId(t *testing.T) {
	segments := GroupTeamScheduleOpenShiftId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupTeamScheduleOpenShiftId has no segments")
	}
