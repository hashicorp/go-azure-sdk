...
func TestNewGroupSiteTermStoreGroupSetChildrenID(t *testing.T) {
	id := NewGroupSiteTermStoreGroupSetChildrenID("groupIdValue", "siteIdValue", "storeIdValue", "groupId1Value", "setIdValue", "termIdValue")
...
func TestFormatGroupSiteTermStoreGroupSetChildrenID(t *testing.T) {
	actual := NewGroupSiteTermStoreGroupSetChildrenID("groupIdValue", "siteIdValue", "storeIdValue", "groupId1Value", "setIdValue", "termIdValue").ID()
	expected := "/groups/groupIdValue/sites/siteIdValue/termStores/storeIdValue/groups/groupId1Value/sets/setIdValue/children/termIdValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseGroupSiteTermStoreGroupSetChildrenID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupSiteTermStoreGroupSetChildrenId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/groups/groupIdValue/sites/siteIdValue/termStores/storeIdValue/groups/groupId1Value/sets/setIdValue/children/termIdValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseGroupSiteTermStoreGroupSetChildrenID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupSiteTermStoreGroupSetChildrenIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupSiteTermStoreGroupSetChildrenId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupSiteTermStoreGroupSetChildrenIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupSiteTermStoreGroupSetChildrenId(t *testing.T) {
	segments := GroupSiteTermStoreGroupSetChildrenId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupSiteTermStoreGroupSetChildrenId has no segments")
	}
