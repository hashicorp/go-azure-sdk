...
func TestNewGroupSiteTermStoreSetTermChildrenID(t *testing.T) {
	id := NewGroupSiteTermStoreSetTermChildrenID("groupIdValue", "siteIdValue", "storeIdValue", "setIdValue", "termIdValue", "termId1Value")
...
func TestFormatGroupSiteTermStoreSetTermChildrenID(t *testing.T) {
	actual := NewGroupSiteTermStoreSetTermChildrenID("groupIdValue", "siteIdValue", "storeIdValue", "setIdValue", "termIdValue", "termId1Value").ID()
...
func TestParseGroupSiteTermStoreSetTermChildrenID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupSiteTermStoreSetTermChildrenId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupSiteTermStoreSetTermChildrenID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupSiteTermStoreSetTermChildrenIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupSiteTermStoreSetTermChildrenId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupSiteTermStoreSetTermChildrenIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupSiteTermStoreSetTermChildrenId(t *testing.T) {
	segments := GroupSiteTermStoreSetTermChildrenId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupSiteTermStoreSetTermChildrenId has no segments")
	}
