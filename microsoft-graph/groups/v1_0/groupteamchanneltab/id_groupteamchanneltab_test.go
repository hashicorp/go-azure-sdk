...
func TestNewGroupTeamChannelTabID(t *testing.T) {
	id := NewGroupTeamChannelTabID("groupIdValue", "channelIdValue", "teamsTabIdValue")
...
func TestFormatGroupTeamChannelTabID(t *testing.T) {
	actual := NewGroupTeamChannelTabID("groupIdValue", "channelIdValue", "teamsTabIdValue").ID()
...
func TestParseGroupTeamChannelTabID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupTeamChannelTabId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupTeamChannelTabID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupTeamChannelTabIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupTeamChannelTabId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupTeamChannelTabIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupTeamChannelTabId(t *testing.T) {
	segments := GroupTeamChannelTabId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupTeamChannelTabId has no segments")
	}
