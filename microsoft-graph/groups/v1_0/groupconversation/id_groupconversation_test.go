...
func TestNewGroupConversationID(t *testing.T) {
	id := NewGroupConversationID("groupIdValue", "conversationIdValue")
...
func TestFormatGroupConversationID(t *testing.T) {
	actual := NewGroupConversationID("groupIdValue", "conversationIdValue").ID()
...
func TestParseGroupConversationID(t *testing.T) {
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupConversationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupConversationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupConversationId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupConversationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupConversationId(t *testing.T) {
	segments := GroupConversationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupConversationId has no segments")
	}
