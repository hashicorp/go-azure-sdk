...
func TestNewGroupGroupLifecyclePolicyID(t *testing.T) {
	id := NewGroupGroupLifecyclePolicyID("groupIdValue", "groupLifecyclePolicyIdValue")
...
func TestFormatGroupGroupLifecyclePolicyID(t *testing.T) {
	actual := NewGroupGroupLifecyclePolicyID("groupIdValue", "groupLifecyclePolicyIdValue").ID()
...
func TestParseGroupGroupLifecyclePolicyID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupGroupLifecyclePolicyId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupGroupLifecyclePolicyID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupGroupLifecyclePolicyIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupGroupLifecyclePolicyId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupGroupLifecyclePolicyIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupGroupLifecyclePolicyId(t *testing.T) {
	segments := GroupGroupLifecyclePolicyId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupGroupLifecyclePolicyId has no segments")
	}
