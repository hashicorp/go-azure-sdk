...
func TestNewGroupOnenoteNotebookSectionGroupID(t *testing.T) {
	id := NewGroupOnenoteNotebookSectionGroupID("groupIdValue", "notebookIdValue", "sectionGroupIdValue")
...
func TestFormatGroupOnenoteNotebookSectionGroupID(t *testing.T) {
	actual := NewGroupOnenoteNotebookSectionGroupID("groupIdValue", "notebookIdValue", "sectionGroupIdValue").ID()
	expected := "/groups/groupIdValue/onenote/notebooks/notebookIdValue/sectionGroups/sectionGroupIdValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseGroupOnenoteNotebookSectionGroupID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupOnenoteNotebookSectionGroupId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/groups/groupIdValue/onenote/notebooks/notebookIdValue/sectionGroups/sectionGroupIdValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseGroupOnenoteNotebookSectionGroupID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupOnenoteNotebookSectionGroupIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupOnenoteNotebookSectionGroupId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupOnenoteNotebookSectionGroupIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupOnenoteNotebookSectionGroupId(t *testing.T) {
	segments := GroupOnenoteNotebookSectionGroupId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupOnenoteNotebookSectionGroupId has no segments")
	}
