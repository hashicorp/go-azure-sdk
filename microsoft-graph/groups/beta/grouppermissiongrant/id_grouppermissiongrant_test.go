...
func TestNewGroupPermissionGrantID(t *testing.T) {
	id := NewGroupPermissionGrantID("groupIdValue", "resourceSpecificPermissionGrantIdValue")
...
func TestFormatGroupPermissionGrantID(t *testing.T) {
	actual := NewGroupPermissionGrantID("groupIdValue", "resourceSpecificPermissionGrantIdValue").ID()
...
func TestParseGroupPermissionGrantID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupPermissionGrantId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupPermissionGrantID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupPermissionGrantIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupPermissionGrantId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupPermissionGrantIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupPermissionGrantId(t *testing.T) {
	segments := GroupPermissionGrantId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupPermissionGrantId has no segments")
	}
