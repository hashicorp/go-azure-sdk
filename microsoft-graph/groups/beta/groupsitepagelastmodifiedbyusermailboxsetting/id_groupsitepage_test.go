package groupsitepagelastmodifiedbyusermailboxsetting
...
func TestNewGroupSitePageID(t *testing.T) {
	id := NewGroupSitePageID("groupIdValue", "siteIdValue", "baseSitePageIdValue")
...
func TestFormatGroupSitePageID(t *testing.T) {
	actual := NewGroupSitePageID("groupIdValue", "siteIdValue", "baseSitePageIdValue").ID()
...
func TestParseGroupSitePageID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupSitePageId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupSitePageID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupSitePageIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupSitePageId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupSitePageIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupSitePageId(t *testing.T) {
	segments := GroupSitePageId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupSitePageId has no segments")
	}
