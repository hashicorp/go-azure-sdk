...
func TestNewGroupThreadPostInReplyToMentionID(t *testing.T) {
	id := NewGroupThreadPostInReplyToMentionID("groupIdValue", "conversationThreadIdValue", "postIdValue", "mentionIdValue")
...
func TestFormatGroupThreadPostInReplyToMentionID(t *testing.T) {
	actual := NewGroupThreadPostInReplyToMentionID("groupIdValue", "conversationThreadIdValue", "postIdValue", "mentionIdValue").ID()
...
func TestParseGroupThreadPostInReplyToMentionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupThreadPostInReplyToMentionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupThreadPostInReplyToMentionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupThreadPostInReplyToMentionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupThreadPostInReplyToMentionId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupThreadPostInReplyToMentionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupThreadPostInReplyToMentionId(t *testing.T) {
	segments := GroupThreadPostInReplyToMentionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupThreadPostInReplyToMentionId has no segments")
	}
