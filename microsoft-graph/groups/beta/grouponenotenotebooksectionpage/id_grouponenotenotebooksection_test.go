...
func TestNewGroupOnenoteNotebookSectionID(t *testing.T) {
	id := NewGroupOnenoteNotebookSectionID("groupIdValue", "notebookIdValue", "onenoteSectionIdValue")
...
func TestFormatGroupOnenoteNotebookSectionID(t *testing.T) {
	actual := NewGroupOnenoteNotebookSectionID("groupIdValue", "notebookIdValue", "onenoteSectionIdValue").ID()
	expected := "/groups/groupIdValue/onenote/notebooks/notebookIdValue/sections/onenoteSectionIdValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseGroupOnenoteNotebookSectionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupOnenoteNotebookSectionId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/groups/groupIdValue/onenote/notebooks/notebookIdValue/sections/onenoteSectionIdValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseGroupOnenoteNotebookSectionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupOnenoteNotebookSectionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupOnenoteNotebookSectionId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupOnenoteNotebookSectionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupOnenoteNotebookSectionId(t *testing.T) {
	segments := GroupOnenoteNotebookSectionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupOnenoteNotebookSectionId has no segments")
	}
