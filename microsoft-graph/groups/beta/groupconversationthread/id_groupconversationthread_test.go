...
func TestNewGroupConversationThreadID(t *testing.T) {
	id := NewGroupConversationThreadID("groupIdValue", "conversationIdValue", "conversationThreadIdValue")
...
func TestFormatGroupConversationThreadID(t *testing.T) {
	actual := NewGroupConversationThreadID("groupIdValue", "conversationIdValue", "conversationThreadIdValue").ID()
...
func TestParseGroupConversationThreadID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupConversationThreadId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupConversationThreadID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupConversationThreadIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupConversationThreadId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupConversationThreadIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupConversationThreadId(t *testing.T) {
	segments := GroupConversationThreadId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupConversationThreadId has no segments")
	}
