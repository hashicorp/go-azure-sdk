...
func TestNewGroupPlannerPlanID(t *testing.T) {
	id := NewGroupPlannerPlanID("groupIdValue", "plannerPlanIdValue")
...
func TestFormatGroupPlannerPlanID(t *testing.T) {
	actual := NewGroupPlannerPlanID("groupIdValue", "plannerPlanIdValue").ID()
...
func TestParseGroupPlannerPlanID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupPlannerPlanId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupPlannerPlanID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupPlannerPlanIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupPlannerPlanId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupPlannerPlanIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupPlannerPlanId(t *testing.T) {
	segments := GroupPlannerPlanId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupPlannerPlanId has no segments")
	}
