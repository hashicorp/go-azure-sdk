...
func TestNewGroupThreadPostInReplyToExtensionID(t *testing.T) {
	id := NewGroupThreadPostInReplyToExtensionID("groupIdValue", "conversationThreadIdValue", "postIdValue", "extensionIdValue")
...
func TestFormatGroupThreadPostInReplyToExtensionID(t *testing.T) {
	actual := NewGroupThreadPostInReplyToExtensionID("groupIdValue", "conversationThreadIdValue", "postIdValue", "extensionIdValue").ID()
...
func TestParseGroupThreadPostInReplyToExtensionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupThreadPostInReplyToExtensionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupThreadPostInReplyToExtensionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupThreadPostInReplyToExtensionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupThreadPostInReplyToExtensionId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupThreadPostInReplyToExtensionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupThreadPostInReplyToExtensionId(t *testing.T) {
	segments := GroupThreadPostInReplyToExtensionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupThreadPostInReplyToExtensionId has no segments")
	}
