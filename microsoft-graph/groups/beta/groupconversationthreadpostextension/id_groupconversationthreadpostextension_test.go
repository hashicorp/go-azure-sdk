...
func TestNewGroupConversationThreadPostExtensionID(t *testing.T) {
	id := NewGroupConversationThreadPostExtensionID("groupIdValue", "conversationIdValue", "conversationThreadIdValue", "postIdValue", "extensionIdValue")
...
func TestFormatGroupConversationThreadPostExtensionID(t *testing.T) {
	actual := NewGroupConversationThreadPostExtensionID("groupIdValue", "conversationIdValue", "conversationThreadIdValue", "postIdValue", "extensionIdValue").ID()
...
func TestParseGroupConversationThreadPostExtensionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupConversationThreadPostExtensionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupConversationThreadPostExtensionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupConversationThreadPostExtensionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupConversationThreadPostExtensionId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupConversationThreadPostExtensionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupConversationThreadPostExtensionId(t *testing.T) {
	segments := GroupConversationThreadPostExtensionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupConversationThreadPostExtensionId has no segments")
	}
