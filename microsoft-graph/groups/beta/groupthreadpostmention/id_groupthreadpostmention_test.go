...
func TestNewGroupThreadPostMentionID(t *testing.T) {
	id := NewGroupThreadPostMentionID("groupIdValue", "conversationThreadIdValue", "postIdValue", "mentionIdValue")
...
func TestFormatGroupThreadPostMentionID(t *testing.T) {
	actual := NewGroupThreadPostMentionID("groupIdValue", "conversationThreadIdValue", "postIdValue", "mentionIdValue").ID()
	expected := "/groups/groupIdValue/threads/conversationThreadIdValue/posts/postIdValue/mentions/mentionIdValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseGroupThreadPostMentionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupThreadPostMentionId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		{
			// Incomplete URI
			Input: "/groups/groupIdValue/threads/conversationThreadIdValue/posts/postIdValue/mentions",
			Error: true,
		},
		{
			// Valid URI
			Input: "/groups/groupIdValue/threads/conversationThreadIdValue/posts/postIdValue/mentions/mentionIdValue",
			Expected: &GroupThreadPostMentionId{
		...
		},
		{
			...
			Input: "/groups/groupIdValue/threads/conversationThreadIdValue/posts/postIdValue/mentions/mentionIdValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseGroupThreadPostMentionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupThreadPostMentionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupThreadPostMentionId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupThreadPostMentionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupThreadPostMentionId(t *testing.T) {
	segments := GroupThreadPostMentionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupThreadPostMentionId has no segments")
	}
