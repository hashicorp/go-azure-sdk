...
func TestNewGroupOnenoteResourceID(t *testing.T) {
	id := NewGroupOnenoteResourceID("groupIdValue", "onenoteResourceIdValue")
...
func TestFormatGroupOnenoteResourceID(t *testing.T) {
	actual := NewGroupOnenoteResourceID("groupIdValue", "onenoteResourceIdValue").ID()
...
func TestParseGroupOnenoteResourceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupOnenoteResourceId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupOnenoteResourceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupOnenoteResourceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupOnenoteResourceId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupOnenoteResourceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupOnenoteResourceId(t *testing.T) {
	segments := GroupOnenoteResourceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupOnenoteResourceId has no segments")
	}
