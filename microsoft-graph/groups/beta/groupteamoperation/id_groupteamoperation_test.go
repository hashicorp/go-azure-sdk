...
func TestNewGroupTeamOperationID(t *testing.T) {
	id := NewGroupTeamOperationID("groupIdValue", "teamsAsyncOperationIdValue")
...
func TestFormatGroupTeamOperationID(t *testing.T) {
	actual := NewGroupTeamOperationID("groupIdValue", "teamsAsyncOperationIdValue").ID()
...
func TestParseGroupTeamOperationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupTeamOperationId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupTeamOperationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupTeamOperationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupTeamOperationId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupTeamOperationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupTeamOperationId(t *testing.T) {
	segments := GroupTeamOperationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupTeamOperationId has no segments")
	}
