...
// ListPlannerPlans ...
func (c PlannerPlanClient) ListPlannerPlans(ctx context.Context, id GroupId) (result ListPlannerPlansOperationResponse, err error) {
...
func (c PlannerPlanClient) ListPlannerPlansComplete(ctx context.Context, id GroupId) (ListPlannerPlansCompleteResult, error) {
	return c.ListPlannerPlansCompleteMatchingPredicate(ctx, id, PlannerPlanOperationPredicate{})
}

// ListPlannerPlansCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c PlannerPlanClient) ListPlannerPlansCompleteMatchingPredicate(ctx context.Context, id GroupId, predicate PlannerPlanOperationPredicate) (result ListPlannerPlansCompleteResult, err error) {
	items := make([]stable.PlannerPlan, 0)

	resp, err := c.ListPlannerPlans(ctx, id)
	if err != nil {
