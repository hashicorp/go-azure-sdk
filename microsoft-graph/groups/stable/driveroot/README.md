...
The `driveroot` SDK allows for interaction with the Azure Resource Manager Service `groups` (API Version `stable`).
...
```go
ctx := context.TODO()
id := driveroot.NewGroupIdDriveID("groupIdValue", "driveIdValue")
