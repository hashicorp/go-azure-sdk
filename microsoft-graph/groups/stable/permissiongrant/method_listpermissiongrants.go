...
func (c PermissionGrantClient) ListPermissionGrants(ctx context.Context, id GroupId) (result ListPermissionGrantsOperationResponse, err error) {
...
func (c PermissionGrantClient) ListPermissionGrantsComplete(ctx context.Context, id GroupId) (ListPermissionGrantsCompleteResult, error) {
	return c.ListPermissionGrantsCompleteMatchingPredicate(ctx, id, ResourceSpecificPermissionGrantOperationPredicate{})
}

// ListPermissionGrantsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c PermissionGrantClient) ListPermissionGrantsCompleteMatchingPredicate(ctx context.Context, id GroupId, predicate ResourceSpecificPermissionGrantOperationPredicate) (result ListPermissionGrantsCompleteResult, err error) {
	items := make([]stable.ResourceSpecificPermissionGrant, 0)

	resp, err := c.ListPermissionGrants(ctx, id)
	if err != nil {
