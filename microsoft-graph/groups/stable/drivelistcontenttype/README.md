...
## `github.com/hashicorp/go-azure-sdk/microsoft-graph/groups/stable/drivelistcontenttype` Documentation

The `drivelistcontenttype` SDK allows for interaction with the Azure Resource Manager Service `groups` (API Version `stable`).
...
```go
import "github.com/hashicorp/go-azure-sdk/microsoft-graph/groups/stable/drivelistcontenttype"
```
...
```go
ctx := context.TODO()
id := drivelistcontenttype.NewGroupIdDriveID("groupIdValue", "driveIdValue")
...
```go
ctx := context.TODO()
id := drivelistcontenttype.NewGroupIdDriveIdListContentTypeID("groupIdValue", "driveIdValue", "contentTypeIdValue")
