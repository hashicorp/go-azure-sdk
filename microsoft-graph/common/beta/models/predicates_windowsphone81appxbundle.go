...
type WindowsPhone81AppXBundleOperationPredicate struct {
...
func (p WindowsPhone81AppXBundleOperationPredicate) Matches(input WindowsPhone81AppXBundle) bool {
	...
	if p.IdentityPublisherHash != nil && (input.IdentityPublisherHash == nil || *p.IdentityPublisherHash != *input.IdentityPublisherHash) {
		return false
	}
	...
	if p.PhoneProductIdentifier != nil && (input.PhoneProductIdentifier == nil || *p.PhoneProductIdentifier != *input.PhoneProductIdentifier) {
		return false
	}

	if p.PhonePublisherId != nil && (input.PhonePublisherId == nil || *p.PhonePublisherId != *input.PhonePublisherId) {
		return false
	}
