...
type AndroidManagedStoreWebAppOperationPredicate struct {
...
func (p AndroidManagedStoreWebAppOperationPredicate) Matches(input AndroidManagedStoreWebApp) bool {

	if p.AppIdentifier != nil && (input.AppIdentifier == nil || *p.AppIdentifier != *input.AppIdentifier) {
		return false
	}
	...
	if p.IsSystemApp != nil && (input.IsSystemApp == nil || *p.IsSystemApp != *input.IsSystemApp) {
		return false
	}
	...
	if p.SupportsOemConfig != nil && (input.SupportsOemConfig == nil || *p.SupportsOemConfig != *input.SupportsOemConfig) {
		return false
	}

	if p.TotalLicenseCount != nil && (input.TotalLicenseCount == nil || *p.TotalLicenseCount != *input.TotalLicenseCount) {
		return false
	}
