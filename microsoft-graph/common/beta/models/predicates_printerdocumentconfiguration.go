...
type PrinterDocumentConfigurationOperationPredicate struct {
	Collate       *bool
	Copies        *int64
	Dpi           *int64
	FitPdfToPage  *bool
...
func (p PrinterDocumentConfigurationOperationPredicate) Matches(input PrinterDocumentConfiguration) bool {

	if p.Collate != nil && (input.Collate == nil || *p.Collate != *input.Collate) {
		return false
	}

	if p.Copies != nil && (input.Copies == nil || *p.Copies != *input.Copies) {
		return false
	}
