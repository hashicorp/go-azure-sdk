...
type PurchaseInvoiceLineOperationPredicate struct {
	AccountId                 *string
...
func (p PurchaseInvoiceLineOperationPredicate) Matches(input PurchaseInvoiceLine) bool {
	...
	if p.ExpectedReceiptDate != nil && (input.ExpectedReceiptDate == nil || *p.ExpectedReceiptDate != *input.ExpectedReceiptDate) {
		return false
	}
