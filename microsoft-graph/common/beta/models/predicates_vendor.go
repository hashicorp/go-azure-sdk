...
type VendorOperationPredicate struct {
	Balance               *float64
	Blocked               *string
	CurrencyCode          *string
	CurrencyId            *string
...
func (p VendorOperationPredicate) Matches(input Vendor) bool {

	if p.Balance != nil && (input.Balance == nil || *p.Balance != *input.Balance) {
		return false
	}
