...
type IosEnterpriseWiFiConfigurationOperationPredicate struct {
...
func (p IosEnterpriseWiFiConfigurationOperationPredicate) Matches(input IosEnterpriseWiFiConfiguration) bool {
	...
	if p.DisableMacAddressRandomization != nil && (input.DisableMacAddressRandomization == nil || *p.DisableMacAddressRandomization != *input.DisableMacAddressRandomization) {
		return false
	}
	...
	if p.PasswordFormatString != nil && (input.PasswordFormatString == nil || *p.PasswordFormatString != *input.PasswordFormatString) {
		return false
	}
	...
	if p.UsernameFormatString != nil && (input.UsernameFormatString == nil || *p.UsernameFormatString != *input.UsernameFormatString) {
		return false
	}
