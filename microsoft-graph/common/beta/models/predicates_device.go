...
func (p DeviceOperationPredicate) Matches(input Device) bool {
	...
	if p.EnrollmentType != nil && (input.EnrollmentType == nil || *p.EnrollmentType != *input.EnrollmentType) {
		return false
	}
	...
	if p.IsRooted != nil && (input.IsRooted == nil || *p.IsRooted != *input.IsRooted) {
		return false
	}
	...
	if p.ManagementType != nil && (input.ManagementType == nil || *p.ManagementType != *input.ManagementType) {
		return false
	}
