...
type OfficeSuiteAppOperationPredicate struct {
	AutoAcceptEula                       *bool
...
func (p OfficeSuiteAppOperationPredicate) Matches(input OfficeSuiteApp) bool {

	if p.AutoAcceptEula != nil && (input.AutoAcceptEula == nil || *p.AutoAcceptEula != *input.AutoAcceptEula) {
		return false
	}
	...
	if p.OfficeConfigurationXml != nil && (input.OfficeConfigurationXml == nil || *p.OfficeConfigurationXml != *input.OfficeConfigurationXml) {
		return false
	}
	...
	if p.ShouldUninstallOlderVersionsOfOffice != nil && (input.ShouldUninstallOlderVersionsOfOffice == nil || *p.ShouldUninstallOlderVersionsOfOffice != *input.ShouldUninstallOlderVersionsOfOffice) {
		return false
	}
	...
	if p.TargetVersion != nil && (input.TargetVersion == nil || *p.TargetVersion != *input.TargetVersion) {
		return false
	}

	if p.UpdateVersion != nil && (input.UpdateVersion == nil || *p.UpdateVersion != *input.UpdateVersion) {
		return false
	}
	...
	if p.UseSharedComputerActivation != nil && (input.UseSharedComputerActivation == nil || *p.UseSharedComputerActivation != *input.UseSharedComputerActivation) {
		return false
	}
