...
type UserTrainingContentEventInfoOperationPredicate struct {
	Browser                 *string
...
func (p UserTrainingContentEventInfoOperationPredicate) Matches(input UserTrainingContentEventInfo) bool {

	if p.Browser != nil && (input.Browser == nil || *p.Browser != *input.Browser) {
		return false
	}

	if p.ContentDateTime != nil && (input.ContentDateTime == nil || *p.ContentDateTime != *input.ContentDateTime) {
		return false
	}
