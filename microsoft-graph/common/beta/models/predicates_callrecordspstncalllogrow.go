...
func (p CallRecordsPstnCallLogRowOperationPredicate) Matches(input CallRecordsPstnCallLogRow) bool {
	...
	if p.ClientLocalIpV4Address != nil && (input.ClientLocalIpV4Address == nil || *p.ClientLocalIpV4Address != *input.ClientLocalIpV4Address) {
		return false
	}

	if p.ClientLocalIpV6Address != nil && (input.ClientLocalIpV6Address == nil || *p.ClientLocalIpV6Address != *input.ClientLocalIpV6Address) {
		return false
	}

	if p.ClientPublicIpV4Address != nil && (input.ClientPublicIpV4Address == nil || *p.ClientPublicIpV4Address != *input.ClientPublicIpV4Address) {
		return false
	}

	if p.ClientPublicIpV6Address != nil && (input.ClientPublicIpV6Address == nil || *p.ClientPublicIpV6Address != *input.ClientPublicIpV6Address) {
		return false
	}
