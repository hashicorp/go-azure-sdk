...
type EmailActivityStatisticsOperationPredicate struct {
	AfterHours   *string
	Duration     *string
	EndDate      *string
	Id           *string
...
func (p EmailActivityStatisticsOperationPredicate) Matches(input EmailActivityStatistics) bool {
	...
	if p.ReadEmail != nil && (input.ReadEmail == nil || *p.ReadEmail != *input.ReadEmail) {
		return false
	}

	if p.SentEmail != nil && (input.SentEmail == nil || *p.SentEmail != *input.SentEmail) {
		return false
	}
