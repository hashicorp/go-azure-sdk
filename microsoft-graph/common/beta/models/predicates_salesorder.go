...
type SalesOrderOperationPredicate struct {
...
func (p SalesOrderOperationPredicate) Matches(input SalesOrder) bool {
	...
	if p.FullyShipped != nil && (input.FullyShipped == nil || *p.FullyShipped != *input.FullyShipped) {
		return false
	}
	...
	if p.OrderDate != nil && (input.OrderDate == nil || *p.OrderDate != *input.OrderDate) {
		return false
	}

	if p.PartialShipping != nil && (input.PartialShipping == nil || *p.PartialShipping != *input.PartialShipping) {
		return false
	}
	...
	if p.RequestedDeliveryDate != nil && (input.RequestedDeliveryDate == nil || *p.RequestedDeliveryDate != *input.RequestedDeliveryDate) {
		return false
	}
