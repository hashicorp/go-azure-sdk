...
func (p UnifiedRoleAssignmentOperationPredicate) Matches(input UnifiedRoleAssignment) bool {
	...
	if p.PrincipalOrganizationId != nil && (input.PrincipalOrganizationId == nil || *p.PrincipalOrganizationId != *input.PrincipalOrganizationId) {
		return false
	}

	if p.ResourceScope != nil && (input.ResourceScope == nil || *p.ResourceScope != *input.ResourceScope) {
		return false
	}
