...
func (p ConfigurationManagerClientEnabledFeaturesOperationPredicate) Matches(input ConfigurationManagerClientEnabledFeatures) bool {
	...
	if p.EndpointProtection != nil && (input.EndpointProtection == nil || *p.EndpointProtection != *input.EndpointProtection) {
		return false
	}
	...
	if p.OfficeApps != nil && (input.OfficeApps == nil || *p.OfficeApps != *input.OfficeApps) {
		return false
	}
