...
type WinGetAppOperationPredicate struct {
...
func (p WinGetAppOperationPredicate) Matches(input WinGetApp) bool {
	...
	if p.ManifestHash != nil && (input.ManifestHash == nil || *p.ManifestHash != *input.ManifestHash) {
		return false
	}
	...
	if p.PackageIdentifier != nil && (input.PackageIdentifier == nil || *p.PackageIdentifier != *input.PackageIdentifier) {
		return false
	}
