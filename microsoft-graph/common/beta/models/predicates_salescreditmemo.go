...
type SalesCreditMemoOperationPredicate struct {
...
func (p SalesCreditMemoOperationPredicate) Matches(input SalesCreditMemo) bool {
	...
	if p.CreditMemoDate != nil && (input.CreditMemoDate == nil || *p.CreditMemoDate != *input.CreditMemoDate) {
		return false
	}
	...
	if p.InvoiceNumber != nil && (input.InvoiceNumber == nil || *p.InvoiceNumber != *input.InvoiceNumber) {
		return false
	}
