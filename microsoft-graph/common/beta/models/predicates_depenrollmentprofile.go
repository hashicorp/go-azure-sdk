...
type DepEnrollmentProfileOperationPredicate struct {
...
func (p DepEnrollmentProfileOperationPredicate) Matches(input DepEnrollmentProfile) bool {
	...
	if p.MacOSFileVaultDisabled != nil && (input.MacOSFileVaultDisabled == nil || *p.MacOSFileVaultDisabled != *input.MacOSFileVaultDisabled) {
		return false
	}

	if p.MacOSRegistrationDisabled != nil && (input.MacOSRegistrationDisabled == nil || *p.MacOSRegistrationDisabled != *input.MacOSRegistrationDisabled) {
		return false
	}
