...
func (p PrinterDefaultsOperationPredicate) Matches(input PrinterDefaults) bool {
	...
	if p.DocumentMimeType != nil && (input.DocumentMimeType == nil || *p.DocumentMimeType != *input.DocumentMimeType) {
		return false
	}
	...
	if p.PdfFitToPage != nil && (input.PdfFitToPage == nil || *p.PdfFitToPage != *input.PdfFitToPage) {
		return false
	}
