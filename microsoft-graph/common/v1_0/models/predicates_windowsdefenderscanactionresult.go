...
type WindowsDefenderScanActionResultOperationPredicate struct {
	ActionName          *string
...
func (p WindowsDefenderScanActionResultOperationPredicate) Matches(input WindowsDefenderScanActionResult) bool {
	...
	if p.ScanType != nil && (input.ScanType == nil || *p.ScanType != *input.ScanType) {
		return false
	}
