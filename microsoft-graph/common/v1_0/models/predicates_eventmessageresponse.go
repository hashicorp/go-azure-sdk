...
type EventMessageResponseOperationPredicate struct {
	BodyPreview                *string
	ChangeKey                  *string
...
func (p EventMessageResponseOperationPredicate) Matches(input EventMessageResponse) bool {
	...
	if p.IsDelegated != nil && (input.IsDelegated == nil || *p.IsDelegated != *input.IsDelegated) {
		return false
	}
	...
	if p.IsOutOfDate != nil && (input.IsOutOfDate == nil || *p.IsOutOfDate != *input.IsOutOfDate) {
		return false
	}
