package profilename

import (
	"context"
	"fmt"
	"net/http"

	"github.com/hashicorp/go-azure-sdk/microsoft-graph/common-types/beta"
	"github.com/hashicorp/go-azure-sdk/sdk/client"
	"github.com/hashicorp/go-azure-sdk/sdk/odata"
)

// Copyright (c) HashiCorp Inc. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.

type ListProfileNamesOperationResponse struct {
	HttpResponse *http.Response
	OData        *odata.OData
	Model        *[]beta.PersonName
}

type ListProfileNamesCompleteResult struct {
	LatestHttpResponse *http.Response
	Items              []beta.PersonName
}

type ListProfileNamesCustomPager struct {
	NextLink *odata.Link `json:"@odata.nextLink"`
}

func (p *ListProfileNamesCustomPager) NextPageLink() *odata.Link {
	defer func() {
		p.NextLink = nil
	}()

	return p.NextLink
}

// ListProfileNames ...
func (c ProfileNameClient) ListProfileNames(ctx context.Context) (result ListProfileNamesOperationResponse, err error) {
	opts := client.RequestOptions{
		ContentType: "application/json; charset=utf-8",
		ExpectedStatusCodes: []int{
			http.StatusOK,
		},
		HttpMethod: http.MethodGet,
		Pager:      &ListProfileNamesCustomPager{},
		Path:       "/me/profile/names",
	}

	req, err := c.Client.NewRequest(ctx, opts)
	if err != nil {
		return
	}

	var resp *client.Response
	resp, err = req.ExecutePaged(ctx)
	if resp != nil {
		result.OData = resp.OData
		result.HttpResponse = resp.Response
	}
	if err != nil {
		return
	}

	var values struct {
		Values *[]beta.PersonName `json:"value"`
	}
	if err = resp.Unmarshal(&values); err != nil {
		return
	}

	result.Model = values.Values

	return
}

// ListProfileNamesComplete retrieves all the results into a single object
func (c ProfileNameClient) ListProfileNamesComplete(ctx context.Context) (ListProfileNamesCompleteResult, error) {
	return c.ListProfileNamesCompleteMatchingPredicate(ctx, PersonNameOperationPredicate{})
}

// ListProfileNamesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ProfileNameClient) ListProfileNamesCompleteMatchingPredicate(ctx context.Context, predicate PersonNameOperationPredicate) (result ListProfileNamesCompleteResult, err error) {
	items := make([]beta.PersonName, 0)

	resp, err := c.ListProfileNames(ctx)
	if err != nil {
		result.LatestHttpResponse = resp.HttpResponse
		err = fmt.Errorf("loading results: %+v", err)
		return
	}
	if resp.Model != nil {
		for _, v := range *resp.Model {
			if predicate.Matches(v) {
				items = append(items, v)
			}
		}
	}

	result = ListProfileNamesCompleteResult{
		LatestHttpResponse: resp.HttpResponse,
		Items:              items,
	}
	return
}
