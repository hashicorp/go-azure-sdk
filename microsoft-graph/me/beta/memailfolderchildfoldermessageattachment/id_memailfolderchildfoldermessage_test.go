...
func TestNewMeMailFolderChildFolderMessageID(t *testing.T) {
	id := NewMeMailFolderChildFolderMessageID("mailFolderIdValue", "mailFolderId1Value", "messageIdValue")
...
func TestFormatMeMailFolderChildFolderMessageID(t *testing.T) {
	actual := NewMeMailFolderChildFolderMessageID("mailFolderIdValue", "mailFolderId1Value", "messageIdValue").ID()
	expected := "/me/mailFolders/mailFolderIdValue/childFolders/mailFolderId1Value/messages/messageIdValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseMeMailFolderChildFolderMessageID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeMailFolderChildFolderMessageId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/me/mailFolders/mailFolderIdValue/childFolders/mailFolderId1Value/messages/messageIdValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseMeMailFolderChildFolderMessageID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeMailFolderChildFolderMessageIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeMailFolderChildFolderMessageId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeMailFolderChildFolderMessageIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeMailFolderChildFolderMessageId(t *testing.T) {
	segments := MeMailFolderChildFolderMessageId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeMailFolderChildFolderMessageId has no segments")
	}
