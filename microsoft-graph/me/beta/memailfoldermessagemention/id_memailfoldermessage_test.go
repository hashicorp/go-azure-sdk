...
func TestNewMeMailFolderMessageID(t *testing.T) {
	id := NewMeMailFolderMessageID("mailFolderIdValue", "messageIdValue")
...
func TestFormatMeMailFolderMessageID(t *testing.T) {
	actual := NewMeMailFolderMessageID("mailFolderIdValue", "messageIdValue").ID()
...
func TestParseMeMailFolderMessageID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeMailFolderMessageId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseMeMailFolderMessageID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeMailFolderMessageIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeMailFolderMessageId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeMailFolderMessageIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeMailFolderMessageId(t *testing.T) {
	segments := MeMailFolderMessageId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeMailFolderMessageId has no segments")
	}
