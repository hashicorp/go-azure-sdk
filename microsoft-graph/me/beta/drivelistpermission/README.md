...
The `drivelistpermission` SDK allows for interaction with Microsoft Graph `me` (API Version `beta`).
...
```go
ctx := context.TODO()
id := drivelistpermission.NewMeDriveID("driveId")
...
```go
ctx := context.TODO()
id := drivelistpermission.NewMeDriveIdListPermissionID("driveId", "permissionId")
