...
The `driveitemlistitempermission` SDK allows for interaction with Microsoft Graph `me` (API Version `beta`).
...
```go
ctx := context.TODO()
id := driveitemlistitempermission.NewMeDriveIdItemID("driveId", "driveItemId")
...
```go
ctx := context.TODO()
id := driveitemlistitempermission.NewMeDriveIdItemIdListItemPermissionID("driveId", "driveItemId", "permissionId")
