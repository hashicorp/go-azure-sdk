...
func TestNewMeApprovalStepID(t *testing.T) {
	id := NewMeApprovalStepID("approvalIdValue", "approvalStepIdValue")
...
func TestFormatMeApprovalStepID(t *testing.T) {
	actual := NewMeApprovalStepID("approvalIdValue", "approvalStepIdValue").ID()
...
func TestParseMeApprovalStepID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeApprovalStepId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseMeApprovalStepID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeApprovalStepIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeApprovalStepId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeApprovalStepIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeApprovalStepId(t *testing.T) {
	segments := MeApprovalStepId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeApprovalStepId has no segments")
	}
