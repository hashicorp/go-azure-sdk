...
func TestNewMeChatOperationID(t *testing.T) {
	id := NewMeChatOperationID("chatIdValue", "teamsAsyncOperationIdValue")
...
func TestFormatMeChatOperationID(t *testing.T) {
	actual := NewMeChatOperationID("chatIdValue", "teamsAsyncOperationIdValue").ID()
...
func TestParseMeChatOperationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeChatOperationId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseMeChatOperationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeChatOperationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeChatOperationId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeChatOperationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeChatOperationId(t *testing.T) {
	segments := MeChatOperationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeChatOperationId has no segments")
	}
