...
The `eventexceptionoccurrence` SDK allows for interaction with Microsoft Graph `me` (API Version `beta`).
...
```go
ctx := context.TODO()
id := eventexceptionoccurrence.NewMeEventIdExceptionOccurrenceID("eventId", "eventId1")
...
```go
ctx := context.TODO()
id := eventexceptionoccurrence.NewMeEventID("eventId")
