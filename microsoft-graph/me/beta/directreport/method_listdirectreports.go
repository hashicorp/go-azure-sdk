...
// ListDirectReports ...
func (c DirectReportClient) ListDirectReports(ctx context.Context) (result ListDirectReportsOperationResponse, err error) {
...
func (c DirectReportClient) ListDirectReportsComplete(ctx context.Context) (ListDirectReportsCompleteResult, error) {
	return c.ListDirectReportsCompleteMatchingPredicate(ctx, DirectoryObjectOperationPredicate{})
}

// ListDirectReportsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c DirectReportClient) ListDirectReportsCompleteMatchingPredicate(ctx context.Context, predicate DirectoryObjectOperationPredicate) (result ListDirectReportsCompleteResult, err error) {
	items := make([]beta.DirectoryObject, 0)

	resp, err := c.ListDirectReports(ctx)
	if err != nil {
