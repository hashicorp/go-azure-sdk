...
// ListOutlookTasks ...
func (c OutlookTaskClient) ListOutlookTasks(ctx context.Context) (result ListOutlookTasksOperationResponse, err error) {
...
func (c OutlookTaskClient) ListOutlookTasksComplete(ctx context.Context) (ListOutlookTasksCompleteResult, error) {
	return c.ListOutlookTasksCompleteMatchingPredicate(ctx, OutlookTaskOperationPredicate{})
}

// ListOutlookTasksCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OutlookTaskClient) ListOutlookTasksCompleteMatchingPredicate(ctx context.Context, predicate OutlookTaskOperationPredicate) (result ListOutlookTasksCompleteResult, err error) {
	items := make([]beta.OutlookTask, 0)

	resp, err := c.ListOutlookTasks(ctx)
	if err != nil {
