...
func (c ProfilePublicationClient) ListProfilePublications(ctx context.Context) (result ListProfilePublicationsOperationResponse, err error) {
...
func (c ProfilePublicationClient) ListProfilePublicationsComplete(ctx context.Context) (ListProfilePublicationsCompleteResult, error) {
	return c.ListProfilePublicationsCompleteMatchingPredicate(ctx, ItemPublicationOperationPredicate{})
}

// ListProfilePublicationsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ProfilePublicationClient) ListProfilePublicationsCompleteMatchingPredicate(ctx context.Context, predicate ItemPublicationOperationPredicate) (result ListProfilePublicationsCompleteResult, err error) {
	items := make([]beta.ItemPublication, 0)

	resp, err := c.ListProfilePublications(ctx)
	if err != nil {
