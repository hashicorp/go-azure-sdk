...
func TestNewMeOnenoteNotebookSectionPageID(t *testing.T) {
	id := NewMeOnenoteNotebookSectionPageID("notebookIdValue", "onenoteSectionIdValue", "onenotePageIdValue")
...
func TestFormatMeOnenoteNotebookSectionPageID(t *testing.T) {
	actual := NewMeOnenoteNotebookSectionPageID("notebookIdValue", "onenoteSectionIdValue", "onenotePageIdValue").ID()
	expected := "/me/onenote/notebooks/notebookIdValue/sections/onenoteSectionIdValue/pages/onenotePageIdValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseMeOnenoteNotebookSectionPageID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeOnenoteNotebookSectionPageId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		{
			// Incomplete URI
			Input: "/me/onenote/notebooks/notebookIdValue/sections/onenoteSectionIdValue/pages",
			Error: true,
		},
		{
			// Valid URI
			Input: "/me/onenote/notebooks/notebookIdValue/sections/onenoteSectionIdValue/pages/onenotePageIdValue",
			Expected: &MeOnenoteNotebookSectionPageId{
		...
		},
		{
			...
			Input: "/me/onenote/notebooks/notebookIdValue/sections/onenoteSectionIdValue/pages/onenotePageIdValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseMeOnenoteNotebookSectionPageID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeOnenoteNotebookSectionPageIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeOnenoteNotebookSectionPageId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeOnenoteNotebookSectionPageIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeOnenoteNotebookSectionPageId(t *testing.T) {
	segments := MeOnenoteNotebookSectionPageId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeOnenoteNotebookSectionPageId has no segments")
	}
