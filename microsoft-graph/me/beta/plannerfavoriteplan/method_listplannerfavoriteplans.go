...
func (c PlannerFavoritePlanClient) ListPlannerFavoritePlans(ctx context.Context) (result ListPlannerFavoritePlansOperationResponse, err error) {
...
func (c PlannerFavoritePlanClient) ListPlannerFavoritePlansComplete(ctx context.Context) (ListPlannerFavoritePlansCompleteResult, error) {
	return c.ListPlannerFavoritePlansCompleteMatchingPredicate(ctx, PlannerPlanOperationPredicate{})
}

// ListPlannerFavoritePlansCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c PlannerFavoritePlanClient) ListPlannerFavoritePlansCompleteMatchingPredicate(ctx context.Context, predicate PlannerPlanOperationPredicate) (result ListPlannerFavoritePlansCompleteResult, err error) {
	items := make([]beta.PlannerPlan, 0)

	resp, err := c.ListPlannerFavoritePlans(ctx)
	if err != nil {
