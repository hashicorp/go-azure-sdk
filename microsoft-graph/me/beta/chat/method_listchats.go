...
// ListChats ...
func (c ChatClient) ListChats(ctx context.Context) (result ListChatsOperationResponse, err error) {
...
func (c ChatClient) ListChatsComplete(ctx context.Context) (ListChatsCompleteResult, error) {
	return c.ListChatsCompleteMatchingPredicate(ctx, ChatOperationPredicate{})
}

// ListChatsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ChatClient) ListChatsCompleteMatchingPredicate(ctx context.Context, predicate ChatOperationPredicate) (result ListChatsCompleteResult, err error) {
	items := make([]beta.Chat, 0)

	resp, err := c.ListChats(ctx)
	if err != nil {
