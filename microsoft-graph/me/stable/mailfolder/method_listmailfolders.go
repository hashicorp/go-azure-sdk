...
// ListMailFolders ...
func (c MailFolderClient) ListMailFolders(ctx context.Context) (result ListMailFoldersOperationResponse, err error) {
...
func (c MailFolderClient) ListMailFoldersComplete(ctx context.Context) (ListMailFoldersCompleteResult, error) {
	return c.ListMailFoldersCompleteMatchingPredicate(ctx, MailFolderOperationPredicate{})
}

// ListMailFoldersCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c MailFolderClient) ListMailFoldersCompleteMatchingPredicate(ctx context.Context, predicate MailFolderOperationPredicate) (result ListMailFoldersCompleteResult, err error) {
	items := make([]stable.MailFolder, 0)

	resp, err := c.ListMailFolders(ctx)
	if err != nil {
