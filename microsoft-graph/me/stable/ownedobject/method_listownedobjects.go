...
// ListOwnedObjects ...
func (c OwnedObjectClient) ListOwnedObjects(ctx context.Context) (result ListOwnedObjectsOperationResponse, err error) {
...
func (c OwnedObjectClient) ListOwnedObjectsComplete(ctx context.Context) (ListOwnedObjectsCompleteResult, error) {
	return c.ListOwnedObjectsCompleteMatchingPredicate(ctx, DirectoryObjectOperationPredicate{})
}

// ListOwnedObjectsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OwnedObjectClient) ListOwnedObjectsCompleteMatchingPredicate(ctx context.Context, predicate DirectoryObjectOperationPredicate) (result ListOwnedObjectsCompleteResult, err error) {
	items := make([]stable.DirectoryObject, 0)

	resp, err := c.ListOwnedObjects(ctx)
	if err != nil {
