...
func (c OwnedObjectClient) ListOwnedObjectGroups(ctx context.Context, options ListOwnedObjectGroupsOperationOptions) (result ListOwnedObjectGroupsOperationResponse, err error) {
...
func (c OwnedObjectClient) ListOwnedObjectGroupsComplete(ctx context.Context, options ListOwnedObjectGroupsOperationOptions) (ListOwnedObjectGroupsCompleteResult, error) {
	return c.ListOwnedObjectGroupsCompleteMatchingPredicate(ctx, options, GroupOperationPredicate{})
}

// ListOwnedObjectGroupsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OwnedObjectClient) ListOwnedObjectGroupsCompleteMatchingPredicate(ctx context.Context, options ListOwnedObjectGroupsOperationOptions, predicate GroupOperationPredicate) (result ListOwnedObjectGroupsCompleteResult, err error) {
	items := make([]stable.Group, 0)

	resp, err := c.ListOwnedObjectGroups(ctx, options)
	if err != nil {
