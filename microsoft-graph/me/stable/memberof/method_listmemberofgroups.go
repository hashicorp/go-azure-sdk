...
// ListMemberOfGroups - Get the items of type microsoft.graph.group in the microsoft.graph.directoryObject collection
func (c MemberOfClient) ListMemberOfGroups(ctx context.Context, options ListMemberOfGroupsOperationOptions) (result ListMemberOfGroupsOperationResponse, err error) {
...
func (c MemberOfClient) ListMemberOfGroupsComplete(ctx context.Context, options ListMemberOfGroupsOperationOptions) (ListMemberOfGroupsCompleteResult, error) {
	return c.ListMemberOfGroupsCompleteMatchingPredicate(ctx, options, GroupOperationPredicate{})
}

// ListMemberOfGroupsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c MemberOfClient) ListMemberOfGroupsCompleteMatchingPredicate(ctx context.Context, options ListMemberOfGroupsOperationOptions, predicate GroupOperationPredicate) (result ListMemberOfGroupsCompleteResult, err error) {
	items := make([]stable.Group, 0)

	resp, err := c.ListMemberOfGroups(ctx, options)
	if err != nil {
