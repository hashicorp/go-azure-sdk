...
// ListOwnedDevices ...
func (c OwnedDeviceClient) ListOwnedDevices(ctx context.Context) (result ListOwnedDevicesOperationResponse, err error) {
...
func (c OwnedDeviceClient) ListOwnedDevicesComplete(ctx context.Context) (ListOwnedDevicesCompleteResult, error) {
	return c.ListOwnedDevicesCompleteMatchingPredicate(ctx, DirectoryObjectOperationPredicate{})
}

// ListOwnedDevicesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OwnedDeviceClient) ListOwnedDevicesCompleteMatchingPredicate(ctx context.Context, predicate DirectoryObjectOperationPredicate) (result ListOwnedDevicesCompleteResult, err error) {
	items := make([]stable.DirectoryObject, 0)

	resp, err := c.ListOwnedDevices(ctx)
	if err != nil {
