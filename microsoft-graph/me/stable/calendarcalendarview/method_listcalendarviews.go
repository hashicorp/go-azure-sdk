...
// view defined by a time range, from a user's default calendar (../me/calendarView) or some other calendar of the
// user's.
func (c CalendarCalendarViewClient) ListCalendarViews(ctx context.Context, options ListCalendarViewsOperationOptions) (result ListCalendarViewsOperationResponse, err error) {
...
func (c CalendarCalendarViewClient) ListCalendarViewsComplete(ctx context.Context, options ListCalendarViewsOperationOptions) (ListCalendarViewsCompleteResult, error) {
	return c.ListCalendarViewsCompleteMatchingPredicate(ctx, options, EventOperationPredicate{})
}

// ListCalendarViewsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c CalendarCalendarViewClient) ListCalendarViewsCompleteMatchingPredicate(ctx context.Context, options ListCalendarViewsOperationOptions, predicate EventOperationPredicate) (result ListCalendarViewsCompleteResult, err error) {
	items := make([]stable.Event, 0)

	resp, err := c.ListCalendarViews(ctx, options)
	if err != nil {
