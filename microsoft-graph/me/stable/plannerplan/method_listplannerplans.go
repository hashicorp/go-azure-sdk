package plannerplan

import (
	"context"
	"fmt"
	"net/http"

	"github.com/hashicorp/go-azure-sdk/microsoft-graph/common-types/stable"
	"github.com/hashicorp/go-azure-sdk/sdk/client"
	"github.com/hashicorp/go-azure-sdk/sdk/odata"
)

// Copyright (c) HashiCorp Inc. All rights reserved.
// Licensed under the MIT License. See NOTICE.txt in the project root for license information.

type ListPlannerPlansOperationResponse struct {
	HttpResponse *http.Response
	OData        *odata.OData
	Model        *[]stable.PlannerPlan
}

type ListPlannerPlansCompleteResult struct {
	LatestHttpResponse *http.Response
	Items              []stable.PlannerPlan
}

type ListPlannerPlansCustomPager struct {
	NextLink *odata.Link `json:"@odata.nextLink"`
}

func (p *ListPlannerPlansCustomPager) NextPageLink() *odata.Link {
	defer func() {
		p.NextLink = nil
	}()

	return p.NextLink
}

// ListPlannerPlans ...
func (c PlannerPlanClient) ListPlannerPlans(ctx context.Context) (result ListPlannerPlansOperationResponse, err error) {
	opts := client.RequestOptions{
		ContentType: "application/json; charset=utf-8",
		ExpectedStatusCodes: []int{
			http.StatusOK,
		},
		HttpMethod: http.MethodGet,
		Pager:      &ListPlannerPlansCustomPager{},
		Path:       "/me/planner/plans",
	}

	req, err := c.Client.NewRequest(ctx, opts)
	if err != nil {
		return
	}

	var resp *client.Response
	resp, err = req.ExecutePaged(ctx)
	if resp != nil {
		result.OData = resp.OData
		result.HttpResponse = resp.Response
	}
	if err != nil {
		return
	}

	var values struct {
		Values *[]stable.PlannerPlan `json:"value"`
	}
	if err = resp.Unmarshal(&values); err != nil {
		return
	}

	result.Model = values.Values

	return
}

// ListPlannerPlansComplete retrieves all the results into a single object
func (c PlannerPlanClient) ListPlannerPlansComplete(ctx context.Context) (ListPlannerPlansCompleteResult, error) {
	return c.ListPlannerPlansCompleteMatchingPredicate(ctx, PlannerPlanOperationPredicate{})
}

// ListPlannerPlansCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c PlannerPlanClient) ListPlannerPlansCompleteMatchingPredicate(ctx context.Context, predicate PlannerPlanOperationPredicate) (result ListPlannerPlansCompleteResult, err error) {
	items := make([]stable.PlannerPlan, 0)

	resp, err := c.ListPlannerPlans(ctx)
	if err != nil {
		result.LatestHttpResponse = resp.HttpResponse
		err = fmt.Errorf("loading results: %+v", err)
		return
	}
	if resp.Model != nil {
		for _, v := range *resp.Model {
			if predicate.Matches(v) {
				items = append(items, v)
			}
		}
	}

	result = ListPlannerPlansCompleteResult{
		LatestHttpResponse: resp.HttpResponse,
		Items:              items,
	}
	return
}
