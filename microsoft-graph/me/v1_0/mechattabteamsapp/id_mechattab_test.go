...
func TestNewMeChatTabID(t *testing.T) {
	id := NewMeChatTabID("chatIdValue", "teamsTabIdValue")
...
func TestFormatMeChatTabID(t *testing.T) {
	actual := NewMeChatTabID("chatIdValue", "teamsTabIdValue").ID()
...
func TestParseMeChatTabID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeChatTabId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseMeChatTabID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeChatTabIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeChatTabId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeChatTabIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeChatTabId(t *testing.T) {
	segments := MeChatTabId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeChatTabId has no segments")
	}
