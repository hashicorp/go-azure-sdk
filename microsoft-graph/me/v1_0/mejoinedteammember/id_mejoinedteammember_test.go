...
func TestNewMeJoinedTeamMemberID(t *testing.T) {
	id := NewMeJoinedTeamMemberID("teamIdValue", "conversationMemberIdValue")
...
func TestFormatMeJoinedTeamMemberID(t *testing.T) {
	actual := NewMeJoinedTeamMemberID("teamIdValue", "conversationMemberIdValue").ID()
...
func TestParseMeJoinedTeamMemberID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeJoinedTeamMemberId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseMeJoinedTeamMemberID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeJoinedTeamMemberIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeJoinedTeamMemberId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeJoinedTeamMemberIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeJoinedTeamMemberId(t *testing.T) {
	segments := MeJoinedTeamMemberId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeJoinedTeamMemberId has no segments")
	}
