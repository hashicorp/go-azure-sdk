...
func TestNewMePlannerPlanTaskID(t *testing.T) {
	id := NewMePlannerPlanTaskID("plannerPlanIdValue", "plannerTaskIdValue")
...
func TestFormatMePlannerPlanTaskID(t *testing.T) {
	actual := NewMePlannerPlanTaskID("plannerPlanIdValue", "plannerTaskIdValue").ID()
...
func TestParseMePlannerPlanTaskID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MePlannerPlanTaskId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseMePlannerPlanTaskID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMePlannerPlanTaskIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MePlannerPlanTaskId
	...
	for _, v := range testData {
		...
		actual, err := ParseMePlannerPlanTaskIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMePlannerPlanTaskId(t *testing.T) {
	segments := MePlannerPlanTaskId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MePlannerPlanTaskId has no segments")
	}
