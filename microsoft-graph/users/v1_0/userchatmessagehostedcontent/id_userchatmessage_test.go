...
func TestNewUserChatMessageID(t *testing.T) {
	id := NewUserChatMessageID("userIdValue", "chatIdValue", "chatMessageIdValue")
...
func TestFormatUserChatMessageID(t *testing.T) {
	actual := NewUserChatMessageID("userIdValue", "chatIdValue", "chatMessageIdValue").ID()
...
func TestParseUserChatMessageID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatMessageId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatMessageID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserChatMessageIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatMessageId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatMessageIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserChatMessageId(t *testing.T) {
	segments := UserChatMessageId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserChatMessageId has no segments")
	}
