...
func TestNewUserAuthenticationTemporaryAccessPassMethodID(t *testing.T) {
	id := NewUserAuthenticationTemporaryAccessPassMethodID("userIdValue", "temporaryAccessPassAuthenticationMethodIdValue")
...
func TestFormatUserAuthenticationTemporaryAccessPassMethodID(t *testing.T) {
	actual := NewUserAuthenticationTemporaryAccessPassMethodID("userIdValue", "temporaryAccessPassAuthenticationMethodIdValue").ID()
...
func TestParseUserAuthenticationTemporaryAccessPassMethodID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserAuthenticationTemporaryAccessPassMethodId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserAuthenticationTemporaryAccessPassMethodID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserAuthenticationTemporaryAccessPassMethodIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserAuthenticationTemporaryAccessPassMethodId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserAuthenticationTemporaryAccessPassMethodIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserAuthenticationTemporaryAccessPassMethodId(t *testing.T) {
	segments := UserAuthenticationTemporaryAccessPassMethodId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserAuthenticationTemporaryAccessPassMethodId has no segments")
	}
