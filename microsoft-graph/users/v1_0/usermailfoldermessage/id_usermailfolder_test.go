...
func TestNewUserMailFolderID(t *testing.T) {
	id := NewUserMailFolderID("userIdValue", "mailFolderIdValue")
...
func TestFormatUserMailFolderID(t *testing.T) {
	actual := NewUserMailFolderID("userIdValue", "mailFolderIdValue").ID()
...
func TestParseUserMailFolderID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserMailFolderIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserMailFolderId(t *testing.T) {
	segments := UserMailFolderId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserMailFolderId has no segments")
	}
