...
func TestNewUserChatInstalledAppID(t *testing.T) {
	id := NewUserChatInstalledAppID("userIdValue", "chatIdValue", "teamsAppInstallationIdValue")
...
func TestFormatUserChatInstalledAppID(t *testing.T) {
	actual := NewUserChatInstalledAppID("userIdValue", "chatIdValue", "teamsAppInstallationIdValue").ID()
...
func TestParseUserChatInstalledAppID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatInstalledAppId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatInstalledAppID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserChatInstalledAppIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatInstalledAppId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatInstalledAppIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserChatInstalledAppId(t *testing.T) {
	segments := UserChatInstalledAppId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserChatInstalledAppId has no segments")
	}
