...
func TestNewUserInsightUsedID(t *testing.T) {
	id := NewUserInsightUsedID("userIdValue", "usedInsightIdValue")
...
func TestFormatUserInsightUsedID(t *testing.T) {
	actual := NewUserInsightUsedID("userIdValue", "usedInsightIdValue").ID()
...
func TestParseUserInsightUsedID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserInsightUsedId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserInsightUsedID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserInsightUsedIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserInsightUsedId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserInsightUsedIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserInsightUsedId(t *testing.T) {
	segments := UserInsightUsedId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserInsightUsedId has no segments")
	}
