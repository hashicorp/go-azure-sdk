...
func TestNewUserInsightTrendingID(t *testing.T) {
	id := NewUserInsightTrendingID("userIdValue", "trendingIdValue")
...
func TestFormatUserInsightTrendingID(t *testing.T) {
	actual := NewUserInsightTrendingID("userIdValue", "trendingIdValue").ID()
...
func TestParseUserInsightTrendingID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserInsightTrendingId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserInsightTrendingID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserInsightTrendingIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserInsightTrendingId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserInsightTrendingIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserInsightTrendingId(t *testing.T) {
	segments := UserInsightTrendingId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserInsightTrendingId has no segments")
	}
