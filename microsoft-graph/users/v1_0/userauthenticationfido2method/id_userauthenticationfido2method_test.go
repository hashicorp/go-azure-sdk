...
func TestNewUserAuthenticationFido2MethodID(t *testing.T) {
	id := NewUserAuthenticationFido2MethodID("userIdValue", "fido2AuthenticationMethodIdValue")
...
func TestFormatUserAuthenticationFido2MethodID(t *testing.T) {
	actual := NewUserAuthenticationFido2MethodID("userIdValue", "fido2AuthenticationMethodIdValue").ID()
...
func TestParseUserAuthenticationFido2MethodID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserAuthenticationFido2MethodId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserAuthenticationFido2MethodID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserAuthenticationFido2MethodIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserAuthenticationFido2MethodId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserAuthenticationFido2MethodIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserAuthenticationFido2MethodId(t *testing.T) {
	segments := UserAuthenticationFido2MethodId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserAuthenticationFido2MethodId has no segments")
	}
