...
func TestNewUserLicenseDetailID(t *testing.T) {
	id := NewUserLicenseDetailID("userIdValue", "licenseDetailsIdValue")
...
func TestFormatUserLicenseDetailID(t *testing.T) {
	actual := NewUserLicenseDetailID("userIdValue", "licenseDetailsIdValue").ID()
...
func TestParseUserLicenseDetailID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserLicenseDetailId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserLicenseDetailID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserLicenseDetailIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserLicenseDetailId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserLicenseDetailIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserLicenseDetailId(t *testing.T) {
	segments := UserLicenseDetailId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserLicenseDetailId has no segments")
	}
