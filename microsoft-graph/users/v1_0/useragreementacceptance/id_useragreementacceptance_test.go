...
func TestNewUserAgreementAcceptanceID(t *testing.T) {
	id := NewUserAgreementAcceptanceID("userIdValue", "agreementAcceptanceIdValue")
...
func TestFormatUserAgreementAcceptanceID(t *testing.T) {
	actual := NewUserAgreementAcceptanceID("userIdValue", "agreementAcceptanceIdValue").ID()
...
func TestParseUserAgreementAcceptanceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserAgreementAcceptanceId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserAgreementAcceptanceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserAgreementAcceptanceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserAgreementAcceptanceId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserAgreementAcceptanceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserAgreementAcceptanceId(t *testing.T) {
	segments := UserAgreementAcceptanceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserAgreementAcceptanceId has no segments")
	}
