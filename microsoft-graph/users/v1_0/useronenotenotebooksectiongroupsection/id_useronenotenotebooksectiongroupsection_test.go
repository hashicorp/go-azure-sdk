...
func TestNewUserOnenoteNotebookSectionGroupSectionID(t *testing.T) {
	id := NewUserOnenoteNotebookSectionGroupSectionID("userIdValue", "notebookIdValue", "sectionGroupIdValue", "onenoteSectionIdValue")
...
func TestFormatUserOnenoteNotebookSectionGroupSectionID(t *testing.T) {
	actual := NewUserOnenoteNotebookSectionGroupSectionID("userIdValue", "notebookIdValue", "sectionGroupIdValue", "onenoteSectionIdValue").ID()
	expected := "/users/userIdValue/onenote/notebooks/notebookIdValue/sectionGroups/sectionGroupIdValue/sections/onenoteSectionIdValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseUserOnenoteNotebookSectionGroupSectionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOnenoteNotebookSectionGroupSectionId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/users/userIdValue/onenote/notebooks/notebookIdValue/sectionGroups/sectionGroupIdValue/sections/onenoteSectionIdValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseUserOnenoteNotebookSectionGroupSectionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserOnenoteNotebookSectionGroupSectionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOnenoteNotebookSectionGroupSectionId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOnenoteNotebookSectionGroupSectionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserOnenoteNotebookSectionGroupSectionId(t *testing.T) {
	segments := UserOnenoteNotebookSectionGroupSectionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserOnenoteNotebookSectionGroupSectionId has no segments")
	}
