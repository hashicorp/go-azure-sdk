...
func TestNewUserChatMessageReplyID(t *testing.T) {
	id := NewUserChatMessageReplyID("userIdValue", "chatIdValue", "chatMessageIdValue", "chatMessageId1Value")
...
func TestFormatUserChatMessageReplyID(t *testing.T) {
	actual := NewUserChatMessageReplyID("userIdValue", "chatIdValue", "chatMessageIdValue", "chatMessageId1Value").ID()
	expected := "/users/userIdValue/chats/chatIdValue/messages/chatMessageIdValue/replies/chatMessageId1Value"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseUserChatMessageReplyID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatMessageReplyId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/users/userIdValue/chats/chatIdValue/messages/chatMessageIdValue/replies/chatMessageId1Value/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseUserChatMessageReplyID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserChatMessageReplyIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatMessageReplyId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatMessageReplyIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserChatMessageReplyId(t *testing.T) {
	segments := UserChatMessageReplyId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserChatMessageReplyId has no segments")
	}
