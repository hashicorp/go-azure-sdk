...
func TestNewUserChatTabID(t *testing.T) {
	id := NewUserChatTabID("userIdValue", "chatIdValue", "teamsTabIdValue")
...
func TestFormatUserChatTabID(t *testing.T) {
	actual := NewUserChatTabID("userIdValue", "chatIdValue", "teamsTabIdValue").ID()
...
func TestParseUserChatTabID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatTabId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatTabID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserChatTabIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatTabId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatTabIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserChatTabId(t *testing.T) {
	segments := UserChatTabId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserChatTabId has no segments")
	}
