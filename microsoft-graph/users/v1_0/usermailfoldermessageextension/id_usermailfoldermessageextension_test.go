...
func TestNewUserMailFolderMessageExtensionID(t *testing.T) {
	id := NewUserMailFolderMessageExtensionID("userIdValue", "mailFolderIdValue", "messageIdValue", "extensionIdValue")
...
func TestFormatUserMailFolderMessageExtensionID(t *testing.T) {
	actual := NewUserMailFolderMessageExtensionID("userIdValue", "mailFolderIdValue", "messageIdValue", "extensionIdValue").ID()
...
func TestParseUserMailFolderMessageExtensionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderMessageExtensionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderMessageExtensionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserMailFolderMessageExtensionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderMessageExtensionId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderMessageExtensionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserMailFolderMessageExtensionId(t *testing.T) {
	segments := UserMailFolderMessageExtensionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserMailFolderMessageExtensionId has no segments")
	}
