...
func TestNewUserChatID(t *testing.T) {
	id := NewUserChatID("userIdValue", "chatIdValue")
...
func TestFormatUserChatID(t *testing.T) {
	actual := NewUserChatID("userIdValue", "chatIdValue").ID()
...
func TestParseUserChatID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserChatIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserChatId(t *testing.T) {
	segments := UserChatId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserChatId has no segments")
	}
