...
// ListProfilePatents ...
func (c ProfilePatentClient) ListProfilePatents(ctx context.Context, id UserId) (result ListProfilePatentsOperationResponse, err error) {
...
func (c ProfilePatentClient) ListProfilePatentsComplete(ctx context.Context, id UserId) (ListProfilePatentsCompleteResult, error) {
	return c.ListProfilePatentsCompleteMatchingPredicate(ctx, id, ItemPatentOperationPredicate{})
}

// ListProfilePatentsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ProfilePatentClient) ListProfilePatentsCompleteMatchingPredicate(ctx context.Context, id UserId, predicate ItemPatentOperationPredicate) (result ListProfilePatentsCompleteResult, err error) {
	items := make([]beta.ItemPatent, 0)

	resp, err := c.ListProfilePatents(ctx, id)
	if err != nil {
