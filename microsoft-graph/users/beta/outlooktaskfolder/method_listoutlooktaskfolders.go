...
func (c OutlookTaskFolderClient) ListOutlookTaskFolders(ctx context.Context, id UserId) (result ListOutlookTaskFoldersOperationResponse, err error) {
...
func (c OutlookTaskFolderClient) ListOutlookTaskFoldersComplete(ctx context.Context, id UserId) (ListOutlookTaskFoldersCompleteResult, error) {
	return c.ListOutlookTaskFoldersCompleteMatchingPredicate(ctx, id, OutlookTaskFolderOperationPredicate{})
}

// ListOutlookTaskFoldersCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OutlookTaskFolderClient) ListOutlookTaskFoldersCompleteMatchingPredicate(ctx context.Context, id UserId, predicate OutlookTaskFolderOperationPredicate) (result ListOutlookTaskFoldersCompleteResult, err error) {
	items := make([]beta.OutlookTaskFolder, 0)

	resp, err := c.ListOutlookTaskFolders(ctx, id)
	if err != nil {
