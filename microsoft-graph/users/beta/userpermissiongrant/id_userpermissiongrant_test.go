...
func TestNewUserPermissionGrantID(t *testing.T) {
	id := NewUserPermissionGrantID("userIdValue", "resourceSpecificPermissionGrantIdValue")
...
func TestFormatUserPermissionGrantID(t *testing.T) {
	actual := NewUserPermissionGrantID("userIdValue", "resourceSpecificPermissionGrantIdValue").ID()
...
func TestParseUserPermissionGrantID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserPermissionGrantId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserPermissionGrantID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserPermissionGrantIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserPermissionGrantId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserPermissionGrantIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserPermissionGrantId(t *testing.T) {
	segments := UserPermissionGrantId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserPermissionGrantId has no segments")
	}
