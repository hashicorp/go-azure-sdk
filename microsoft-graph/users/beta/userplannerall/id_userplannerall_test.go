...
func TestNewUserPlannerAllID(t *testing.T) {
	id := NewUserPlannerAllID("userIdValue", "plannerDeltaIdValue")
...
func TestFormatUserPlannerAllID(t *testing.T) {
	actual := NewUserPlannerAllID("userIdValue", "plannerDeltaIdValue").ID()
...
func TestParseUserPlannerAllID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserPlannerAllId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserPlannerAllID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserPlannerAllIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserPlannerAllId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserPlannerAllIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserPlannerAllId(t *testing.T) {
	segments := UserPlannerAllId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserPlannerAllId has no segments")
	}
