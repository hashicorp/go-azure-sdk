...
func TestNewUserPhotoID(t *testing.T) {
	id := NewUserPhotoID("userIdValue", "profilePhotoIdValue")
...
func TestFormatUserPhotoID(t *testing.T) {
	actual := NewUserPhotoID("userIdValue", "profilePhotoIdValue").ID()
...
func TestParseUserPhotoID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserPhotoId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserPhotoID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserPhotoIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserPhotoId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserPhotoIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserPhotoId(t *testing.T) {
	segments := UserPhotoId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserPhotoId has no segments")
	}
