...
func TestNewUserOnenoteSectionID(t *testing.T) {
	id := NewUserOnenoteSectionID("userIdValue", "onenoteSectionIdValue")
...
func TestFormatUserOnenoteSectionID(t *testing.T) {
	actual := NewUserOnenoteSectionID("userIdValue", "onenoteSectionIdValue").ID()
...
func TestParseUserOnenoteSectionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOnenoteSectionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOnenoteSectionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserOnenoteSectionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOnenoteSectionId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOnenoteSectionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserOnenoteSectionId(t *testing.T) {
	segments := UserOnenoteSectionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserOnenoteSectionId has no segments")
	}
