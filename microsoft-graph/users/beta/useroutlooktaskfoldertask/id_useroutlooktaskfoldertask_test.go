...
func TestNewUserOutlookTaskFolderTaskID(t *testing.T) {
	id := NewUserOutlookTaskFolderTaskID("userIdValue", "outlookTaskFolderIdValue", "outlookTaskIdValue")
...
func TestFormatUserOutlookTaskFolderTaskID(t *testing.T) {
	actual := NewUserOutlookTaskFolderTaskID("userIdValue", "outlookTaskFolderIdValue", "outlookTaskIdValue").ID()
...
func TestParseUserOutlookTaskFolderTaskID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOutlookTaskFolderTaskId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOutlookTaskFolderTaskID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserOutlookTaskFolderTaskIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOutlookTaskFolderTaskId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOutlookTaskFolderTaskIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserOutlookTaskFolderTaskId(t *testing.T) {
	segments := UserOutlookTaskFolderTaskId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserOutlookTaskFolderTaskId has no segments")
	}
