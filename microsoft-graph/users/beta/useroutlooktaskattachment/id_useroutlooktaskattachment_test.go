...
func TestNewUserOutlookTaskAttachmentID(t *testing.T) {
	id := NewUserOutlookTaskAttachmentID("userIdValue", "outlookTaskIdValue", "attachmentIdValue")
...
func TestFormatUserOutlookTaskAttachmentID(t *testing.T) {
	actual := NewUserOutlookTaskAttachmentID("userIdValue", "outlookTaskIdValue", "attachmentIdValue").ID()
...
func TestParseUserOutlookTaskAttachmentID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOutlookTaskAttachmentId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOutlookTaskAttachmentID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserOutlookTaskAttachmentIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOutlookTaskAttachmentId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOutlookTaskAttachmentIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserOutlookTaskAttachmentId(t *testing.T) {
	segments := UserOutlookTaskAttachmentId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserOutlookTaskAttachmentId has no segments")
	}
