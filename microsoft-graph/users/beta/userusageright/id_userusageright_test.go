...
func TestNewUserUsageRightID(t *testing.T) {
	id := NewUserUsageRightID("userIdValue", "usageRightIdValue")
...
func TestFormatUserUsageRightID(t *testing.T) {
	actual := NewUserUsageRightID("userIdValue", "usageRightIdValue").ID()
...
func TestParseUserUsageRightID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserUsageRightId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserUsageRightID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserUsageRightIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserUsageRightId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserUsageRightIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserUsageRightId(t *testing.T) {
	segments := UserUsageRightId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserUsageRightId has no segments")
	}
