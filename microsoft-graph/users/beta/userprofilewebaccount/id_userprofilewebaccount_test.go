...
func TestNewUserProfileWebAccountID(t *testing.T) {
	id := NewUserProfileWebAccountID("userIdValue", "webAccountIdValue")
...
func TestFormatUserProfileWebAccountID(t *testing.T) {
	actual := NewUserProfileWebAccountID("userIdValue", "webAccountIdValue").ID()
...
func TestParseUserProfileWebAccountID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserProfileWebAccountId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserProfileWebAccountID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserProfileWebAccountIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserProfileWebAccountId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserProfileWebAccountIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserProfileWebAccountId(t *testing.T) {
	segments := UserProfileWebAccountId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserProfileWebAccountId has no segments")
	}
