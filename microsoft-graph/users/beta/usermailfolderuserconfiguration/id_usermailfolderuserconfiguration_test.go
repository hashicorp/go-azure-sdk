...
func TestNewUserMailFolderUserConfigurationID(t *testing.T) {
	id := NewUserMailFolderUserConfigurationID("userIdValue", "mailFolderIdValue", "userConfigurationIdValue")
...
func TestFormatUserMailFolderUserConfigurationID(t *testing.T) {
	actual := NewUserMailFolderUserConfigurationID("userIdValue", "mailFolderIdValue", "userConfigurationIdValue").ID()
...
func TestParseUserMailFolderUserConfigurationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderUserConfigurationId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderUserConfigurationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserMailFolderUserConfigurationIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderUserConfigurationId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderUserConfigurationIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserMailFolderUserConfigurationId(t *testing.T) {
	segments := UserMailFolderUserConfigurationId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserMailFolderUserConfigurationId has no segments")
	}
