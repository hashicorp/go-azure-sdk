...
func (c PlannerRosterPlanClient) ListPlannerRosterPlans(ctx context.Context, id UserId) (result ListPlannerRosterPlansOperationResponse, err error) {
...
func (c PlannerRosterPlanClient) ListPlannerRosterPlansComplete(ctx context.Context, id UserId) (ListPlannerRosterPlansCompleteResult, error) {
	return c.ListPlannerRosterPlansCompleteMatchingPredicate(ctx, id, PlannerPlanOperationPredicate{})
}

// ListPlannerRosterPlansCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c PlannerRosterPlanClient) ListPlannerRosterPlansCompleteMatchingPredicate(ctx context.Context, id UserId, predicate PlannerPlanOperationPredicate) (result ListPlannerRosterPlansCompleteResult, err error) {
	items := make([]beta.PlannerPlan, 0)

	resp, err := c.ListPlannerRosterPlans(ctx, id)
	if err != nil {
