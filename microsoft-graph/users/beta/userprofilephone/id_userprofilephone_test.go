...
func TestNewUserProfilePhoneID(t *testing.T) {
	id := NewUserProfilePhoneID("userIdValue", "itemPhoneIdValue")
...
func TestFormatUserProfilePhoneID(t *testing.T) {
	actual := NewUserProfilePhoneID("userIdValue", "itemPhoneIdValue").ID()
...
func TestParseUserProfilePhoneID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserProfilePhoneId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserProfilePhoneID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserProfilePhoneIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserProfilePhoneId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserProfilePhoneIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserProfilePhoneId(t *testing.T) {
	segments := UserProfilePhoneId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserProfilePhoneId has no segments")
	}
