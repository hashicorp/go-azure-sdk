...
func TestNewUserMailFolderChildFolderID(t *testing.T) {
	id := NewUserMailFolderChildFolderID("userIdValue", "mailFolderIdValue", "mailFolderId1Value")
...
func TestFormatUserMailFolderChildFolderID(t *testing.T) {
	actual := NewUserMailFolderChildFolderID("userIdValue", "mailFolderIdValue", "mailFolderId1Value").ID()
	expected := "/users/userIdValue/mailFolders/mailFolderIdValue/childFolders/mailFolderId1Value"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseUserMailFolderChildFolderID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderChildFolderId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/users/userIdValue/mailFolders/mailFolderIdValue/childFolders/mailFolderId1Value/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderChildFolderID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserMailFolderChildFolderIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderChildFolderId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderChildFolderIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserMailFolderChildFolderId(t *testing.T) {
	segments := UserMailFolderChildFolderId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserMailFolderChildFolderId has no segments")
	}
