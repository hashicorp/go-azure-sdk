...
func TestNewUserMailFolderChildFolderMessageMentionID(t *testing.T) {
	id := NewUserMailFolderChildFolderMessageMentionID("userIdValue", "mailFolderIdValue", "mailFolderId1Value", "messageIdValue", "mentionIdValue")
...
func TestFormatUserMailFolderChildFolderMessageMentionID(t *testing.T) {
	actual := NewUserMailFolderChildFolderMessageMentionID("userIdValue", "mailFolderIdValue", "mailFolderId1Value", "messageIdValue", "mentionIdValue").ID()
...
func TestParseUserMailFolderChildFolderMessageMentionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderChildFolderMessageMentionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderChildFolderMessageMentionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserMailFolderChildFolderMessageMentionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderChildFolderMessageMentionId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderChildFolderMessageMentionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserMailFolderChildFolderMessageMentionId(t *testing.T) {
	segments := UserMailFolderChildFolderMessageMentionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserMailFolderChildFolderMessageMentionId has no segments")
	}
