...
// ListProfileNames ...
func (c ProfileNameClient) ListProfileNames(ctx context.Context, id UserId) (result ListProfileNamesOperationResponse, err error) {
...
func (c ProfileNameClient) ListProfileNamesComplete(ctx context.Context, id UserId) (ListProfileNamesCompleteResult, error) {
	return c.ListProfileNamesCompleteMatchingPredicate(ctx, id, PersonNameOperationPredicate{})
}

// ListProfileNamesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ProfileNameClient) ListProfileNamesCompleteMatchingPredicate(ctx context.Context, id UserId, predicate PersonNameOperationPredicate) (result ListProfileNamesCompleteResult, err error) {
	items := make([]beta.PersonName, 0)

	resp, err := c.ListProfileNames(ctx, id)
	if err != nil {
