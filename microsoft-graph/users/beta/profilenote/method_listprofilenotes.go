...
// ListProfileNotes ...
func (c ProfileNoteClient) ListProfileNotes(ctx context.Context, id UserId) (result ListProfileNotesOperationResponse, err error) {
...
func (c ProfileNoteClient) ListProfileNotesComplete(ctx context.Context, id UserId) (ListProfileNotesCompleteResult, error) {
	return c.ListProfileNotesCompleteMatchingPredicate(ctx, id, PersonAnnotationOperationPredicate{})
}

// ListProfileNotesCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c ProfileNoteClient) ListProfileNotesCompleteMatchingPredicate(ctx context.Context, id UserId, predicate PersonAnnotationOperationPredicate) (result ListProfileNotesCompleteResult, err error) {
	items := make([]beta.PersonAnnotation, 0)

	resp, err := c.ListProfileNotes(ctx, id)
	if err != nil {
