...
func TestNewUserProfileSkillID(t *testing.T) {
	id := NewUserProfileSkillID("userIdValue", "skillProficiencyIdValue")
...
func TestFormatUserProfileSkillID(t *testing.T) {
	actual := NewUserProfileSkillID("userIdValue", "skillProficiencyIdValue").ID()
...
func TestParseUserProfileSkillID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserProfileSkillId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserProfileSkillID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserProfileSkillIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserProfileSkillId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserProfileSkillIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserProfileSkillId(t *testing.T) {
	segments := UserProfileSkillId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserProfileSkillId has no segments")
	}
