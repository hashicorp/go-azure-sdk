...
func TestNewUserOutlookTaskGroupTaskFolderID(t *testing.T) {
	id := NewUserOutlookTaskGroupTaskFolderID("userIdValue", "outlookTaskGroupIdValue", "outlookTaskFolderIdValue")
...
func TestFormatUserOutlookTaskGroupTaskFolderID(t *testing.T) {
	actual := NewUserOutlookTaskGroupTaskFolderID("userIdValue", "outlookTaskGroupIdValue", "outlookTaskFolderIdValue").ID()
...
func TestParseUserOutlookTaskGroupTaskFolderID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOutlookTaskGroupTaskFolderId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOutlookTaskGroupTaskFolderID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserOutlookTaskGroupTaskFolderIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOutlookTaskGroupTaskFolderId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOutlookTaskGroupTaskFolderIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserOutlookTaskGroupTaskFolderId(t *testing.T) {
	segments := UserOutlookTaskGroupTaskFolderId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserOutlookTaskGroupTaskFolderId has no segments")
	}
