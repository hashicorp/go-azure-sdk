...
func TestNewUserCalendarGroupCalendarID(t *testing.T) {
	id := NewUserCalendarGroupCalendarID("userIdValue", "calendarGroupIdValue", "calendarIdValue")
...
func TestFormatUserCalendarGroupCalendarID(t *testing.T) {
	actual := NewUserCalendarGroupCalendarID("userIdValue", "calendarGroupIdValue", "calendarIdValue").ID()
...
func TestParseUserCalendarGroupCalendarID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserCalendarGroupCalendarId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserCalendarGroupCalendarID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserCalendarGroupCalendarIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserCalendarGroupCalendarId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserCalendarGroupCalendarIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserCalendarGroupCalendarId(t *testing.T) {
	segments := UserCalendarGroupCalendarId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserCalendarGroupCalendarId has no segments")
	}
