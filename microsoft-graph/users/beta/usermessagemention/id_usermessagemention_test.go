...
func TestNewUserMessageMentionID(t *testing.T) {
	id := NewUserMessageMentionID("userIdValue", "messageIdValue", "mentionIdValue")
...
func TestFormatUserMessageMentionID(t *testing.T) {
	actual := NewUserMessageMentionID("userIdValue", "messageIdValue", "mentionIdValue").ID()
...
func TestParseUserMessageMentionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMessageMentionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMessageMentionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserMessageMentionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMessageMentionId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMessageMentionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserMessageMentionId(t *testing.T) {
	segments := UserMessageMentionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserMessageMentionId has no segments")
	}
