...
// ListOutlookTasks ...
func (c OutlookTaskClient) ListOutlookTasks(ctx context.Context, id UserId) (result ListOutlookTasksOperationResponse, err error) {
...
func (c OutlookTaskClient) ListOutlookTasksComplete(ctx context.Context, id UserId) (ListOutlookTasksCompleteResult, error) {
	return c.ListOutlookTasksCompleteMatchingPredicate(ctx, id, OutlookTaskOperationPredicate{})
}

// ListOutlookTasksCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c OutlookTaskClient) ListOutlookTasksCompleteMatchingPredicate(ctx context.Context, id UserId, predicate OutlookTaskOperationPredicate) (result ListOutlookTasksCompleteResult, err error) {
	items := make([]beta.OutlookTask, 0)

	resp, err := c.ListOutlookTasks(ctx, id)
	if err != nil {
