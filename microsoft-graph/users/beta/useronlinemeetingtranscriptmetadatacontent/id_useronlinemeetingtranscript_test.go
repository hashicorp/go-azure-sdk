...
func TestNewUserOnlineMeetingTranscriptID(t *testing.T) {
	id := NewUserOnlineMeetingTranscriptID("userIdValue", "onlineMeetingIdValue", "callTranscriptIdValue")
...
func TestFormatUserOnlineMeetingTranscriptID(t *testing.T) {
	actual := NewUserOnlineMeetingTranscriptID("userIdValue", "onlineMeetingIdValue", "callTranscriptIdValue").ID()
...
func TestParseUserOnlineMeetingTranscriptID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOnlineMeetingTranscriptId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOnlineMeetingTranscriptID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserOnlineMeetingTranscriptIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserOnlineMeetingTranscriptId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserOnlineMeetingTranscriptIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserOnlineMeetingTranscriptId(t *testing.T) {
	segments := UserOnlineMeetingTranscriptId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserOnlineMeetingTranscriptId has no segments")
	}
