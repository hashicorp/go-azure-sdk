...
func TestNewUserChatMemberID(t *testing.T) {
	id := NewUserChatMemberID("userIdValue", "chatIdValue", "conversationMemberIdValue")
...
func TestFormatUserChatMemberID(t *testing.T) {
	actual := NewUserChatMemberID("userIdValue", "chatIdValue", "conversationMemberIdValue").ID()
...
func TestParseUserChatMemberID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatMemberId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatMemberID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserChatMemberIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserChatMemberId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserChatMemberIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserChatMemberId(t *testing.T) {
	segments := UserChatMemberId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserChatMemberId has no segments")
	}
