...
func TestNewUserMailFolderChildFolderMessageID(t *testing.T) {
	id := NewUserMailFolderChildFolderMessageID("userIdValue", "mailFolderIdValue", "mailFolderId1Value", "messageIdValue")
...
func TestFormatUserMailFolderChildFolderMessageID(t *testing.T) {
	actual := NewUserMailFolderChildFolderMessageID("userIdValue", "mailFolderIdValue", "mailFolderId1Value", "messageIdValue").ID()
...
func TestParseUserMailFolderChildFolderMessageID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderChildFolderMessageId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderChildFolderMessageID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserMailFolderChildFolderMessageIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderChildFolderMessageId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderChildFolderMessageIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserMailFolderChildFolderMessageId(t *testing.T) {
	segments := UserMailFolderChildFolderMessageId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserMailFolderChildFolderMessageId has no segments")
	}
