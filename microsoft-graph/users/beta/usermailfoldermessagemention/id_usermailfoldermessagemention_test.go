...
func TestNewUserMailFolderMessageMentionID(t *testing.T) {
	id := NewUserMailFolderMessageMentionID("userIdValue", "mailFolderIdValue", "messageIdValue", "mentionIdValue")
...
func TestFormatUserMailFolderMessageMentionID(t *testing.T) {
	actual := NewUserMailFolderMessageMentionID("userIdValue", "mailFolderIdValue", "messageIdValue", "mentionIdValue").ID()
...
func TestParseUserMailFolderMessageMentionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderMessageMentionId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderMessageMentionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserMailFolderMessageMentionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserMailFolderMessageMentionId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserMailFolderMessageMentionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserMailFolderMessageMentionId(t *testing.T) {
	segments := UserMailFolderMessageMentionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserMailFolderMessageMentionId has no segments")
	}
