...
The `drivelistitem` SDK allows for interaction with the Azure Resource Manager Service `users` (API Version `stable`).
...
```go
ctx := context.TODO()
id := drivelistitem.NewUserIdDriveID("userIdValue", "driveIdValue")
...
```go
ctx := context.TODO()
id := drivelistitem.NewUserIdDriveIdListItemID("userIdValue", "driveIdValue", "listItemIdValue")
