...
// collection
func (c DirectReportClient) ListDirectReportOrgContacts(ctx context.Context, id stable.UserId, options ListDirectReportOrgContactsOperationOptions) (result ListDirectReportOrgContactsOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:          fmt.Sprintf("%s/directReports/orgContact", id.ID()),
...
func (c DirectReportClient) ListDirectReportOrgContactsComplete(ctx context.Context, id stable.UserId, options ListDirectReportOrgContactsOperationOptions) (ListDirectReportOrgContactsCompleteResult, error) {
	return c.ListDirectReportOrgContactsCompleteMatchingPredicate(ctx, id, options, OrgContactOperationPredicate{})
}

// ListDirectReportOrgContactsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c DirectReportClient) ListDirectReportOrgContactsCompleteMatchingPredicate(ctx context.Context, id stable.UserId, options ListDirectReportOrgContactsOperationOptions, predicate OrgContactOperationPredicate) (result ListDirectReportOrgContactsCompleteResult, err error) {
	items := make([]stable.OrgContact, 0)

	resp, err := c.ListDirectReportOrgContacts(ctx, id, options)
	if err != nil {
