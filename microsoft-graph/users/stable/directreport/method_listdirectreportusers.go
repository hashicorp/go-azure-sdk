...
func (c DirectReportClient) ListDirectReportUsers(ctx context.Context, id stable.UserId, options ListDirectReportUsersOperationOptions) (result ListDirectReportUsersOperationResponse, err error) {
	opts := client.RequestOptions{
		...
		Path:          fmt.Sprintf("%s/directReports/user", id.ID()),
...
func (c DirectReportClient) ListDirectReportUsersComplete(ctx context.Context, id stable.UserId, options ListDirectReportUsersOperationOptions) (ListDirectReportUsersCompleteResult, error) {
	return c.ListDirectReportUsersCompleteMatchingPredicate(ctx, id, options, UserOperationPredicate{})
}

// ListDirectReportUsersCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c DirectReportClient) ListDirectReportUsersCompleteMatchingPredicate(ctx context.Context, id stable.UserId, options ListDirectReportUsersOperationOptions, predicate UserOperationPredicate) (result ListDirectReportUsersCompleteResult, err error) {
	items := make([]stable.User, 0)

	resp, err := c.ListDirectReportUsers(ctx, id, options)
	if err != nil {
