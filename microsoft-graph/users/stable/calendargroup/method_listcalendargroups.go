...
// ListCalendarGroups - Get calendarGroups from users. The user's calendar groups. Read-only. Nullable.
func (c CalendarGroupClient) ListCalendarGroups(ctx context.Context, id stable.UserId, options ListCalendarGroupsOperationOptions) (result ListCalendarGroupsOperationResponse, err error) {
...
func (c CalendarGroupClient) ListCalendarGroupsComplete(ctx context.Context, id stable.UserId, options ListCalendarGroupsOperationOptions) (ListCalendarGroupsCompleteResult, error) {
	return c.ListCalendarGroupsCompleteMatchingPredicate(ctx, id, options, CalendarGroupOperationPredicate{})
}

// ListCalendarGroupsCompleteMatchingPredicate retrieves all the results and then applies the predicate
func (c CalendarGroupClient) ListCalendarGroupsCompleteMatchingPredicate(ctx context.Context, id stable.UserId, options ListCalendarGroupsOperationOptions, predicate CalendarGroupOperationPredicate) (result ListCalendarGroupsCompleteResult, err error) {
	items := make([]stable.CalendarGroup, 0)

	resp, err := c.ListCalendarGroups(ctx, id, options)
	if err != nil {
