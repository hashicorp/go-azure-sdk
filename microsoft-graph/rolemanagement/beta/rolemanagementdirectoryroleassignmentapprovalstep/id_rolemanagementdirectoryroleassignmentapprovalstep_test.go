...
func TestNewRoleManagementDirectoryRoleAssignmentApprovalStepID(t *testing.T) {
	id := NewRoleManagementDirectoryRoleAssignmentApprovalStepID("approvalIdValue", "approvalStepIdValue")
...
func TestFormatRoleManagementDirectoryRoleAssignmentApprovalStepID(t *testing.T) {
	actual := NewRoleManagementDirectoryRoleAssignmentApprovalStepID("approvalIdValue", "approvalStepIdValue").ID()
...
func TestParseRoleManagementDirectoryRoleAssignmentApprovalStepID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *RoleManagementDirectoryRoleAssignmentApprovalStepId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseRoleManagementDirectoryRoleAssignmentApprovalStepID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseRoleManagementDirectoryRoleAssignmentApprovalStepIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *RoleManagementDirectoryRoleAssignmentApprovalStepId
	...
	for _, v := range testData {
		...
		actual, err := ParseRoleManagementDirectoryRoleAssignmentApprovalStepIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForRoleManagementDirectoryRoleAssignmentApprovalStepId(t *testing.T) {
	segments := RoleManagementDirectoryRoleAssignmentApprovalStepId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("RoleManagementDirectoryRoleAssignmentApprovalStepId has no segments")
	}
